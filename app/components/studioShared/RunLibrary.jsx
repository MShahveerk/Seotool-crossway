...
/**
 * RunLibrary — a studio's run history as a master/detail workspace.
 *
 ...
 *
 * Studio-agnostic. Each studio supplies its own pipeline blueprint (so a live
 * card can count "3 of 4"), the cockpit to render, how to find a run's
 * thumbnail, and the wording for the empty states.
 */
...
import { formatCost, formatWhen, isLiveStatus, normalizeStatus } from "./runFormat";
...
/* ── one row in the list ────────────────────────────────────────────────── */
function RunRow({ run, selected, onSelect, onCancel, cancelling, requiredSteps, getThumbSrc, Icon }) {
  ...
  const thumb = getThumbSrc?.(run) || null;
...
function DetailSkeleton({ row, requiredSteps }) {
  return (
...
export default function RunLibrary({
  ...
  const requiredSteps = useMemo(
    () => pipeline.filter((s) => !s.optional).length || 1,
    [pipeline]
  );
        ...
        {visible.length ? (
          ...
          <Panel className="px-5 py-12 text-center">
            ...
            <p className="font-heading mt-3 text-sm font-semibold text-[var(--cw-ink)]">
              {copy.emptyTitle || "No runs yet"}
            </p>
            <p className="mx-auto mt-1.5 max-w-xs text-[13px] leading-relaxed text-[var(--cw-ink-muted)]">
              {copy.emptyHint ||
                "Generate your first draft and it lands here — every agent, its cost and the finished result, kept for as long as you need it."}
            </p>
            {onGoCompose ? (
      ...
      <div className={cn("min-w-0", !showDetail && "hidden xl:block")}>
        ...
        {detailError ? (
          ...
          <div key={selectedRun.id} className="animate-soft-rise">
            {renderConsole?.(selectedRun)}
          </div>
        ) : detailLoading || showDetail ? (
          <DetailSkeleton row={selectedRow} requiredSteps={requiredSteps} />
        ) : (
          <Panel className="px-6 py-16 text-center">
            ...
            <p className="mx-auto mt-1.5 max-w-md text-[13px] leading-relaxed text-[var(--cw-ink-muted)]">
              {copy.detailHint ||
                "Every agent in the chain, its full output, what it cost and the finished result — all in this pane, without leaving the list."}
            </p>
            {liveRunId ? (
              <Btn
                variant="outline"
