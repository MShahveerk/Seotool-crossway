...
export function describeBoardMove(kind, fromColumn, toColumn, item = {}) {
  ...
  if (toColumn === "published") {
    ...
    if (kind === "post") {
      implications.push("You can drag it back out of Published later. That deletes the live Meta post.");
    } else {
      implications.push("You cannot pull it back from Published on this board.");
    }
  }

  if (kind === "post" && fromColumn === "published" && toColumn !== "published") {
    severity = "danger";
    implications.length = 0;
    implications.push("The live Facebook and Instagram posts are deleted from Meta.");
    implications.push(`The card returns to ${to}. You can edit it and publish again later.`);
    implications.push("If Meta has no stored post id, the board still moves and a warning is shown.");
    if (toColumn === "pending") {
      implications.push("Approval emails are not sent for a take-down.");
    }
  }
  ...
  const willUnpublishNow = kind === "post" && fromColumn === "published" && toColumn !== "published";
  return {
    requiresConfirm,
    title: willPublishNow
      ? "Publish now?"
      : willUnpublishNow
        ? "Take this post down?"
        : `Move to ${to}?`,
    summary: willPublishNow
      ...
      : willUnpublishNow
        ? `“${titleText}” will be deleted from Meta and moved to ${to}.`
