...
import { cdxThrottle } from "./cdxRateLimit.js";
...
const DEFAULT_INDEX_COUNT = 4;
const DEFAULT_TIMEOUT_MS = 10000;
const CDX_RETRIES = 2;
const REFERRER_TLD_SEEDS = ["com", "org", "net"];
...
async function fetchWithRetry(url, timeoutMs = DEFAULT_TIMEOUT_MS) {
  ...
  for (let attempt = 0; attempt <= CDX_RETRIES; attempt++) {
    try {
      ...
      lastErr = err;
      if (attempt < CDX_RETRIES) await sleep(400 * (attempt + 1));
    }
  }
  throw lastErr;
}
...
export async function getCrawlCollections({ refresh = false } = {}) {
  ...
  const res = await fetchWithRetry(COLLINFO_URL, 12000);
...
/** Parse NDJSON CDX response lines. Never throws on network — returns blocked flag instead. */
export async function queryCdx(crawlId, params) {
  ...
  try {
    ...
    return {
      ...
      pages: null,
      blocked: true,
      error: err?.message || "Common Crawl request failed",
    };
  }
  ...
  if (!res.ok) {
    ...
    return { rows: [], pages: null, blocked: true, error: text.slice(0, 200) || `CDX query failed (${res.status})` };
  }
  ...
  if (trimmed.startsWith("{") && trimmed.endsWith("}")) {
    try {
      ...
      if (meta.message && /no captures found/i.test(meta.message)) {
        return { rows: [], pages: null, blocked: false };
      }
    } catch {
      /* single row */
    }
...
/** Pick the newest crawl index that actually has captures for this domain. */
export async function pickWorkingCrawlId(domain, crawlIds) {
  const ids = crawlIds.filter(Boolean);
  for (const crawlId of ids.slice(0, 2)) {
    const probe = await queryCdx(crawlId, {
      url: `${domain}/*`,
      matchType: "prefix",
      output: "json",
      limit: "1",
    ...
    if (probe.rows.length > 0 || (typeof probe.pages === "number" && probe.pages > 0)) {
      return crawlId;
    }
  }
  return ids[0] || null;
...
export async function fetchSubdomains(domain, crawlId, sampleLimit = 500) {
  const params = {
    url: domain,
    matchType: "domain",
    output: "json",
    limit: String(Math.min(sampleLimit, 2000)),
    collapse: "urlhost",
...
/**
 * Approximate referring hosts — expensive; run on cron only, not manual refresh.
 */
export async function fetchReferringHosts(domain, crawlId, { limitPerSeed = 100 } = {}) {
  const hosts = new Map();
  let blocked = false;
  const filter = `url:${domain}`;

  const tasks = REFERRER_TLD_SEEDS.map((tld) => ({ tld, filter }));
  const results = await mapPool(tasks, 2, async ({ tld, filter: urlFilter }) => {
    const params = {
      url: `*.${tld}`,
      matchType: "domain",
      output: "json",
      limit: String(Math.min(limitPerSeed, 200)),
      collapse: "urlhost",
      fl: "url,urlhost,timestamp,status",
      filter: urlFilter,
    };
...
export async function buildSiteExplorerReport(
  siteUrl,
  { crawlCount = DEFAULT_INDEX_COUNT, includeReferring = true } = {}
) {
  ...
  const primaryCrawl = await pickWorkingCrawlId(domain, crawlIds);
  if (!primaryCrawl) throw new Error("Common Crawl index list is empty.");
  ...
  const [pagesResult, subdomainsResult, referringResult] = await Promise.all([
    ...
    includeReferring
      ...
      : Promise.resolve({ crawlId: primaryCrawl, referring: [], blocked: false, method: "cdx-url-mention" }),
  ]);
  ...
  if (includeReferring) {
    ...
    notes.push(
      "Link samples refresh on the weekly Site Explorer cron (Mondays 05:00). Manual Analyze loads Open PageRank authority only unless this is the first crawl for the domain.",
    );
  }
