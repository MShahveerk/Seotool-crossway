...
const REFERRER_TLD_SEEDS = ["com", "org", "net", "io", "co.uk", "edu", "gov"];
...
export async function fetchSubdomains(domain, crawlId, sampleLimit = 3000) {
  const params = {
    url: domain,
    matchType: "domain",
    output: "json",
 ...
 */
export async function fetchReferringHosts(domain, crawlId, { limitPerSeed = 400 } = {}) {
  const hosts = new Map();
  let blocked = false;
  const filters = [`url:${domain}`, `url:www.${domain}`, `url:https://${domain}`, `url:http://${domain}`];

  for (const tld of REFERRER_TLD_SEEDS) {
    for (const filter of filters) {
      const params = {
        url: `*.${tld}`,
        matchType: "domain",
        output: "json",
        limit: String(Math.min(limitPerSeed, 800)),
        collapse: "urlhost",
        fl: "url,urlhost,timestamp,status",
        filter,
      };
      const result = await queryCdx(crawlId, params);
