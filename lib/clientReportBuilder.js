...
export async function buildSectionReportPdf(section, context, opts = {}) {
  ...
  const clientName = context.displayName || context.siteKey;
  const { baselines } = await fetchSmmBaselines(context.smmSiteKey);
  const smmReportData = await fetchSmmReportData(prisma, context.smmSiteKey, reportMonth);
  ...
  if (section === "smm") {
    return buildUnifiedMarketingReportPdfBytes({
      ...
      smmPeriodLabel: smmReportData.periodLabel || humanMonthYear(reportMonth),
