...
function logoUrl() {
  ...
  return `${baseUrl}/brand/roboseo-mark.png`;
}
...
<head>
  ...
  <title>${escapeHtml(title)} · RoboSEO.Ai</title>
...
<body>
  <main class="card">
    <img class="logo" src="${escapeHtml(logoUrl())}" alt="RoboSEO.Ai logo" />
    <div class="brand">RoboSEO.Ai</div>
 ...
 *   postUrl: string,
 *   noun?: string,
 *   showStudioRevision?: boolean,
 *   showRevisionTarget?: boolean,
 * }} opts
 * postUrl is the full POST action URL (e.g. /api/blogs/quick-action)
 */
export function declineFormPage({
  id,
  token,
  itemTitle = "",
  postUrl,
  ...
  showRevisionTarget = true,
}) {
  const baseUrl = (process.env.NEXTAUTH_URL || "http://localhost:3000").replace(/\/+$/, "");
  const actionUrl = postUrl.startsWith("http") ? postUrl : `${baseUrl}${postUrl.startsWith("/") ? "" : "/"}${postUrl}`;
  const reasonHint = showStudioRevision
    ? "Required. Your reason is recorded either way. Use the checkbox below if Automation Studio should apply the corrections."
    : `Required. This helps the team revise the ${escapeHtml(noun)}.`;
  const studioBlock = showStudioRevision
    ? `
      <input type="hidden" name="studioRevisionOffered" value="1" />
      <label class="studio">
        <input type="checkbox" name="runStudioRevision" value="1" checked />
        <span>
          <strong>Ask Automation Studio to make the corrections needed</strong>
          <small>Checked by default. Uncheck to record the decline without an automatic rewrite.</small>
        </span>
      </label>`
    : "";
  const targetBlock = showRevisionTarget
    ? `
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Decline ${escapeHtml(noun)} · RoboSEO.Ai</title>
  <link href="https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800&display=swap" rel="stylesheet">
  <style>
    * { box-sizing: border-box; }
    body {
    ...
    .studio {
      display: flex; align-items: flex-start; gap: 12px; margin-top: 18px; padding: 12px 14px;
      border: 1px solid #d1d5db; border-radius: 12px; background: #f9fafb; cursor: pointer;
      text-transform: none; letter-spacing: 0; font-weight: 500; color: #111827; font-size: 0.9rem;
    }
    .studio input { margin-top: 3px; width: 16px; height: 16px; flex-shrink: 0; accent-color: #111827; }
    .studio strong { display: block; font-size: 0.9rem; font-weight: 800; }
    .studio small { display: block; margin-top: 3px; font-size: 0.75rem; font-weight: 500; color: #6b7280; line-height: 1.4; }
