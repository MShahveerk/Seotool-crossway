 ...
 */
import { getWordpressConfig, formatWordpressError, detectWordpressBlockResponse, getWordpressHttpTimeout, wordpressRawGet } from "./wordpressClient.js";
...
async function tracedGet(trace, label, url, { auth, params = {}, timeout = getWordpressHttpTimeout() } = {}) {
  ...
  try {
    const res = await wordpressRawGet(url, {
      auth,
      timeout,
      params,
      logLabel: label,
    ...
    if (block.blocked) entry.blocked = block;
    trace.push(entry);
    return res;
  } catch (error) {
...
export async function runWordpressDiagnostics(config, meta = {}) {
  ...
  try {
    const rootRes = await tracedGet(trace, "Discover site (wp-json)", `${base}/wp-json/`, { timeout: 8000 });
    const rootBlock = detectWordpressBlockResponse(rootRes);
    if (rootBlock.blocked) {
      access.diagnosis = rootBlock.message;
      summary.push(rootBlock.message);
      logWordpress("diagnostics_blocked", { siteLink: meta.selectedSite, block: rootBlock });
