...
/** Pick the first finite number > 0 (0 is treated as missing, not a real count). */
...
function roundScore(value) {
  ...
  // Some payloads store 0–1 fractions; dashboard expects 0–100.
  const score = n > 0 && n <= 1 ? Math.round(n * 100) : Math.round(n);
  if (score < 0 || score > 100) return null;
  return score;
}

function extractNormalizedAudit(payload) {
...
function auditFromInternalSnapshot(snap, { running = false, lastError = null } = {}) {
  ...
  const pages =
    snap.totalPages ??
    snap.payload?.stats?.pagesCrawled ??
    snap.payload?.stats?.indexablePages ??
    null;
  const score = roundScore(snap.healthScore ?? snap.payload?.healthScore);
  const critical = snap.criticalCount ?? snap.payload?.counts?.critical ?? null;
  const warning = snap.warningCount ?? snap.payload?.counts?.warning ?? null;
  // Empty / blocked crawls often persist score 0 with no pages — treat as missing.
  if (score == null && critical == null && warning == null) return null;
  if (score === 0 && !(pages > 0) && !(critical > 0) && !(warning > 0)) {
    return null;
  }
  return {
    source: "internal",
...
function auditFromSerankingPayload(payload, meta = {}) {
  const normalized = extractNormalizedAudit(payload);
  if (!normalized) return null;
  const score = roundScore(normalized.score);
...
/** Prefer live audit with a real score; never let a hollow internal 0 hide it. */
function pickBestAudit(liveAudit, internalAudit, { running = false, lastError = null } = {}) {
  const strength = (a) => {
    if (!a) return -1;
    let s = 0;
    if (a.source === "seranking") s += 4;
    if (a.score != null && a.score > 0) s += 10;
    if (a.score === 0 && ((a.critical || 0) > 0 || (a.warning || 0) > 0 || (a.pages || 0) > 0)) s += 6;
    if ((a.critical || 0) > 0) s += 2;
    if ((a.warning || 0) > 0) s += 1;
    if ((a.pages || 0) > 0) s += 1;
    return s;
  };
  const primary = strength(liveAudit) >= strength(internalAudit) ? liveAudit : internalAudit;
  const chosen = primary || liveAudit || internalAudit;
  if (!chosen) return { ...emptyAudit(), running, error: lastError };
  return {
    score: chosen.score,
    critical: chosen.critical,
    warning: chosen.warning,
    running: Boolean(running || chosen.running),
    finishedAt: chosen.finishedAt || null,
    error: lastError,
  };
}

async function loadLiveAuditCached(websiteUrl) {
  ...
  for (const key of variants) {
    ...
    const mapped = auditFromSerankingPayload(cached?.payload, cached || {});
  ...
  if (domain) {
    const row = await prisma.serankingSnapshot
      ...
      .catch(() => null);
    const mapped = auditFromSerankingPayload(row?.payload, row || {});
    if (mapped) return mapped;
  }
  ...
  const job = await prisma.serankingAuditJob
    .findFirst({
      where: {
        status: "success",
        OR: [{ siteUrl: { in: variants } }, domain ? { domain } : undefined].filter(Boolean),
      },
      orderBy: [{ finishedAt: "desc" }, { startedAt: "desc" }],
    })
    .catch(() => null);
  return auditFromSerankingPayload(job?.payload, {
    fetchedAt: job?.finishedAt || job?.startedAt,
  });
}

async function loadInternalAuditCached(websiteUrl) {
  const variants = siteUrlVariants(websiteUrl);
  let running = false;
  let lastError = null;
  let best = null;
  ...
  for (const key of variants) {
    ...
    if (mapped && (!best || (mapped.score || 0) > (best.score || 0))) best = mapped;
  }
  ...
  return { audit: best, running, lastError };
}

async function resolveDashboardAudit(websiteUrl) {
  const [liveAudit, internalPack] = await Promise.all([
    loadLiveAuditCached(websiteUrl).catch(() => null),
    loadInternalAuditCached(websiteUrl).catch(() => ({ audit: null, running: false, lastError: null })),
  ]);
  return pickBestAudit(liveAudit, internalPack.audit, {
    running: internalPack.running,
    lastError: internalPack.lastError,
  });
}

async function resolveIndexedUrls(websiteUrl, explorer) {
  // Common Crawl / explorer rows default to 0 — never treat 0 as indexed.
  const fromExplorer = firstPositive(
    explorer?.indexedUrls,
    explorer?.payload?.overview?.indexedUrls,
    explorer?.payload?.overview?.sitemapUrlCount,
    explorer?.payload?.overview?.inspectionIndexed,
    explorer?.payload?.overview?.pagesInSearch,
    explorer?.payload?.gsc?.sitemapUrlCount,
    explorer?.payload?.gsc?.inspectionIndexed,
    explorer?.payload?.gsc?.pagesInSearch,
    explorer?.payload?.totalPages
  );
  ...
  // Cached URL Inspection snapshots (fast, no live GSC call)
  for (const key of variants) {
    const snap = await prisma.urlInspectionSnapshot
      .findFirst({
        where: { siteUrl: key, status: "completed", indexedCount: { gt: 0 } },
        orderBy: { runDate: "desc" },
        select: { indexedCount: true, totalUrls: true },
      })
      .catch(() => null);
    const n = firstPositive(snap?.indexedCount, snap?.totalUrls);
    if (n != null) return n;
  }
  if (domain) {
    const snap = await prisma.urlInspectionSnapshot
      .findFirst({
        where: { siteUrl: { contains: domain }, status: "completed", indexedCount: { gt: 0 } },
        orderBy: { runDate: "desc" },
        select: { indexedCount: true, totalUrls: true },
      })
      .catch(() => null);
  ...
  // Live / cached GSC signals
  try {
    const { fetchGscIndexingData, buildOverviewFromGsc } = await import("./siteExplorerGsc.js");
    const gscData = await fetchGscIndexingData(websiteUrl);
    const overview = buildOverviewFromGsc({
      sitemapsResult: gscData.sitemapsResult,
      monitor: gscData.monitor,
      topPagesResult: gscData.topPagesResult,
      pages: [],
    });
    const fromGsc = firstPositive(
      overview.inspectionIndexed,
      overview.sitemapUrlCount,
      overview.pagesInSearch,
      overview.indexedUrls
    );
    if (fromGsc != null) return fromGsc;
  } catch {
    /* GSC optional */
  }

  // Direct GSC top-pages count (often available even when sitemaps report 0)
  try {
    const { getTopPages } = await import("./searchconsole.js");
    const { getDateRangeForPresetId } = await import("./searchConsoleDateRanges.js");
    const { startDate, endDate } = getDateRangeForPresetId("28d");
    const top = await getTopPages(websiteUrl, startDate, endDate, 1000).catch(() => null);
    const n = firstPositive((top?.pages || []).length);
    if (n != null) return n;
  } catch {
    /* optional */
  }

  // Live audit crawled pages / domain-pages cache as last resort
  for (const key of variants) {
    const cached = await getCachedSnapshot(key, DATA_TYPES.AUDIT_REPORT).catch(() => null);
    const normalized = extractNormalizedAudit(cached?.payload);
    const pages = firstPositive(normalized?.totalPages);
    if (pages != null) return pages;

    const domainPages = await getCachedSnapshot(key, DATA_TYPES.DOMAIN_PAGES).catch(() => null);
    const list =
      domainPages?.payload?.pages ||
      domainPages?.payload?.data ||
      (Array.isArray(domainPages?.payload) ? domainPages.payload : null);
    const count = firstPositive(Array.isArray(list) ? list.length : null);
...
async function fetchSmmBaseline(siteEquivalents, targetSiteNormalized) {
  ...
  // Read across all users for this site — live Meta pulls may write under owner or admin.
  ...
  // Forward-fill: Meta frequently writes the newest day with followers=0 (reach
  // only), so the latest row alone reads as 0. `rows` is ordered statDate desc,
  // so the first followers>0 we see per platform is the most recent real count —
  // the same baseline the Social section surfaces.
  const lastKnownFollowers = new Map();
  for (const row of rows) {
    const key = normalizePlatformKey(row.platform);
    if (!key) continue;
    if (Number(row.followers || 0) > 0 && !lastKnownFollowers.has(key)) {
      lastKnownFollowers.set(key, Number(row.followers));
    }
  ...
  const baselines = Array.from(latestByPlatform.values()).map((row) => {
    const latestFollowers = Number(row.followers || 0);
    const followers = latestFollowers > 0 ? latestFollowers : lastKnownFollowers.get(row.platform) || 0;
    return {
...
async function fetchHealthBlock(websiteUrl) {
  ...
  const indexedUrls = await resolveIndexedUrls(websiteUrl, explorer).catch(() => null);
  ...
  // Prefer SE Ranking's domain inlink rank (0-100) — the same authority figure
  // the Site Intelligence page surfaces — falling back to Open PageRank (scaled).
  const seInlinkRank = seBacklinks?.domainInlinkRank ?? null;
  const oprScore100 = authority?.score != null ? toScore100(authority.score) : null;

  return {
    audit,
    authority: {
      score100: seInlinkRank != null ? seInlinkRank : oprScore100,
    ...
    // Never surface a fake 0 — UI and consumers should see null when unknown.
    indexedUrls: firstPositive(indexedUrls),
    referringDomains: firstPositive(referringDomains),
  };
}
