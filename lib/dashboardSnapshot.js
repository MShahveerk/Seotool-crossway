...
async function fetchHealthBlock(websiteUrl) {
  ...
  const [audit, pageSpeedMobile, pageSpeedDesktop, explorerResult, authorityMap] = await Promise.all([
    getLatestSiteAudit(websiteUrl).catch(() => ({ snapshot: null, running: false, lastError: null })),
  ...
  return {
    audit: {
      score: audit.snapshot?.healthScore ?? null,
      critical: audit.snapshot?.criticalCount ?? null,
      warning: audit.snapshot?.warningCount ?? null,
      running: audit.running,
      finishedAt: audit.snapshot?.finishedAt?.toISOString?.() || null,
      error: audit.lastError,
    },
    authority: {
    ...
    indexedUrls: explorer?.indexedUrls ?? null,
    referringDomains: explorer?.referringDomainsCount ?? authority?.referringDomains ?? null,
  };
}
