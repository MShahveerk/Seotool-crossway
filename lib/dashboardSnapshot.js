...
import { normalizeBacklinksSummary } from "./seranking/normalize.js";
import { healthFromSerankingPayload } from "./serankingHealth.js";
...
async function loadLiveAuditCached(websiteUrl) {
  ...
  for (const key of variants) {
    ...
    const mapped = healthFromSerankingPayload(cached?.payload, cached || {});
  ...
  if (domain) {
    const row = await prisma.serankingSnapshot
      ...
      .catch(() => null);
    const mapped = healthFromSerankingPayload(row?.payload, row || {});
    if (mapped) return mapped;
  }
  ...
  const job = await prisma.serankingAuditJob
    ...
    .catch(() => null);
  return healthFromSerankingPayload(job?.payload, {
    fetchedAt: job?.finishedAt || job?.startedAt,
  });
}

/** True while SE Ranking is still crawling, so the card can say so. */
async function serankingAuditRunning(websiteUrl) {
  const variants = siteUrlVariants(websiteUrl);
  const domain = toDomain(websiteUrl);
  const job = await prisma.serankingAuditJob
    .findFirst({
...
/**
 * SE Ranking is the single source for audit health.
 *
 * Our own crawler used to be blended in here as a fallback, which meant the same
 * site could show two different health scores depending on which one happened to
 * be fresher. One number, one provenance — even when that means showing nothing.
 */
async function resolveDashboardAudit(websiteUrl) {
  const [audit, running] = await Promise.all([
    loadLiveAuditCached(websiteUrl).catch(() => null),
    serankingAuditRunning(websiteUrl).catch(() => false),
  ]);
  if (!audit) return { ...emptyAudit(), running };
  return {
    score: audit.score,
