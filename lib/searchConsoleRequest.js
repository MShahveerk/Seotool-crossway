...
export async function resolveSearchConsoleRequest(req) {
  ...
  const isDomainProp = typeof siteUrl === "string" && siteUrl.startsWith("sc-domain:");

  if (siteUrl && !isValidUrl(siteUrl) && !isDomainProp) {
  ...
  const finalIsDomainProp = typeof siteUrl === "string" && siteUrl.startsWith("sc-domain:");

  if (!siteUrl || (!isValidUrl(siteUrl) && !finalIsDomainProp)) {
    const err = new Error(
      "A website URL or sc-domain property is required for this tool. Select a website from the client dropdown."
    );
    err.status = 400;
    throw err;
  }

  const normalizedUrl = finalIsDomainProp ? siteUrl.trim().toLowerCase() : normalizeSiteOrigin(siteUrl);
  if (!normalizedUrl) {
  ...
  if (userRole === ROLES.USER) {
    const own = session.user.siteLink || "";
    const ownIsDomain = own.startsWith("sc-domain:");
    const ownNormalized = ownIsDomain ? own.trim().toLowerCase() : normalizeSiteOrigin(own);
    if (!ownNormalized || ownNormalized !== normalizedUrl) {
