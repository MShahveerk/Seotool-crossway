...
const BLOG_MANUAL = {
  pending: new Set(["edited", "approved", "declined"]),
  edited: new Set(["pending", "approved", "declined"]),
  approved: new Set(["pending", "declined", "edited"]),
  failed: new Set(["approved", "pending"]),
  declined: new Set(["pending", "edited"]),
  published: new Set(),
};
...
export function blogColumnToUpdate(columnId) {
  switch (columnId) {
    case "pending":
      return { status: "pending", publishStatus: "unpublish" };
    case "edited":
      return { status: "edited", publishStatus: "unpublish" };
    case "approved":
      return { status: "approved", publishStatus: "unpublish" };
    case "failed":
      return { status: "approved", publishStatus: "failed" };
    case "declined":
      return { status: "declined", publishStatus: "unpublish" };
    default:
      return null;
  }
}
