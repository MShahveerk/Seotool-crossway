...
/** Latest row per platform on or before a date. */
function latestFollowersByPlatform(rows, onOrBeforeYmd) {
  ...
  for (const row of rows) {
    ...
    if (!prev || t >= toDateOnly(prev.statDate).getTime()) {
      map.set(pk, row);
    }
  }
  return map;
...
export async function fetchSmmReportData(prisma, siteKey, reportMonth) {
  ...
  const platforms = PDF_PLATFORMS.map(({ key, label }) => {
    ...
    return {
      platform: label,
      platformKey: key,
      accountName: current ? extractAccountLabel(current) : weekRow ? extractAccountLabel(weekRow) : "Not linked",
      followers,
