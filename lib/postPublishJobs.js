...
import { publishPostViaMeta } from "./postPublishers/meta.js";
import { publishPostViaWebhook } from "./postPublishers/webhook.js";
import { publishPostViaApi } from "./postPublishers/api.js";
import { publishPostViaEmail } from "./postPublishers/email.js";
  ...
  for (const method of chain) {
    ...
    try {
      ...
      return {
        ...
        errors: Array.isArray(result.errors) ? result.errors : errors,
        responseBody: result.responseBody,
        partial: Boolean(result.partial),
        instagramUserId: result.instagramUserId || null,
        facebookPageId: result.facebookPageId || null,
      };
    } catch (err) {
  ...
  for (const post of posts) {
    try {
      ...
      const updatedPost = await prisma.approval.update({
        where: { id: post.id },
        data: publishResultToUpdate(result, finalStatus),
        include: {
...
function publishResultToUpdate(result, finalStatus) {
  const errors = Array.isArray(result?.errors) ? result.errors.filter(Boolean) : [];
  const data = {
    publishStatus: finalStatus,
    publishError: finalStatus === "published" ? (result.partial && errors.length ? errors.join(" | ") : null) : errors.join(" | ") || null,
  };
  if (result?.externalId) data.externalId = String(result.externalId).slice(0, 191);
  if (result?.instagramUserId) data.instagramUserId = String(result.instagramUserId).slice(0, 191);
  return data;
}

export async function unpublishApprovalNow(approvalId) {
  const approval = await loadApprovalWithCaption(approvalId);
  if (!approval) {
    const err = new Error("Approval not found.");
    err.status = 404;
  ...
  if (approval.publishStatus !== "published") {
    return { attempted: false, deleted: false, errors: [], reason: "not_published" };
  }
  const { unpublishPostFromMeta } = await import("./postPublishers/meta.js");
  return unpublishPostFromMeta(approval);
}
