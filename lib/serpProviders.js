...
function publicAppOrigin() {
  const raw =
    process.env.GOOGLE_CSE_REFERER ||
...
function explainGoogleCseError(data, status) {
  const message = String(data?.error?.message || "");
  const reason = String(data?.error?.errors?.[0]?.reason || data?.error?.status || "");
  if (/blocked/i.test(message) || reason === "ipRefererBlocked") {
    return "Google blocked this API key for server use. Cloud Console → APIs & Services → Credentials → that key → Application restrictions = None (not HTTP referrers / websites). API restrictions = Custom Search API only. Save, wait one minute, Refresh.";
  }
  if (/has not been used|is disabled|accessNotConfigured/i.test(message) || reason === "accessNotConfigured") {
    return "Custom Search API is not enabled on this Google Cloud project. Cloud Console → APIs & Services → Library → Custom Search API → Enable.";
  }
  if (/dailyLimitExceeded|quota/i.test(message) || reason === "dailyLimitExceeded") {
    return "Google Programmable Search hit the free 100 queries/day cap. Try again tomorrow, or top up SerpAPI.";
  }
  return message || `Google Programmable Search error (HTTP ${status})`;
}
...
export async function fetchGoogleCseSerp(keyword, opts, { apiKey, cx }) {
  ...
  const { res, data } = await fetchJson(`https://www.googleapis.com/customsearch/v1?${params}`, { headers });
  if (!res.ok || data?.error) {
    const err = new Error(explainGoogleCseError(data, res.status));
...
export async function fetchDuckDuckGoSerp(keyword, opts = {}) {
  ...
  let lastError = "DuckDuckGo returned no web results.";
  for (const url of urls) {
    ...
    try {
      ...
      if (!res.ok) {
        lastError = `DuckDuckGo returned HTTP ${res.status}`;
        continue;
      }
      const html = await res.text();
      if (/anomaly-modal|captcha|blocked/i.test(html) && !/result__a/i.test(html)) {
        lastError = "DuckDuckGo blocked this server.";
        continue;
      }
      const rows = parseDuckDuckGoHtml(html);
      if (!rows.length) {
        lastError = "DuckDuckGo returned no web results.";
        continue;
      }
      logger.info("Organic search used DuckDuckGo fallback", { keyword, url });
      ...
      lastError = `DuckDuckGo ${networkErrorDetail(err)}`;
    } finally {
      clearTimeout(timeoutId);
    }
  }
  throw new Error(`${lastError} The host likely cannot reach DuckDuckGo. Fix Google CSE key restrictions instead.`);
}
