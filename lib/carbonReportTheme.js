 ...
 *   canvas #151A1F · surface #1F252C · raised #2A313A · hairline #3D454F
 *   ink #F4F6F7 · dim #CCD2D9 · muted #9BA3AD · faint #79818B · neon #0EFF2A
 */
...
async function loadLogo(pdf) {
  ...
  for (const name of ["crossway-logo-white.png", "crossway-logo.png"]) {
    try {
...
export async function createCarbonReport({ eyebrow, reportTitle, subject, meta, introNote }) {
  ...
  const drawCoverHeader = (p) => {
    ...
    p.drawText(safeText((eyebrow || "Crossway SEO").toUpperCase(), 48), {
      x: textX,
      y: PAGE_H - 34,
      size: 7.5,
      font: display,
  ...
  const drawFooter = (p, n) => {
    ...
    p.drawText(safeText("Crossway SEO Tool", 30), {
      x: MARGIN,
      y: FOOTER_H - 6,
      size: 7,
      font: body,
