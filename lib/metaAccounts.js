...
export async function fetchAccountsForToken(token) {
  ...
  if (accounts.length === 0) {
    const { data: meData, error: meError } = await graphGetWithFieldFallback("/me", token, {
      withLimit: false,
    });
...
async function enrichPageIdsFromGraph(pageIds, tokens, byId) {
  ...
  for (const id of ids) {
    ...
    const alreadyLive =
      ...
      existing?.name &&
      !String(existing.name).startsWith("Facebook Page");
...
function rememberDbPage(byId, { id, name, instagramUserId, siteLink }) {
  ...
  byId.set(pageId, {
    id: pageId,
    name: name || `Facebook Page ${pageId}`,
...
export async function accountsFromDatabase() {
  const [sites, users, configs, accessible, statKeys] = await Promise.all([
    ...
    prisma.sitePostConfig.findMany({
  ...
  for (const c of configs) {
    rememberDbPage(byId, {
      ...
      name: isMetaPageId(c.siteKey) ? `Facebook Page ${c.siteKey}` : c.siteKey,
      instagramUserId: c.instagramUserId,
      siteLink: isMetaPageId(c.siteKey) ? "" : c.siteKey,
    });
  }
  for (const row of accessible) {
    rememberDbPage(byId, { id: row.siteLink, name: `Facebook Page ${row.siteLink}` });
  }
...
function mergeAccount(existing, incoming) {
  ...
  return {
    ...existing,
    name:
      incoming.source === "graph" && incoming.name && !String(incoming.name).startsWith("Facebook Page")
        ? incoming.name
        : existing.name && !String(existing.name).startsWith("Facebook Page")
          ? existing.name
