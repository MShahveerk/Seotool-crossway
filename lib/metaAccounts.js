...
export { formatMetaTokenReconnectHint, isInvalidatedMetaTokenError, metaLiveFetchFailed };
...
export async function loadMetaAccounts({ includeDatabase = true } = {}) {
  ...
  const graphLookupFailed = tokens.length > 0 && graphCount === 0;
  const tokenInvalid = graphErrors.some((e) => isInvalidatedMetaTokenError(e));
  let error = null;
  let warning = null;
  if (!tokens.length && accounts.length === 0) {
    ...
    warning = `Showing ${databaseCount} cached page(s). Set META_PAGE_ACCESS_TOKEN on the server to fetch live pages.`;
  } else if (accounts.length === 0) {
    error = formatMetaTokenReconnectHint(
      graphErrors.filter(Boolean)[0] || "Meta Graph returned no pages for this token."
    );
  } else if (graphLookupFailed) {
    warning = formatMetaTokenReconnectHint(
      `Showing ${databaseCount} cached page(s) from the last successful fetch. Graph live lookup failed${
...
export async function persistFetchedMetaPages(accounts) {
  const list = (Array.isArray(accounts) ? accounts : []).filter((acc) => acc?.source === "graph");
  let persisted = 0;
...
export async function fetchAndPersistMetaPages() {
  ...
  if (metaLiveFetchFailed(loaded)) {
    const error = formatMetaTokenReconnectHint(
      loaded.error || loaded.warning || loaded.graphErrors?.[0]
    );
    return {
      ...loaded,
      persisted: 0,
      graphLookupFailed: true,
      tokenInvalid: loaded.tokenInvalid || isInvalidatedMetaTokenError(error),
      error,
    };
  }
  const live = (loaded.accounts || []).filter((acc) => acc.source === "graph");
  const persisted = await persistFetchedMetaPages(live);
