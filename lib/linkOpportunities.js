...
import { surveyPitchCosts } from "./prospectProbe.js";
...
function isUnpitchable(domain) {
  ...
  return /\.(edu|gov|mil)(\.[a-z]{2})?$/.test(domain);
}
...
function isNotABusinessRival(domain) {
  return isUnpitchable(domain) || isNotNicheRival(domain) || isScraper(domain);
}
...
function looksLikeListing(domain, _title, tag) {
  if (tag === "directory") return true;
  // Domain shape only. Titles like "Best Web Design in Dallas" are how
  // agencies advertise — they are competitors, not directories to submit to.
...
const ACTIONABLE_TYPES = new Set(["serp-listing", "guest-post", "directory", "resource", "roundup"]);
...
function isViablePitch(row, kind) {
  ...
  if (kind.type === "guest-post") return hasPage;
...
function cacheKey(keyword, geo, device, location, depth) {
  ...
  return `lo-v10:${geo}:${device}:${depth.rankers}x${depth.refdomains}:${h}`;
}
...
export async function buildLinkOpportunities(siteUrl, keyword, opts = {}) {
  ...
  for (const row of ladder) {
    ...
    if (analysedHosts.has(host)) continue;
    // Platforms and publishers can rank, and their titles even look like
    // directories. There is still nobody to pitch.
    if (isUnpitchable(host)) continue;
  ...
  const intersect = [...byLinker.values()]
    .map((row) => {
       ...
       */
      const provenByLinks = hits >= 2;
      if (!onNiche && !provenByLinks && (kind.type === "directory" || kind.type === "serp-listing")) {
  ...
  // Paid vs unpaid: homepage copy + known networks. Guest posts / roundups /
  // resources are earned (unpaid) unless the URL itself is a sponsored slot.
  // Parked domains and backlink mills get thrown out of Can pitch here.
  const pitchable = intersect.filter((r) => ACTIONABLE_TYPES.has(r.type) && !r.youHaveIt);
  try {
    const costs = await surveyPitchCosts(
      pitchable.map((r) => ({ domain: r.domain, type: r.type, sourceUrls: r.sourceUrls })),
      { concurrency: 6, limit: 50 }
    );
    const dead = PROSPECT_TYPES.find((x) => x.type === "link-farm");
    for (const row of intersect) {
      const surveyed = costs.get(row.domain);
      if (surveyed?.alive === false && ACTIONABLE_TYPES.has(row.type) && dead) {
        row.type = dead.type;
        row.typeLabel = dead.label;
        row.typeHint = "Parked, for sale, or selling backlinks — not a pitchable site.";
        row.prospectScore = 0;
        row.cost = "unknown";
        row.costNote = surveyed.costNote || "";
        continue;
      }
      if (surveyed) {
        row.cost = surveyed.cost;
        row.costNote = surveyed.costNote || "";
      } else if (ACTIONABLE_TYPES.has(row.type)) {
        row.cost = row.type === "directory" || row.type === "serp-listing" ? "unknown" : "unpaid";
        row.costNote =
          row.cost === "unpaid"
            ? "Earned placement — pitch the page, don't buy it."
            : "Couldn't confirm whether a listing is free.";
      }
    }
    intersect.sort(
      (a, b) =>
    ...
    logger?.warn?.(`Link opportunities: cost survey failed — ${err.message}`);
  }
