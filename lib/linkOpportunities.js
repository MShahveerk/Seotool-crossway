...
function isGenericDirectoryName(domain) {
...
function isLinkFarm(domain) {
  ...
  if (isGenericDirectoryName(d)) return true;
  return false;
}

/** Many generic directories pointing at the same one rival = a paid blast, not proof. */
function directoryFarmHosts(byLinker) {
...
function cacheKey(keyword, geo, device, location, depth, originCountry = "") {
  ...
  return `lo-v22:${geo}:${device}:${depth.rankers}x${depth.refdomains}:${h}`;
}
...
export async function buildLinkOpportunities(siteUrl, keyword, opts = {}) {
  ...
  const details = targets.length
    ? await Promise.all(
        targets.map((t) =>
          fetchCompetitorBacklinkDetail(t.domain, {
            refLimit: refdomains,
            linkLimit: 100,
            urlFromFilter,
          })
  ...
  for (const { target, detail, error } of details) {
    ...
    // When hunting a country TLD, also admit linking hosts that were not in
    // the worldwide top referring domains — that is how the list actually
    // finds local sites instead of only filtering the global authority head.
  ...
  if (originWanted) {
    for (const [domain, row] of byLinker) {
      if (!originMatchesCountry(domain, row.sourceCountry, originWanted)) {
  ...
  const farmHosts = directoryFarmHosts(byLinker);
  ...
  const intersect = [...byLinker.values()]
    .map((row) => {
      ...
      let kind;
      if (isLinkFarm(row.domain) || farmHosts.has(row.domain)) {
       ...
       *
       * Generic *directory.com names are the exception: one IT firm buying a
       * blast of 40 directories is not proof they serve this niche.
       */
      const genericDir = isGenericDirectoryName(row.domain);
      const provenByLinks =
        !genericDir && (hits >= 2 || (hits >= 1 && successfulAnalyses < 3));
      // A host we already recognise as a claimable/paid listing (yelp, a known
      // paid directory, and curated verticals like weedmaps/leafly whose names
      // don't betray them) is a real, reachable prospect by definition.
      const knownListingHost = isClaimableListing(row.domain) || isPaidDirectory(row.domain);
      // Generalises the weedmaps case to EVERY vertical directory by SHAPE, not
      // name: a listing-shaped domain (directory / finder / maps / guide /
      // locator / listings) that links to even one analysed rival is, by
      // behaviour, a directory for this niche. It still has to link into the
      // niche, so a generic off-topic finder that ignores your rivals stays
      // filtered — this only rescues directories that demonstrably point here.
      ...
      if (
        ...
        (kind.type === "directory" ||
          kind.type === "serp-listing" ||
          kind.type === "publication")
      ) {
  ...
  try {
    ...
    for (const row of intersect) {
      ...
      if (surveyed.alive === false && dead) {
        ...
        row.prospectScore = 0;
      } else if (surveyed.siteKind === "product" && row.type !== "guest-post") {
        const product = PROSPECT_TYPES.find((x) => x.type === "product");
        if (product) {
  ...
  for (const { target, detail } of details) {
    ...
    for (const link of detail.links || []) {
      ...
      strongest.push({
        ...
        sourceLocation: originLocationFromHost(sourceDomain),
      });
    }
  }
...
export async function getLinkOpportunities(siteUrl, keyword, opts = {}, { force = false } = {}) {
  ...
  const originWanted = String(originCountry || "").trim() ? parseOriginCountry(originCountry) : null;
  const sourceKey = cacheKey(keyword, geo, device, location, depth, originWanted?.code || "");
