 ...
 *
 * Cost: the backlinks endpoint bills ~1 credit per referring domain returned,
 * so depth is the credit dial. 10 rankers x 100 refdomains ≈ 1,000 credits on a
 * cold keyword.
 */
...
function cacheKey(keyword, geo, device, location, depth) {
  ...
  return `lo-v1:${geo}:${device}:${depth.rankers}x${depth.refdomains}:${h}`;
}
...
export async function buildLinkOpportunities(siteUrl, keyword, opts = {}) {
  ...
  const details = await Promise.all(
    targets.map((t) =>
      fetchCompetitorBacklinkDetail(t.domain, { refLimit: refdomains, linkLimit: 50 })
  ...
  for (const { target, detail, error } of details) {
    ...
    for (const link of detail.links || []) {
      ...
      if (link?.sourceUrl && row.sourceUrls.length < 12) {
  ...
  return {
    ...
    targets,
    intersect,
    strongest: strongest.slice(0, 300),
    byType,
    summary: {
...
export async function getLinkOpportunities(siteUrl, keyword, opts = {}, { force = false } = {}) {
  const { geo = "us", device = "desktop", location = "", rankers = 10, refdomains = 100 } = opts;
