...
function hasLocalIntent(keyword) {
...
function cacheKey(keyword, geo, device, location, depth, originCountry = "") {
  ...
  return `lo-v23:${geo}:${device}:${depth.rankers}x${depth.refdomains}:${h}`;
}
...
function decorateLinker(row, ctx) {
  ...
  const knownListingHost =
    (isClaimableListing(row.domain) &&
      (onNiche || hits > 0 || hasLocalIntent(keyword) || listing)) ||
    isPaidDirectory(row.domain);
  const genericClaimableDropped =
    isGenericClaimable(row.domain) && !onNiche && hits === 0 && !hasLocalIntent(keyword);
...
function discoveryKind(result, keyTokens, tokens) {
  ...
  if (guest.re.test(path) || /write\s+for\s+us|guest\s+post|contribute/i.test(`${title} ${path}`)) {
    return "guest-post";
  }
...
async function loadDiscoverySerps(keyword, { location = "", geo = "us", device = "desktop" } = {}) {
  ...
  if (!q) return [];
  const queries = [`${q} directory`, `${q} resources`, `${q} "write for us"`, `${q} guest post`];
  const out = [];
  await Promise.all(
    queries.map(async (query) => {
      const key = `ldv1:${crypto
        .createHash("sha256")
      ...
      try {
        ...
        let organic = cached?.payload?.organic;
        if (!organic || cached.expired) {
...
export async function buildLinkOpportunities(siteUrl, keyword, opts = {}, { onProgress } = {}) {
  ...
  for (const row of ladder) {
    ...
    if (isGenericClaimable(host) && !hasLocalIntent(serp?.keyword || keyword) && !matchesVocabulary(host, tokens)) {
      continue;
    }
  ...
  await emit("serp", "Ranking listings from the keyword SERP", { current: 0, total: targets.length || 1 });
  ...
  for (let i = 0; i < targets.length; i += 1) {
    const t = targets[i];
    await emit("rivals", `Reading backlinks for ${t.domain}`, { current: i, total: targets.length });
    try {
    ...
    if (originWanted) {
      for (const [domain, row] of byLinker) {
        if (!originMatchesCountry(domain, row.sourceCountry, originWanted) && !serpListings.has(domain) && row.foundVia !== "discover") {
          byLinker.delete(domain);
        }
      }
    }
    await emit("rivals", `Read ${t.domain}`, { current: i + 1, total: targets.length });
  }
  ...
  await emit("probe", "Checking live submit routes on the best prospects", {
    current: 0,
    total: Math.min(60, intersect.filter((r) => ACTIONABLE_TYPES.has(r.type) && !r.youHaveIt).length || 1),
  });
  ...
  const pitchable = intersect
    ...
    .sort((a, b) => {
      const rank = (r) =>
        (r.foundVia === "discover" ? 30 : 0) +
        (r.type === "guest-post" ? 40 : 0) +
        (r.type === "serp-listing" ? 24 : 0) +
        (r.onNiche ? 20 : 0) +
        (r.prospectScore || 0);
      return rank(b) - rank(a);
    });
  probeTotal = Math.min(60, pitchable.length);
...
export async function getLinkOpportunities(siteUrl, keyword, opts = {}, { force = false, onProgress = null } = {}) {
    ...
    try {
      ...
      if (cached?.payload && !cached.expired) {
        ...
        if (cachedKeyword && wanted && cachedKeyword !== wanted) {
          ...
          return { ...cached.payload, cached: true, fetchedAt: cached.fetchedAt, status: cached.payload.status || "done" };
        }
      }
    } catch {
  ...
  return { ...done, cached: false, fetchedAt: new Date() };
}
