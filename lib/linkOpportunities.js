...
function cacheKey(keyword, geo, device, location, depth) {
  ...
  return `lo-v14:${geo}:${device}:${depth.rankers}x${depth.refdomains}:${h}`;
}
...
export async function buildLinkOpportunities(siteUrl, keyword, opts = {}) {
  ...
  // Live check (and LLM probe if a key is saved): Can pitch only keeps sites
  // with a grounded route in. Known listing networks skip the wait.
  ...
  try {
    ...
    for (const row of intersect) {
      ...
      if (!surveyed.qualify && noRoute) {
        row.type = noRoute.type;
        row.typeLabel = noRoute.label;
        row.typeHint = surveyed.costNote || noRoute.hint;
        row.prospectScore = 4;
      }
    }
    intersect.sort(
  ...
  return {
    ...
    notes: [
      ...
      !targets.length
        ...
        : null,
      llmProbed
        ? "Can pitch was checked against live pages, then judged by the saved LLM using only that evidence."
