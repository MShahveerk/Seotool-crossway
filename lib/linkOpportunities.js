...
const UNPITCHABLE = new Set([
  ...
  "libsyn.com", "podbean.com", "beehiiv.com", "webflow.io", "wordpress.org",
...
function nicheVocabulary(keyword, ladder) {
  ...
  for (const row of ladder || []) {
    ...
    for (const token of stem.split(/[^a-z]+/)) {
      if (token.length < 4 || STOP_WORDS.has(token) || NON_NICHE_TOKENS.has(token)) continue;
      counts.set(token, (counts.get(token) || 0) + 1);
    }
  }
  for (const [token, n] of counts) {
    if (n >= 2) vocab.add(token);
  }
  return [...vocab];
}
...
function cacheKey(keyword, geo, device, location, depth) {
  ...
  return `lo-v7:${geo}:${device}:${depth.rankers}x${depth.refdomains}:${h}`;
}
...
export async function buildLinkOpportunities(siteUrl, keyword, opts = {}) {
  ...
  const seeds = targets
    .filter(
      (t) =>
        !looksLikeListing(t.domain, t.title, null) &&
        !isNotNicheRival(t.domain) &&
        !isScraper(t.domain) &&
        !isUnpitchable(t.domain)
    )
    .slice(0, 4)
    .map((t) => t.domain);
    ...
    seeds.map(async (seed) => {
      try {
        ...
        for (const c of normalizeDomainCompetitors(res?.data)) {
          ...
          // Without this the pool fills with directories-of-directories.
          if (isNotNicheRival(host) || isScraper(host) || isUnpitchable(host)) continue;
          ...
          if (!matchesVocabulary(host, tokens) && !looksLikeListing(host, "", null)) continue;
  ...
  const intersect = [...byLinker.values()]
    .map((row) => {
      ...
      if (!onNiche && (kind.type === "directory" || kind.type === "serp-listing")) {
