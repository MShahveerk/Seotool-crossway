...
function cacheKey(keyword, geo, device, location, depth) {
  ...
  return `lo-v19:${geo}:${device}:${depth.rankers}x${depth.refdomains}:${h}`;
}
  ...
  return {
    ...
    notes: [
      ...
      location || serp?.location
        ? "The location field picks the local Google SERP (who ranks). Linking-site origin is a separate column. It is not limited to that place unless you filter it."
        : null,
    ].filter(Boolean),
  };
}
