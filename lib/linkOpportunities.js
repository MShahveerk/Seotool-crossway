 ...
 *   2. Free-intent Google searches (write-for-us, guest post, add-listing)
 *      start in parallel with the SERP and rival backlink reads.
 *   3. Rival backlinks land one at a time (streamed to the UI).
...
import { originLocationFromHost } from "./linkOriginLocation.js";
...
/** Live HTTP qualify budget. These cost no vendor credits. */
export const MAX_LIVE_PROBES = 140;
export const PROBE_CONCURRENCY = 14;
export { MAX_FREE_DISCOVERY_QUERIES, DISCOVERY_FETCH_CONCURRENCY, buildFreeDiscoveryQueries };
...
function cacheKey(keyword, geo, device, location, depth) {
  const h = crypto
    .createHash("sha256")
    .update(
      `${String(keyword || "").toLowerCase().trim()}|${String(location || "").toLowerCase().trim()}|`
    )
    .digest("hex")
    .slice(0, 24);
  ...
  const notes = [
    ...
    liveChecked
      ? liveChecked < (probeTotal || liveChecked)
        ? `Live-checked ${liveChecked} of ${probeTotal} pitch candidates before time ran out. Unpaid is a confirmed free submit, claim, or write-for-us route. Refresh to continue from cache.`
        : `Live-checked ${liveChecked} of ${probeTotal || liveChecked} pitch candidates in parallel (paid vs free, spam, relevance). Unpaid is a confirmed free submit, claim, or write-for-us route. Unconfirmed stays out of Unpaid.`
      : null,
    llmProbed
    ...
    location || serp?.location
      ? "The location field picks the local Google SERP (who ranks)."
      : null,
    ...notesExtra,
  ].filter(Boolean);
...
function ingestBacklinks(byLinker, strongest, target, detail, ctx) {
  const { yourHost, analysedHosts, yoursSet, rankingHosts } = ctx;
...
export function classifyDiscoveryKind(result, keyTokens, tokens, query = "") {
  ...
  const pageKind = discoveryPageKind(result, query);
  if (pageKind) return pageKind;
  if (looksLikePublication(domain, result?.title || "", keyTokens)) return "publication";
  if (looksLikeListing(domain, result?.title || "", null) && !isGenericDirectoryName(domain, tokens)) {
    return "directory";
  }
  return null;
}
...
async function loadDiscoverySerps(keyword, { location = "", geo = "us", device = "desktop" } = {}) {
  const unique = buildFreeDiscoveryQueries(keyword, { location });
  if (!unique.length) return [];

  const specs = unique.map((query) => ({
    query,
    key: `ldv3:${crypto
      .createHash("sha256")
  ...
  const cachedOrganic = await Promise.all(
    specs.map(async (spec) => {
      try {
        const cached = await getCachedSnapshot("__discover__", DATA_TYPES.LINK_DISCOVERY, spec.key);
        if (cached?.payload?.organic && !cached.expired) return cached.payload.organic;
      } catch {
        /* best-effort */
      }
      return null;
  ...
  const pending = specs.filter((_, i) => !cachedOrganic[i]);
  const fetched = new Map();
  const concurrency = isSerpApiQuotaExhausted() ? 8 : DISCOVERY_FETCH_CONCURRENCY;
  await mapLimit(pending, concurrency, async (spec) => {
    try {
      const serp = await fetchGoogleSerp(spec.query, {
        location,
        gl: geo === "uk" ? "uk" : geo,
        device,
        num: 10,
      ...
      fetched.set(spec.query, organic);
      await saveSnapshot({
        siteUrl: "__discover__",
        dataType: DATA_TYPES.LINK_DISCOVERY,
        sourceKey: spec.key,
        payload: { query: spec.query, organic },
        creditsSpent: 0,
      });
    } catch (err) {
      logger?.warn?.(`Link discovery "${spec.query}" failed — ${err.message}`);
    }
  });

  const out = [];
  specs.forEach((spec, i) => {
    const organic = cachedOrganic[i] || fetched.get(spec.query) || [];
    for (const row of organic) out.push({ ...row, query: spec.query });
  });
  return out;
}
...
export async function buildLinkOpportunities(siteUrl, keyword, opts = {}, { onProgress } = {}) {
  ...
  // Free-intent SERPs do not need the keyword ladder. Start them immediately
  // so write-for-us / directory hunts overlap the ranker and backlink work.
  const discoveryFetch = loadDiscoverySerps(keyword, { location, geo, device });
  ...
  const discoveryPromise = discoveryFetch.then(async (rows) => {
    let added = 0;
    for (const result of rows) {
      const host = cleanHost(result.domain);
      if (!host || host === yourHost || analysedHosts.has(host)) continue;
      const kind = classifyDiscoveryKind(result, keyTokens, tokens, result.query);
      if (!kind) continue;
      if (byLinker.has(host)) {
    ...
    if (added) {
      await emit(
        "discover",
        `Found ${added} extra unpaid-route prospect${added === 1 ? "" : "s"} (write-for-us, directories, resource pages)`
      );
    }
    return added;
  });
  ...
  await emit("probe", "Checking live submit routes on the best prospects", {
    current: 0,
    total: Math.min(MAX_LIVE_PROBES, intersect.filter((r) => ACTIONABLE_TYPES.has(r.type) && !r.youHaveIt).length || 1),
  });
  ...
  const pitchable = intersect
    ...
    .sort((a, b) => {
      const rank = (r) =>
        ...
        (r.foundVia === "discover" ? 36 : 0) +
        (r.type === "guest-post" ? 40 : 0) +
        (r.type === "directory" ? 18 : 0) +
        (r.type === "serp-listing" ? 28 : 0) +
        (r.onNiche ? 20 : 0) +
        (r.prospectScore || 0);
      return rank(b) - rank(a);
    });
  probeTotal = Math.min(MAX_LIVE_PROBES, pitchable.length);
  ...
  try {
    ...
    const qualified = await qualifyPitchProspects(
      ...
      })),
      {
        ...
        limit: probeTotal,
        llmLimit: Math.min(80, probeTotal),
...
export function linkOpportunitiesCacheKey(siteUrl, keyword, opts = {}) {
  ...
  const sourceKey = cacheKey(keyword, geo, device, location, { rankers, refdomains });
