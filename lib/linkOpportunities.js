...
const COURSE_HUB_RE =
  /(bootcamp|udacity|simplilearn|coursera|udemy|skillshare|codecademy|pluralsight|khanacademy|teachable|thinkific|springboard)/i;
...
function isNotABusinessRival(domain, title = "") {
  return (
...
function cacheKey(keyword, geo, device, location, depth, originCountry = "") {
  ...
  return `lo-v21:${geo}:${device}:${depth.rankers}x${depth.refdomains}:${h}`;
}
...
export async function buildLinkOpportunities(siteUrl, keyword, opts = {}) {
  ...
  for (const row of ladder) {
    ...
    if (isLowValue(host) || isNotABusinessRival(host, row?.title)) continue;
    if (looksLikeListing(host, row?.title, row?.tag)) continue;
  ...
  const intersect = [...byLinker.values()]
    .map((row) => {
      ...
      const behavesLikeNicheDirectory = hits >= 1 && looksLikeListing(row.domain, "", null);
      if (
        !listing &&
        !onNiche &&
        !provenByLinks &&
