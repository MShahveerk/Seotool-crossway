    ...
    <tr><td align="center">
      <table width="100%" style="max-width:560px;background:#fff;border-radius:12px;overflow:hidden;">
        ...
        <tr><td style="padding:32px;">
          ...
          <p style="margin:0 0 16px;color:#4b5563;font-size:15px;line-height:1.6;">
            Attached are your latest reports for <strong>${property}</strong>.
            ${includesWebsite ? "This pack includes website and SEO sections." : "This account is Meta-only (no website/GTM link), so only social media reports are included."}
          </p>
          <ul style="margin:0 0 16px;padding-left:20px;color:#4b5563;font-size:14px;">
            ${sectionList.map((s) => `<li>${s.replace(/-/g, " ")}</li>`).join("")}
          </ul>
          <p style="margin:0;color:#9ca3af;font-size:12px;">Crossway SEO Tool · ${new Date().toLocaleDateString()}</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
