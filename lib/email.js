...
export async function sendClientReportEmail({ to, recipientName, context, attachments = [], reportMonth }) {
  ...
  const property = context?.displayName || context?.siteKey || "your account";
  const sectionList = attachments.map((a) => a.section).filter(Boolean);
  const includesWebsite = context?.includeWebsiteReports;
  const subject = `Your marketing reports — ${property} (${reportMonth || "latest"})`;
...
<body style="margin:0;padding:0;background:#f7f6f3;font-family:Georgia,'Times New Roman',serif;">
    ...
    <tr><td align="center">
      <table width="100%" style="max-width:560px;background:#fff;border:1px solid #d4d0c8;">
        <tr><td style="background:#111;padding:28px 32px;border-bottom:3px solid #111;">
          ...
          <h1 style="margin:0;font-size:22px;color:#fff;font-weight:600;font-family:Helvetica,Arial,sans-serif;">Your performance reports</h1>
        </td></tr>
        <tr><td style="padding:36px 32px;">
          ...
          <p style="margin:0 0 20px;color:#444;font-size:15px;line-height:1.75;">
            Attached are your client-ready reports for <strong style="color:#111;">${property}</strong>.
            ${includesWebsite ? "They cover social media followers and website performance on Google — written in plain language, no technical jargon." : "Your account is social-media only, so this report focuses on follower counts across your platforms."}
          </p>
          <ul style="margin:0 0 24px;padding-left:18px;color:#444;font-size:14px;line-height:1.8;">
            ${sectionList.map((s) => `<li style="margin-bottom:6px;">${s === "smm" ? "Social media followers" : s === "website" ? "Website performance" : s.replace(/-/g, " ")}</li>`).join("")}
          </ul>
  ...
  const text = `Hi ${name},\n\nYour marketing reports for ${property} are attached (${sectionList.join(", ")}).\n`;
