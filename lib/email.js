...
export async function sendEmail({ to, subject, html, text, attachments }) {
  ...
  try {
    ...
    // Inline the Crossway logo when the template references it, so it renders
    ...
    if (html && html.includes(`cid:${LOGO_CID}`)) {
      ...
      if (logoPath) {
        allAttachments = [
          ...allAttachments,
          { filename: "crossway-logo.png", path: logoPath, cid: LOGO_CID },
        ];
      }
    }
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#0EFF2A 0%,#0BCC22 100%);padding:32px 40px;text-align:center;">
              <h1 style="margin:0;color:#000000;font-size:24px;font-weight:700;font-family:Nunito,Helvetica,Arial,sans-serif;">Crossway SEO Tool</h1>
            </td>
          </tr>
          <!-- Body -->
          <tr>
    ...
    <tr><td align="center">
      <table width="100%" style="max-width:560px;background:#fff;border:1px solid #d4d0c8;">
        <tr><td style="background:#111;padding:28px 32px;border-bottom:3px solid #111;">
          <p style="margin:0 0 6px;font-size:10px;letter-spacing:0.12em;text-transform:uppercase;color:#999;font-family:Nunito,Helvetica,Arial,sans-serif;">Crossway SEO Tool</p>
        ...
        <tr><td style="padding:36px 32px;">
          ...
          <p style="margin:0;font-size:11px;color:#888;font-family:Nunito,Helvetica,Arial,sans-serif;">&copy; ${new Date().getFullYear()} Crossway SEO Tool, ${new Date().toLocaleDateString()}</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
export async function sendBlogPublishNotification(blog, method, externalId) {
  ...
  const html = generateBlogPublishedEmail(blog, method, externalId, baseUrl);
  const text = `Blog "${title}" was published via ${method || "delivery chain"}${externalId ? ` (ID: ${externalId})` : ""}.`;
