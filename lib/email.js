...
export async function sendPostStatusChangeNotification(approval, actionUser, status, detail = "") {
  ...
  if (approval.assignee?.email) {
    ...
    await sendEmail({ to: approval.assignee.email, subject, html, text });
  ...
  if (approval.createdBy?.email) {
    if (!notifiedEmails.has(approval.createdBy.email)) {
      ...
      await sendEmail({ to: approval.createdBy.email, subject, html, text });
  ...
  try {
    ...
    for (const smm of relevantSmms) {
      ...
      if ((isCreator || isSiteMatch || isMetaMatch) && smm.email && !notifiedEmails.has(smm.email)) {
        ...
        await sendEmail({ to: smm.email, subject, html, text });
    ...
    <tr><td align="center">
      <table width="100%" style="max-width:560px;background:#fff;border:1px solid #d4d0c8;">
        <tr><td style="background:#111;padding:28px 32px;border-bottom:3px solid #111;">
          <p style="margin:0 0 6px;font-size:10px;letter-spacing:0.12em;text-transform:uppercase;color:#999;font-family:Helvetica,Arial,sans-serif;">Crossway SEO Tool</p>
          <h1 style="margin:0;font-size:22px;color:#fff;font-weight:600;font-family:Helvetica,Arial,sans-serif;">Your ${monthLabel} reports</h1>
        </td></tr>
        <tr><td style="padding:36px 32px;">
          ...
          <p style="margin:0;font-size:11px;color:#888;font-family:Helvetica,Arial,sans-serif;">&copy; ${new Date().getFullYear()} Crossway SEO Tool, ${new Date().toLocaleDateString()}</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
export async function sendBlogApprovalNotification(email, blog, recipient, token) {
  const baseUrl = process.env.NEXTAUTH_URL || "http://localhost:3000";
  ...
  const approveUrl = token
    ...
    : `${baseUrl}/login`;
  const subject = `Review blog: ${blog.title}`;
  const html = `
<!DOCTYPE html><html><body style="font-family:Georgia,serif;padding:24px;color:#222;">
  <h2 style="font-family:Helvetica,sans-serif;">Blog review requested</h2>
  <p>Hi ${name},</p>
  <p>Please review the blog <strong>${blog.title}</strong> for ${blog.siteLink}.</p>
  <p><a href="${approveUrl}" style="color:#111;font-weight:bold;">Open review</a></p>
</body></html>`;
  const text = `Hi ${name},\n\nPlease review the blog "${blog.title}" for ${blog.siteLink}.\n${approveUrl}\n`;
  return sendEmail({ to: email, subject, html, text });
}

export async function sendBlogPublishNotification(blog, method, externalId) {
  const subject = `Blog published: ${blog.title}`;
  const html = `<p>Blog <strong>${blog.title}</strong> was published via <strong>${method || "delivery chain"}</strong>${externalId ? ` (ID: ${externalId})` : ""}.</p>`;
  const text = `Blog "${blog.title}" published via ${method || "delivery chain"}.`;
