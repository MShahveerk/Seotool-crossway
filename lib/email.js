  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:32px 16px;">
    <tr><td align="center">
      <table width="100%" style="max-width:560px;background:#fff;border-radius:12px;overflow:hidden;">
        <tr><td style="background:linear-gradient(135deg,#0EFF2A,#0BCC22);padding:24px;text-align:center;">
          <h1 style="margin:0;font-size:20px;color:#000;">Your marketing reports</h1>
        </td></tr>
        <tr><td style="padding:32px;">
          <p style="margin:0 0 12px;color:#374151;font-size:15px;">Hi ${name},</p>
          <p style="margin:0 0 16px;color:#4b5563;font-size:15px;line-height:1.6;">
            Attached are your easy-to-read performance reports for <strong>${property}</strong>.
            ${includesWebsite ? "They cover social media, your website on Google, and suggested improvements — written in plain language." : "This account is social-media only (no website linked yet), so the report focuses on your follower counts."}
          </p>
          <ul style="margin:0 0 16px;padding-left:20px;color:#4b5563;font-size:14px;">
            ${sectionList.map((s) => `<li style="margin-bottom:4px;">${s.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}</li>`).join("")}
          </ul>
          <p style="margin:0;color:#9ca3af;font-size:12px;">&copy; ${new Date().getFullYear()} Crossway SEO Tool · ${new Date().toLocaleDateString()}</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
