...
function parseBacklinkLinks(payload) {
    ...
    unique.map(async (domain) => {
      try {
        ...
        const [summaryRes, refRes, listRes] = await Promise.all([
          ...
          fetchBacklinksRefdomains(domain, domain, { allowManual: true, limit: 50 }).catch(() => null),
          fetchBacklinksList(domain, domain, { allowManual: true, limit: 25 }).catch(() => null),
        ]);
        ...
        const anchors = (s?.topAnchors || [])
          ...
          .filter((a) => a.anchor)
          .slice(0, 15);
        if ((!s || !s.hasData) && !hosts.length && !links.length) return;
        out.set(domain, {
          ...
          refdomainList: hosts.slice(0, 50),
          topAnchors: anchors,
          links,
