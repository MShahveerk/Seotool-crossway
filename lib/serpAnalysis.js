...
import { fetchDomainKeywords, fetchBacklinksSummary } from "./seranking/api.js";
...
async function fetchBacklinkProfiles(domains) {
  ...
  await Promise.all(
    unique.map(async (domain) => {
      try {
        // Cache each domain in its own bucket — backlinks/summary caches by siteUrl
        // with an empty sourceKey, so a shared siteUrl across domains would collide.
        const { data, fromCache } = await fetchBacklinksSummary(domain, domain, { allowManual: true });
