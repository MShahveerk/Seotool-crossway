...
import { fetchDomainKeywords, fetchBacklinksSummary, fetchBacklinksRefdomains } from "./seranking/api.js";
  ...
  await Promise.all(
    unique.map(async (domain) => {
      try {
        ...
        const [summaryRes, refRes] = await Promise.all([
          fetchBacklinksSummary(domain, domain, { allowManual: true }).catch(() => null),
          fetchBacklinksRefdomains(domain, domain, { allowManual: true, limit: 25 }).catch(() => null),
        ]);
        const s = summaryRes ? normalizeBacklinksSummary(summaryRes.data) : null;
        const hosts = Array.isArray(refRes?.hosts) ? refRes.hosts : [];
        if ((!s || !s.hasData) && !hosts.length) return;
        out.set(domain, {
          backlinks: s?.backlinks ?? null,
          refdomains: s?.refdomains ?? (hosts.length || null),
          domainTrust: s?.domainInlinkRank ?? s?.inlinkRank ?? null,
          dofollow: s?.dofollowBacklinks ?? null,
          refdomainList: hosts.slice(0, 25),
...
export async function buildSerpAnalysis(siteUrl, keyword, opts = {}) {
  ...
  // 7. Benchmark from the top rankers (words, structure, speed, authority, backlinks).
  const summary = {
    avgWordCount: avg(topRankers.map((c) => c.wordCount)),
    avgH2Count: avg(topRankers.map((c) => c.h2Count)),
    ...
    commonSchemas: [...new Set(topRankers.flatMap((c) => c.schemas))],
    leadersScanned: topRankers.filter((c) => c.scanned).length,
