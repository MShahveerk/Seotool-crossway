...
import { isSerankingConfigured, DEFAULT_SOURCE } from "./seranking/config.js";
 ...
 */
async function fetchDomainKeywordProfiles(domains) {
  const out = new Map();
  ...
  await Promise.all(
    unique.map(async (domain) => {
      try {
        ...
        const rows = normalizeKeywordResearchList(data, DEFAULT_SOURCE)
          .filter((r) => r.keyword && r.position != null)
          .sort((a, b) => (b.traffic ?? 0) - (a.traffic ?? 0) || (a.position ?? 999) - (b.position ?? 999));
        out.set(domain, {
...
export async function buildSerpAnalysis(siteUrl, keyword, opts = {}) {
  ...
  const [authMap, keywordProfiles] = await Promise.all([
    ...
    fetchDomainKeywordProfiles(scanTargets.map((t) => t.domain)),
  ]);
