...
export async function testWordpressConnection(config) {
  ...
  try {
    ...
    try {
      const draftProbe = await axios.get(`${base}/wp-json/wp/v2/posts`, {
        auth,
        timeout: 15000,
        params: { status: "draft", context: "edit", per_page: 1 },
      });
      draftCount = Number(draftProbe.headers["x-wp-total"] ?? draftProbe.data?.length ?? 0);
    } catch {
      draftCount = null;
    }
    try {
      const futureProbe = await axios.get(`${base}/wp-json/wp/v2/posts`, {
        auth,
        timeout: 15000,
        params: { status: "future", context: "edit", per_page: 1 },
      });
      futureCount = Number(futureProbe.headers["x-wp-total"] ?? futureProbe.data?.length ?? 0);
    } catch {
      futureCount = null;
    }
    return {
