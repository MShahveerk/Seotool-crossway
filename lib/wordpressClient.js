...
async function countWordpressPostsForStatus(config, status) {
  ...
  try {
    const res = await axios.get(`${base}/wp-json/wp/v2/posts`, {
      auth,
      timeout: 15000,
      params: { status, context: "edit", per_page: 1, type: "post" },
    });
    return {
      status,
      total: Number(res.headers["x-wp-total"] || res.data?.length || 0),
      totalPages: Number(res.headers["x-wp-totalpages"] || 1),
      ok: true,
    };
  } catch (error) {
    return { status, total: null, totalPages: 0, ok: false, error: error.message };
  }
}
...
export async function getWordpressPostStatusSummary(config) {
  ...
  for (const part of parts) {
    ...
    if (part.ok && typeof part.total === "number") apiTotal += part.total;
  }
  return { counts, apiTotal, wordpressUrl: getWordpressConfig(config).base };
}
...
export async function testWordpressConnection(config) {
  ...
  try {
    ...
    let sampleDrafts = [];
    try {
      const sampleRes = await axios.get(`${base}/wp-json/wp/v2/posts`, {
        auth,
        timeout: 15000,
        params: {
          status: "draft",
      ...
      sampleDrafts = (sampleRes.data || []).map((p) => ({
        id: p.id,
...
export async function fetchWordpressPosts(
  ...
  for (const status of uniqueStatuses) {
    ...
    for (let page = 1; page <= maxPages; page += 1) {
      ...
      if (page === 1) {
        statusCounts[status] = Number(res.headers["x-wp-total"] || batch.length);
      }
  ...
  return { posts: merged, statusCounts, scheduledDraftCount };
}
