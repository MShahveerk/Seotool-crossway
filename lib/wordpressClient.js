...
export async function upsertWordpressPost(config, payload, existingWpId = null) {
  ...
  if (payload.date) {
    // WordPress treats `date` as site-local and `date_gmt` as UTC.
    // Always send both from the same UTC instant so WP and Crossway stay aligned.
    ...
    if (!Number.isNaN(when.getTime())) {
      ...
      body.date_gmt = formatWordpressGmtDate(when);
      body.date = formatWordpressLocalDate(when, siteTimezone);
    }
