...
export async function getWordpressPostStatusSummary(config) {
  ...
  if (statusFilterBroken) {
    ...
    let trashCount = null;
    try {
      const trashRes = await wpGet(`${getWordpressConfig(config).base}/wp-json/wp/v2/posts`, {
        auth: getWordpressConfig(config).auth,
        params: { status: "trash", context: "edit", per_page: 1, page: 1 },
      });
      trashCount = Number(trashRes.headers["x-wp-total"] || 0);
    } catch {
      trashCount = null;
    }

    const counts = tallyWordpressStatuses(posts);
    if (typeof trashCount === "number") counts.trash = trashCount;
    ...
    return {
      ...
      statusFilterNote:
        "WordPress returned the same total for every status (often CDN caching). Counts below are from actual post records.",
    };
  }
