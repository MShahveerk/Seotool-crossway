...
import { logWordpressHttp, logWordpressVerbose } from "./wordpressLogger.js";
...
async function wpGet(url, { auth, params = {}, timeout = 30000, logLabel = "wpGet" } = {}) {
...
export async function probeWordpressAccess(config) {
  ...
  const probes = await Promise.all([
    probe("public published posts", { status: "publish", context: "view" }, { authenticated: false }),
    probe("authenticated publish", { status: "publish", context: "view" }, { authenticated: true }),
    probe("authenticated drafts", { status: "draft", context: "edit" }, { authenticated: true }),
    probe("authenticated scheduled", { status: "future", context: "edit" }, { authenticated: true }),
    probe("authenticated trash", { status: "trash", context: "edit" }, { authenticated: true }),
    probe("authenticated all statuses", { status: "any", context: "edit" }, { authenticated: true }),
  ]);
...
export async function testWordpressConnection(config) {
  ...
  try {
    ...
    const summary = await getWordpressPostStatusSummary(config);
    const { counts, apiTotal } = summary;
    const access = await probeWordpressAccess(config);
    ...
    return {
      ...
      roles: access.user?.roles || res.data?.roles || [],
      wordpressUrl: base,
      canListDrafts: counts.draft !== null,
      draftCount: counts.draft,
      futureCount: counts.future,
      statusCounts: counts,
      apiTotal,
      statusFilterBroken: summary.statusFilterBroken,
      statusFilterNote: summary.statusFilterNote || null,
      accessProbe: access,
      diagnosis: access.diagnosis || null,
      sampleDrafts,
    };
  } catch (error) {
...
export async function fetchWordpressPosts(
  ...
  let probeRes;
  try {
    probeRes = await wpGet(`${base}/wp-json/wp/v2/posts`, {
      auth,
      timeout: 30000,
      params: {
    ...
    throw formatWordpressError(error, "Fetching WordPress posts");
  }
  ...
  const fetchStatus = statusFilterBroken ? "any" : uniqueStatuses.join(",");
  ...
  for (let page = 1; page <= maxPages; page += 1) {
    ...
    try {
      res =
        page === 1 && !statusFilterBroken
          ? probeRes
          : await wpGet(`${base}/wp-json/wp/v2/posts`, {
              auth,
              timeout: 30000,
              params: {
                status: fetchStatus,
      ...
      if (statusFilterBroken && page === 1) {
        break;
      }
      throw formatWordpressError(error, `Fetching WordPress posts (${fetchStatus})`);
    }
  ...
  if (!merged.length) {
    const fallbackPosts = await collectWordpressPostsByStatuses(config, uniqueStatuses, {
      perPage,
      maxPages,
      embed: true,
    });
    ...
    if (fallbackPosts.length && !merged.length) statusFilterBroken = true;
  }
