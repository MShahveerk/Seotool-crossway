...
/**
 * Re-fetch a WP post and confirm it is actually live.
 * Write responses can lie / race; this is the source of truth before we email "published".
 */
export async function verifyWordpressPostPublished(config, postId) {
  if (!postId) return { ok: false, status: null, link: null };
  const { base, auth } = getWordpressConfig(config);
  try {
    const res = await axios.get(`${base}/wp-json/wp/v2/posts/${postId}`, {
      auth,
      timeout: 20000,
      params: { context: "edit", _fields: "id,status,link,date,date_gmt" },
    });
    const status = String(res.data?.status || "");
    const ok = status === "publish" || status === "private";
    return {
      ok,
      status: status || null,
      link: res.data?.link || null,
      externalId: res.data?.id != null ? String(res.data.id) : String(postId),
    };
  } catch (error) {
    throw formatWordpressError(error, `Verifying WordPress post #${postId}`);
  }
}
...
export async function upsertWordpressPost(config, payload, existingWpId = null, options = {}) {
  ...
  // Live publish: prefer the original scheduled instant when already due so the
  // public WP timestamp matches Crossway. Only nudge slightly into the past when
  // we must prevent WP from bouncing the post back to "future".
  let effectiveWhen = when;
  if (forcePublish) {
    if (when && when.getTime() <= now + 90_000) {
      effectiveWhen = when.getTime() <= now ? when : new Date(now - 5_000);
    } else {
      effectiveWhen = new Date(now - 5_000);
    }
  }

  // IMPORTANT: WordPress ignores date_gmt when `date` is also sent, then re-derives
  // GMT from local `date` using the real WP site timezone. A wrong resolved TZ skews
  // the schedule. Always send date_gmt only for absolute timing.
  if (effectiveWhen) {
  ...
  // If WP still returned future/draft on a live publish, force status + date_gmt only.
  if (
    forcePublish &&
    wpId &&
    res.data?.status &&
    ...
    const forceBody = {
      status: "publish",
      date_gmt: formatWordpressGmtDate(forceWhen),
    };
    console.warn(
      `[wordpress] status was "${res.data.status}" after publish upsert — forcing status=publish via date_gmt`
    );
...
export async function syncWordpressSchedule(config, wpPostId, scheduledFor, { asDraft = false } = {}) {
  ...
  const result = await upsertWordpressPost(
    config,
    {
  ...
  // Confirm WP stored the same UTC instant (date_gmt-only writes should already match).
  try {
    const { base, auth } = getWordpressConfig(config);
    const check = await axios.get(`${base}/wp-json/wp/v2/posts/${wpPostId}`, {
      auth,
      timeout: 15000,
      params: { context: "edit", _fields: "id,status,date,date_gmt" },
    });
    const storedGmt = check.data?.date_gmt ? new Date(`${check.data.date_gmt}Z`).getTime() : NaN;
    const skewMs = Number.isFinite(storedGmt) ? Math.abs(storedGmt - when.getTime()) : NaN;
    if (Number.isFinite(skewMs) && skewMs > 90_000) {
      console.warn(
        `[wordpress] schedule skew ${Math.round(skewMs / 1000)}s for #${wpPostId} — rewriting date_gmt`
      );
      return upsertWordpressPost(
        config,
        { date: when.toISOString(), status: asDraft ? "draft" : "future" },
        wpPostId,
        { mode: asDraft ? "draft" : "schedule", datesOnly: true, forceSchedule: !asDraft }
      );
    }
  } catch (err) {
    console.warn(`[wordpress] schedule verify skipped for #${wpPostId}: ${err.message}`);
  }
