...
import {
  ...
  readHumanizerConfig,
  writeHumanizerConfig,
} from "./humanizerConfig.js";
import {
...
function defaultSiteRow(siteLink) {
  return {
    ...
    humanizerProvider: "openai",
    humanizerModel: "gpt-5.4-mini",
    humanizerPrompt: "",
    humanizerSkill: "",
    openaiApiKey: null,
...
export async function getSiteStudioConfig(siteLink) {
  ...
  const stored = await readHumanizerConfig(link);
  return mergeHumanizerConfig(base, stored);
}
...
export function sanitizeSiteConfigForClient(row) {
  ...
  return {
    ...
    agentReady: {
      ...
      humanizer: hasProviderKey(row.humanizerProvider || row.agent2Provider, row),
...
export async function saveSiteStudioConfig(siteLink, input = {}) {
  ...
  const saved = await prisma.postAutomationSiteConfig.upsert({
    where: { siteLink: link },
    create: data,
    update: data,
  });

  if (humanizerFieldsTouched(input)) {
    await writeHumanizerConfig(link, input);
  }

  const stored = await readHumanizerConfig(link);
  return mergeHumanizerConfig(saved, stored);
}
