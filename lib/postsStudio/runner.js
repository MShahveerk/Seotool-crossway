...
import { runPostHumanizerAgent } from "./humanizer.js";
import { appendHashtagsIfMissing } from "./humanizerText.js";
...
import { isLocalStudioUploadPath } from "../studioProjectLabel.js";
import { clipVarchar, DB_VARCHAR_ID, DB_VARCHAR_SITE, DB_VARCHAR_TOPIC } from "../dbVarchar.js";
...
function assertKeysReady(config) {
  ...
  if (config.humanizerEnabled) {
    checks.push(["humanizer", config.humanizerProvider || config.agent2Provider]);
  }
...
function finalizePostCopy(post, topic) {
  const title = String(post?.title || topic || "")
    .trim()
    .slice(0, 255);
  const caption = appendHashtagsIfMissing(post?.caption, post?.hashtags);
...
export async function enqueueStudioRun({
  ...
  const topicOverflows = resolvedTopic.length > DB_VARCHAR_TOPIC;
  const humanizerEnabled = Boolean(config.humanizerEnabled);
  const contextStage =
    hasOverrides || hasRevision || topicOverflows || humanizerEnabled
      ? [
          {
            agent: "_context",
            status: "meta",
            humanizerEnabled,
            ...(topicOverflows ? { topicFull: resolvedTopic } : {}),
  ...
  const run = await prisma.postAutomationRun.create({
    data: {
      siteLink: clipVarchar(String(siteLink).trim(), DB_VARCHAR_SITE),
      trigger: clipVarchar(trigger, 32) || "manual",
      status: "queued",
      topic: clipVarchar(resolvedTopic, DB_VARCHAR_TOPIC) || null,
      seedPromptSnapshot: config.seedPrompt || "",
      keywordsSnapshot: config.hooksOrKeywords || "",
      stagesJson: contextStage,
      triggeredById: triggeredById ? clipVarchar(triggeredById, DB_VARCHAR_ID) : null,
    },
  });
...
export async function executeStudioRun(runId, { generateImage = true } = {}) {
  ...
  try {
    ...
    const metaStage = Array.isArray(run.stagesJson)
      ...
      : null;
    const topic =
      (metaStage?.topicFull && String(metaStage.topicFull).trim()) ||
      run.topic ||
      pickTopic(config);
    const revision =
    ...
    const operatorImagePath = isLocalStudioUploadPath(config.operatorImagePath)
      ? String(config.operatorImagePath).trim()
      : "";
    ...
    const regenImage = revision
      ...
      : generateImage !== false && !operatorImagePath;
    ...
    if (regenText) {
      ...
      post = applyStudioCalendarToDraft(a2.json || {}, {
        keepText: `${topic}\n${config.hooksOrKeywords || ""}\n${config.seedPrompt || ""}`,
      });
      ({ title, caption } = finalizePostCopy(post, topic));
      ...
      if (config.humanizerEnabled) {
        ...
        try {
          const hum = await runPostHumanizerAgent({ config, post, topic });
          post = { ...post, ...hum.json };
          ({ title, caption } = finalizePostCopy(post, topic));
          post = { ...post, title, caption };
          stages = await updateStage(runId, stages, stages.length - 1, {
            ...
            note: hum.usedCopywriterCaption
              ? "Model omitted caption. Kept the copywriter draft and ran the deterministic scrub."
              : undefined,
            preview: previewFromJson({
              title,
        ...
        if (await isCancelled(runId)) throw Object.assign(new Error("Cancelled."), { cancelled: true });
        if (!caption) throw new Error("Humanizer returned empty caption.");
      }
    } else {
    ...
    } else if (operatorImagePath) {
      img = { imagePath: operatorImagePath, backupImagePaths: [] };
      stages = await pushStage(runId, stages, {
        agent: "image",
        role: "Image — supplied by you",
        status: "succeeded",
        startedAt: new Date().toISOString(),
        finishedAt: new Date().toISOString(),
        preview: operatorImagePath,
        note: "You supplied this image. The image agent was skipped.",
      });
