...
function assertKeysReady(config) {
  const checks = [
    ...
    ["image", config.imageProvider === "openrouter" ? "openrouter" : "openai"],
  ];
...
export async function executeStudioRun(runId, { generateImage = true } = {}) {
  ...
  try {
    ...
    const post = a2.json || {};
    const title = String(post.title || topic).trim().slice(0, 255);
    let caption = String(post.caption || "").trim().slice(0, 2000);
  ...
  try {
    if (source === "excel") {
      ...
      const overrides = {
        ...
        hooksOrKeywords: row.keywords || row.topic || "",
        ctaText: row.ctaText || site.ctaText,
        ctaUrl: row.ctaUrl || site.ctaUrl,
        defaultPlatform: row.platform || site.defaultPlatform,
        topicImagePrompt: row.imagePrompt || "",
      };
      try {
