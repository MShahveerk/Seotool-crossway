...
export function generateBlogPublishedEmail(blog, method, externalId, baseUrl, opts = {}) {
  ...
  const bodyHtml = `
    ...
    ${renderMetaTable([
      ...
      ["Live URL", liveLink ? `<a href="${escapeHtml(liveLink)}" style="color:#1d9c35;">${escapeHtml(liveLink)}</a>` : null],
...
export function generateHeadingsApprovalEmail({
  ...
  const displayUrl = String(siteLink || "").replace(/^https?:\/\//i, "").replace(/\/+$/, "") || "Crossway client";
  ...
  const bodyHtml = `
    ...
    ${renderAutomatedNote("This is an automated headings review from Crossway Blog Studio. If you received it in error, contact your administrator.")}`;

  return renderEmailShell({
    title: "Headings approval required — Crossway",
