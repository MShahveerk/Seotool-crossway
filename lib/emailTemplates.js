...
function formatScheduleLabel(dateValue) {
  ...
  const pkt = d.toLocaleString("en-US", { timeZone: "Asia/Karachi", dateStyle: "full", timeStyle: "short" });
  const utc = d.toLocaleString("en-US", { timeZone: "UTC", dateStyle: "full", timeStyle: "short" });
...
function renderImageBlock(label, imageUrl, alt) {
  if (!imageUrl) return "";
  return `
    <div style="margin:0 0 28px;">
      ${sectionLabel(label)}
      <div style="border:1px solid ${T.border};border-radius:8px;overflow:hidden;background-color:${T.altRow};text-align:center;">
        <img src="${imageUrl}" alt="${escapeHtml(alt || label)}" style="max-width:100%;max-height:520px;width:auto;height:auto;display:block;margin:0 auto;" />
      </div>
    </div>`;
}
...
function renderEmailShell({ title, preheader = "", badge, badgeBg = T.offWhite, badgeColor = T.black, heading, subheading = "", bodyHtml, baseUrl }) {
  const logoUrl = `${baseUrl}/crossway-logo.png`;
  return `
<!DOCTYPE html>
<html>
<head>
    ...
    <tr>
      <td align="center">
        <table class="container" width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;background-color:${T.card};border-radius:10px;overflow:hidden;border:1px solid ${T.border};">
          ...
          <!-- Masthead: logo on off-white -->
          <tr>
            <td style="background-color:${T.card};padding:32px 40px 26px;text-align:center;border-bottom:1px solid ${T.border};">
              <img src="${logoUrl}" width="160" alt="Crossway Consulting" style="display:inline-block;width:160px;max-width:60%;height:auto;" />
            </td>
          </tr>
...
export function generateApprovalNotificationEmail(approval, assignee, baseUrl, token) {
  ...
  const imageUrl = resolveMediaUrl(approval.imagePath, baseUrl);
  ...
  const bodyHtml = `
    ...
    ${renderImageBlock("Attached media", imageUrl, "Post media")}
...
const BLOG_SOURCE_LABELS = {
  ...
  inbound: "Received from WordPress",
};

export function generateBlogApprovalEmail(blog, recipient, baseUrl, token, creator = null) {
  ...
  const imageUrl = resolveMediaUrl(blog.featuredImagePath, baseUrl);
  ...
  const bodyHtml = `
    ...
    ${renderMetaTable([
      ...
      ["Slug", blog.slug ? `<span style="font-family:Consolas,Menlo,monospace;font-size:13px;">${escapeHtml(blog.slug)}</span>` : null],
    ...
    ${renderImageBlock("Featured image", imageUrl, blog.featuredImageAlt || title)}
...
export function generateBlogPublishedEmail(blog, method, externalId, baseUrl) {
  const title = blog.userEditedTitle || blog.title || "Untitled blog";
  const displayUrl = String(blog.siteLink || "").replace(/^https?:\/\//i, "").replace(/\/+$/, "") || "Crossway client";
  const imageUrl = resolveMediaUrl(blog.featuredImagePath, baseUrl);
  ...
  const bodyHtml = `
    <p style="margin:0 0 28px;color:${T.body};font-size:14px;line-height:1.7;">
      The blog article below has been published successfully.
    </p>
    ...
    ${renderMetaTable([
      ...
      ["External ID", externalId ? `<span style="font-family:Consolas,Menlo,monospace;font-size:13px;">${escapeHtml(String(externalId))}</span>` : null],
    ])}
