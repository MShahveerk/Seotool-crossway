/**
 * RoboSEO.Ai transactional email templates.
 * Shared black / off-white shell with the RoboSEO.Ai logo, plus builders for
...
function sanitizeEmailHtml(html) {
  return String(html || "")
    ...
    // Keep article body readable — models often wrap most copy in <strong>/<b>.
...
/**
 * Shared shell: off-white masthead with the RoboSEO.Ai logo, black title band,
...
function renderEmailShell({ title, preheader = "", badge, badgeBg = T.offWhite, badgeColor = T.black, heading, subheading = "", bodyHtml, baseUrl }) {
  ...
  const logoUrl = "cid:roboseo-logo";
  return `
<!DOCTYPE html>
<html>
<head>
        ...
        <table class="container" width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;background-color:${T.card};border-radius:10px;overflow:hidden;border:1px solid ${T.border};">
          ...
          <tr>
            <td style="background-color:#ffffff;padding:28px 40px 22px;text-align:center;border-bottom:1px solid ${T.border};">
              <img src="${logoUrl}" width="150" alt="RoboSEO.Ai" style="display:inline-block;width:150px;max-width:55%;height:auto;" />
            </td>
          </tr>
          ...
          <tr>
            <td style="background-color:${T.altRow};padding:24px 40px;text-align:center;border-top:1px solid ${T.border};">
              <p style="margin:0 0 4px;color:${T.muted};font-size:11px;letter-spacing:2px;text-transform:uppercase;font-weight:600;">RoboSEO.Ai</p>
              <p style="margin:0;color:${T.faint};font-size:11px;">&copy; ${new Date().getFullYear()} RoboSEO.Ai. All rights reserved.</p>
            </td>
          </tr>
  ...
  return `
    <div style="text-align:center;margin:16px 0 0;">
      <p style="margin:0;color:${T.muted};font-size:13px;">
        ${escapeHtml(text)} <a href="${dashboardUrl}" style="color:${T.black};font-weight:700;text-decoration:underline;">Open RoboSEO.Ai Dashboard</a>
      </p>
    </div>`;
}
...
export function generateApprovalNotificationEmail(approval, assignee, baseUrl, token, mediaOptions = {}) {
  ...
  const siteName = approval.selectedSite || approval.siteLink || "RoboSEO.Ai client";
  ...
  const bodyHtml = `
    ...
    ${renderAutomatedNote("This is an automated notification from the RoboSEO.Ai content scheduler. If you received it in error, please contact your administrator.")}`;

  return renderEmailShell({
    title: "Post approval required — RoboSEO.Ai",
...
export function generateStatusChangeEmail(approval, actionUser, status, detail, baseUrl, mediaOptions = {}) {
  ...
  const bodyHtml = `
    ...
    ${renderAutomatedNote("This is an automated status update from the RoboSEO.Ai content scheduler.")}`;

  return renderEmailShell({
    title: `Post ${tone.label.toLowerCase()} — RoboSEO.Ai`,
...
export function generateBlogApprovalEmail(blog, recipient, baseUrl, token, creator = null, mediaOptions = {}) {
  ...
  const displayUrl = String(blog.siteLink || "").replace(/^https?:\/\//i, "").replace(/\/+$/, "") || "RoboSEO.Ai client";
  ...
  const bodyHtml = `
    <p style="margin:0 0 8px;color:${T.ink};font-size:16px;font-weight:400;line-height:1.5;">
      Hello ${escapeHtml(recipient?.name || "there")},
    </p>
    <p style="margin:0 0 28px;color:${T.body};font-size:14px;font-weight:400;line-height:1.7;">
    ...
    <div style="margin:32px 0 0;">
      ...
      <div style="border:1px solid ${T.border};border-radius:8px;padding:28px 30px;background-color:#fffefb;color:#2a2823;font-size:15px;font-weight:400;line-height:1.8;font-family:${SERIF};word-break:break-word;">
        ${sanitizeEmailHtml(content)}
      </div>
    </div>` : ""}

    ${renderAutomatedNote("This is an automated notification from the RoboSEO.Ai blog approval queue. If you received it in error, please contact your administrator.")}`;

  return renderEmailShell({
    title: "Blog approval required — RoboSEO.Ai",
...
export function generateBlogPublishedEmail(blog, method, externalId, baseUrl, opts = {}) {
  ...
  const bodyHtml = `
    ...
    ${renderMetaTable([
      ...
      ["Live URL", liveLink ? `<a href="${escapeHtml(liveLink)}" style="color:#00A3FF;">${escapeHtml(liveLink)}</a>` : null],
    ...
    ${renderAutomatedNote("This is an automated publish confirmation from the RoboSEO.Ai blog approval queue.")}`;

  return renderEmailShell({
    title: "Blog published — RoboSEO.Ai",
 ...
 */
export function generateHeadingsApprovalEmail({
  recipient,
  topic,
  siteLink,
  headings,
  ...
  const displayUrl = String(siteLink || "").replace(/^https?:\/\//i, "").replace(/\/+$/, "") || "RoboSEO.Ai client";
  const h = headings && typeof headings === "object" ? headings : {};
  const sectionRows = (h.sections || [])
    .map((s, i) => {
      const h3s = (s.subsections || [])
        .map((sub) => sub.heading_h3)
        .filter(Boolean)
        .map((t) => `<div style="margin:4px 0 0 16px;color:${T.body};font-size:13px;">H3 · ${escapeHtml(t)}</div>`)
        .join("");
      return `
        <tr${i % 2 === 0 ? ` style="background-color:${T.altRow};"` : ""}>
          <td style="padding:14px 18px;border-bottom:1px solid ${T.border};">
            <div style="font-size:14px;font-weight:700;color:${T.ink};">H2 · ${escapeHtml(s.heading_h2 || "")}</div>
            ${h3s}
          </td>
        </tr>`;
    })
  ...
  const bodyHtml = `
    ...
    <p style="margin:0 0 28px;color:${T.body};font-size:14px;font-weight:400;line-height:1.7;">
      Blog Studio is waiting on you before it writes the article. Review this outline for
      <strong>${escapeHtml(displayUrl)}</strong>${round > 1 ? ` (revision ${escapeHtml(String(round))})` : ""}.
      Approve to continue, or decline with a reason and a new outline will be generated.
    </p>

    ${renderMetaTable([
      ["Project", escapeHtml(displayUrl)],
      ["Topic", `<strong>${escapeHtml(topic || "Untitled")}</strong>`],
      ["H1", h.h1 ? escapeHtml(h.h1) : null],
      ["Round", escapeHtml(String(round || 1))],
    ])}

    <div style="margin:0 0 28px;">
      ${sectionLabel("Outline")}
      <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:separate;border-spacing:0;border:1px solid ${T.border};border-radius:8px;overflow:hidden;">
        ${sectionRows || `<tr><td style="padding:16px 18px;color:${T.muted};">No sections yet.</td></tr>`}
      </table>
    </div>
    ...
    ${renderActionButtons({
      approveUrl,
      declineUrl,
      approveLabel: "Approve headings",
      declineLabel: "Decline headings",
      declineNote: "You will be asked for a brief reason",
    })}

    ${renderDashboardLine(`${baseUrl}/login`, "Prefer to watch the run in Studio?")}
    ${renderAutomatedNote("This is an automated headings review from RoboSEO.Ai Blog Studio. If you received it in error, contact your administrator.")}`;

  return renderEmailShell({
    title: "Headings approval required — RoboSEO.Ai",
    preheader: `Approve the outline for “${topic || "Untitled"}” on ${displayUrl}.`,
    badge: "Headings review",
    heading: "Review article outline",
    subheading: displayUrl,
    bodyHtml,
    baseUrl,
  });
...
export function generateSeoDigestEmail(siteSummaries = [], dashboardUrl = "") {
  ...
  const cta = dashboardUrl
    ? `<p style="margin:24px 0 0;text-align:center;">
         <a href="${escapeHtml(dashboardUrl)}" style="display:inline-block;background:#00A3FF;color:#fff;text-decoration:none;font-weight:700;font-size:14px;padding:12px 22px;border-radius:8px;">
      ...
      <table width="100%" style="max-width:640px;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
        <tr>
          <td style="background:#0f172a;padding:22px 28px;">
            <p style="margin:0;color:#86efac;font-size:11px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;">RoboSEO.Ai SEO Tool</p>
        ...
        <tr>
          <td style="background:#f8fafc;padding:16px 28px;border-top:1px solid #e2e8f0;text-align:center;">
            <p style="margin:0;color:#94a3b8;font-size:11px;">&copy; ${new Date().getFullYear()} RoboSEO.Ai</p>
          </td>
        </tr>
      </table>
    </td></tr>
