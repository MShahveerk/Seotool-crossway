export function generateApprovalNotificationEmail(approval, assignee, baseUrl, token) {
  ...
  const scheduledLabel = approval.scheduledFor
    ? (() => {
        const d = new Date(approval.scheduledFor);
        const pkt = d.toLocaleString("en-US", {
              ...
              <table class="btn-table" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 28px;">
                <tr>
                  ...
                  <td class="btn-cell" align="center" width="50%" style="padding-left: 8px;">
                    ...
                    <p style="margin: 8px 0 0; font-size: 11px; color: #64748b; text-align: center;">You will be asked for a brief reason</p>
                  </td>
                </tr>
              </table>
...
export function generateStatusChangeEmail(approval, actionUser, status, detail, baseUrl) {
  const imageUrl = approval.imagePath
    ? (approval.imagePath.startsWith('http') ? approval.imagePath : `${baseUrl}${approval.imagePath}`)
    : null;
  ...
  if (status === "approved") {
    ...
    statusDescription = `The post has been approved by <strong>${actionUser.name || "the assignee"}</strong>.`;
  } else if (status === "declined") {
    statusColor = "#dc2626"; // red
    statusDescription = `The post has been declined by <strong>${actionUser.name || "the assignee"}</strong>.`;
    if (detail && String(detail).trim()) {
      statusDescription += ` See the rejection reason below.`;
    }
  } else if (status === "edited") {
    statusColor = "#d97706"; // amber
    statusDescription = `The post has been edited/updated by <strong>${actionUser.name || "the assignee"}</strong>.`;
  } else if (status === "published") {
    statusColor = "#2563eb"; // blue
    statusDescription = "The post has been successfully published to the target social media accounts / website.";
  }
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Post Status Update: ${statusUpper}</title>
</head>
<body style="margin:0;padding:0;background-color:#f8fafc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f8fafc;padding:40px 0;">
    <tr>
        ...
        <table class="container" width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 8px 30px rgba(0,0,0,0.04);border:1px solid #e2e8f0;">
          ...
          <tr>
            <td style="background-color:#000000;padding:40px 40px;text-align:left;border-bottom:4px solid #1e293b;">
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td>
                    <div style="background-color:${statusColor};color:#ffffff;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:2px;padding:4px 12px;border-radius:4px;display:inline-block;margin-bottom:16px;">
                      POST ${statusUpper}
                    </div>
                  </td>
                </tr>
                <tr>
                  <td style="font-size:26px;font-weight:800;color:#ffffff;letter-spacing:-0.5px;line-height:1.2;">
                    ${approval.title}
                  </td>
                </tr>
                <tr>
          ...
          <tr>
            <td class="content" style="padding: 40px; background-color: #ffffff;">
              ...
              ${detail ? `
              <div style="margin-bottom: 28px; border-left: 4px solid ${statusColor}; background-color: #f8fafc; padding: 16px; border-radius: 0 8px 8px 0;">
                <p style="margin: 0 0 6px; font-size: 11px; font-weight: 800; text-transform: uppercase; color: #64748b; letter-spacing: 0.5px;">${status === "declined" ? "Rejection Reason" : "Change / Comments Detail"}</p>
                <div style="font-size: 14px; color: #1e293b; line-height: 1.6; white-space: pre-wrap;">${detail.replace(/^Rejection reason:\s*/i, "")}</div>
              </div>` : ''}
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse; background-color: #ffffff; border-radius: 8px; border: 1px solid #e2e8f0; overflow: hidden; margin-bottom: 28px;">
                ...
                <tr>
                  <td style="padding: 14px 18px; font-size: 11px; font-weight: 800; color: #64748b; border-bottom: 1px solid #e2e8f0; text-transform: uppercase; letter-spacing: 0.5px;">Status</td>
                  <td style="padding: 14px 18px; font-size: 13px; font-weight: 700; color: ${statusColor}; border-bottom: 1px solid #e2e8f0; text-transform: uppercase;">${status}</td>
                </tr>
                <tr style="background-color: #f8fafc;">
                  <td style="padding: 14px 18px; font-size: 11px; font-weight: 800; color: #64748b; text-transform: uppercase; letter-spacing: 0.5px;">User Info</td>
                  <td style="padding: 14px 18px; font-size: 13px; color: #334155;">${actionUser.name || "N/A"} (${actionUser.email || "N/A"})</td>
                </tr>
              </table>
              ...
              <div style="margin-bottom: 28px;">
                <p style="margin: 0 0 8px; font-size: 11px; font-weight: 800; text-transform: uppercase; color: #64748b; letter-spacing: 0.5px;">Post Caption</p>
                <div style="background-color: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; padding: 18px; color: #1e293b; font-size: 14px; line-height: 1.6; white-space: pre-wrap; font-family: inherit;">${previewCaption}</div>
              </div>` : ''}
              ...
              <div style="margin-bottom: 32px;">
                <p style="margin: 0 0 8px; font-size: 11px; font-weight: 800; text-transform: uppercase; color: #64748b; letter-spacing: 0.5px;">Attached Media</p>
                <div style="background-color: #fafafa; border-radius: 8px; border: 1px solid #e2e8f0; overflow: hidden; padding: 8px; text-align: center;">
                  <img src="${imageUrl}" alt="Post Media" style="max-width: 100%; max-height: 480px; width: auto; height: auto; border-radius: 6px; display: block; margin: 0 auto; object-fit: contain;" />
                </div>
              </div>` : ''}

              <hr style="margin: 36px 0; border: none; border-top: 1px solid #e2e8f0;">
              <p style="margin: 0; color: #94a3b8; font-size: 11px; line-height: 1.5; text-align: center;">
                This is an automated status update notification from Crossway SEO & Marketing Scheduler.
              </p>
            </td>
          </tr>
