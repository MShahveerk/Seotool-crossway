export function generateApprovalNotificationEmail(approval, assignee, baseUrl, token) {
  const approvalUrl = `${baseUrl.replace(/\/$/, "")}/api/approvals/quick-action?id=${approval.id}&token=${token}&action=approve`;
  const rejectUrl = `${baseUrl.replace(/\/$/, "")}/api/approvals/quick-action?id=${approval.id}&token=${token}&action=decline`;
  const dashboardUrl = `${baseUrl.replace(/\/$/, "")}/login`;

  const imageUrl = approval.imagePath
    ? (approval.imagePath.startsWith('http') ? approval.imagePath : `${baseUrl.replace(/\/$/, "")}${approval.imagePath.startsWith("/") ? approval.imagePath : `/${approval.imagePath}`}`)
    : null;
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center" style="padding-bottom: 16px;">
                    <!-- Crossway Logo -->
                    <img src="${baseUrl.replace(/\/$/, "")}/crossway-logo.png" alt="Crossway Logo" width="67" height="55" style="display: inline-block; border-radius: 8px; background-color: #ffffff; padding: 4px; object-fit: contain;" />
                  </td>
                </tr>
                <tr>
                  <td align="center">
