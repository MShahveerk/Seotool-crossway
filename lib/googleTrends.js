 ...
 * The Decider always uses the keyword library (Research or an uploaded bank).
 * Trends are an extra lane: ongoing world queries that are still relevant to
 * this brand. Trends never replace the library.
 */
...
/**
 * Relevant world trends for this brand — never a replacement for the keyword library.
 */
...
function mergeLibraryAndTrends(library, trends) {
  ...
  for (const c of lib) {
    ...
    candidates.push({
      ...c,
      lane: hook ? "library×trend" : "library",
      trendHook: hook?.query || null,
      score: Number(c.score || 0) + (hook ? 50000 : 0),
    });
  }
  ...
  for (const t of world) {
    ...
    trendHooks.push({
      ...t,
      id: String(t.id || key).startsWith("trend-") ? t.id : `trend-${t.id || key}`.slice(0, 56),
      lane: "world",
    });
  }
  ...
  return {
    ...
    source: trendHooks.length || world.length ? "library+trends" : library?.source || "harvest",
    candidates: candidates.slice(0, 28),
    trendHooks: trendHooks.slice(0, 12),
...
/**
 * Keyword library is always the seed list. Trends (when SerpAPI is ready) are
 * relevant world hooks layered on top — never a replacement.
 */
