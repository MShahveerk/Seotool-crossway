...
/**
 * True when a real SERP key is configured (not DuckDuckGo HTML). Blog Studio
 * uses this so a DDG timeout cannot fail a draft. Link hunt still uses
 * isOrganicSearchReady() and may fall through to DDG.
 */
export async function hasConfiguredOrganicSearch() {
  const creds = await resolveSearchCredentials();
  return Boolean(
    creds.serpApiKey ||
      (creds.googleCseKey && creds.googleCseCx) ||
      creds.braveSearchKey
  );
}
...
export async function shouldCheckGoogleDuplicates() {
  ...
  return hasConfiguredOrganicSearch();
}
