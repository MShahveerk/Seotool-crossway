...
function formatAdsApiError(status, data, text) {
  ...
  if (status === 403 || status === 401 || raw.includes("PERMISSION") || raw.includes("NOT_ADS_USER")) {
    return (
      `${base} — The Service Account "${sa || "your-service-account"}" does not have access to Google Ads Customer ID "${process.env.GOOGLE_ADS_CUSTOMER_ID}". ` +
      `Resolution: (1) Go to your Google Ads Account → Admin → Access and security → Users. ` +
      `(2) Click (+) and invite "${sa || "your-service-account-email"}" as Admin. ` +
      `(3) If this account is under an MCC Manager, also set GOOGLE_ADS_LOGIN_CUSTOMER_ID="<mcc_manager_id>" in .env.`
    );
  }
