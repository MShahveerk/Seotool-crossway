...
import {
  SEO_DATA_NOT_CONFIGURED,
  SEO_API_ERROR,
  SEO_CREDIT_BUDGET_EXHAUSTED,
  SEO_RATE_LIMIT,
} from "../seoDataMessages.js";
import { canSpendCredits, recordCreditSpend, logBlockedAttempt } from "./credits.js";
import { scheduleSerankingCall, fetchSerankingWithRetry, isSerankingRateLimit } from "./requestQueue.js";
...
function getApiKey() {
  ...
  if (!key) throw new SerankingApiError(SEO_DATA_NOT_CONFIGURED, { status: 503 });
  return key;
}
...
export async function serankingRequest({
  ...
  if (!isSerankingConfigured()) {
    throw new SerankingApiError(SEO_DATA_NOT_CONFIGURED, { status: 503 });
  }
  ...
  if (!skipBudget && estimate > 0) {
    ...
    if (!gate.ok) {
      ...
      throw new SerankingApiError(gate.reason || SEO_CREDIT_BUDGET_EXHAUSTED, {
        status: 402,
        code: "CREDIT_BUDGET",
      });
    }
  ...
  if (!q.has("apikey")) q.set("apikey", apiKey);
  ...
  return scheduleSerankingCall(async () => {
    const { res, text } = await fetchSerankingWithRetry(url, init);
    let data = null;
    try {
    ...
    if (!res.ok) {
      const msg =
        ...
        data?.error?.message ||
        data?.message ||
        (typeof data === "string" ? data : `${SEO_API_ERROR} (${res.status})`);
      const friendly =
        res.status === 429 || isSerankingRateLimit(res.status, data)
          ? SEO_RATE_LIMIT
          : String(msg).slice(0, 500);
      throw new SerankingApiError(friendly, { status: res.status, body: data });
    }
