...
/** Individual backlinks (source page URL + anchor text) pointing at a domain. */
export async function fetchBacklinksList(cacheSiteUrl, domain, { allowManual = false, force = false, limit = 25 } = {}) {
  ...
  const { target, mode } = resolveBacklinkTarget(cacheSiteUrl, domain);
  const data = await serankingRequest({
    method: "GET",
    path: "/backlinks/backlinks",
    query: { target, mode, limit: String(lim), order_by: "domain_inlink_rank", output: "json" },
    creditEstimate: lim,
    creditsOnSuccess: lim,
    siteUrl: cacheSiteUrl,
    allowManual,
    endpointLabel: "backlinks/backlinks",
  });
