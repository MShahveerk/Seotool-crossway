...
function extractRefdomainHosts(payload) {
  if (Array.isArray(payload?.hosts) && payload.hosts.every((h) => typeof h === "string")) {
    return [...new Set(payload.hosts.map((h) => String(h).toLowerCase().replace(/^www\./, "")).filter(Boolean))];
  }
...
/** Cached list of referring domains (for Autopilot pitch de-dupe). */
export async function fetchBacklinksRefdomains(
  cacheSiteUrl,
  domain,
  { allowManual = false, force = false, siteUrlForApi, limit = 100 } = {}
) {
  const apiSiteUrl = siteUrlForApi || cacheSiteUrl;
  const lim = Math.min(200, Math.max(10, Number(limit) || 100));
  const cached = await getCachedSnapshot(cacheSiteUrl, DATA_TYPES.BACKLINKS_REFDOMAINS);
  if (!force && cached?.payload && !cached.expired) {
    const hosts = extractRefdomainHosts(cached.payload);
    if (hosts.length) {
      return {
        data: cached.payload,
        hosts,
  ...
  const { target, mode } = resolveBacklinkTarget(apiSiteUrl, domain);
  const data = await serankingRequest({
    method: "GET",
    path: "/backlinks/refdomains",
    query: {
      target,
      mode,
      limit: String(lim),
    ...
    creditEstimate: lim,
    creditsOnSuccess: lim,
    siteUrl: apiSiteUrl,
    allowManual,
    endpointLabel: "backlinks/refdomains",
  });

  const hosts = extractRefdomainHosts(data);
  await saveSnapshot({
    siteUrl: cacheSiteUrl,
    dataType: DATA_TYPES.BACKLINKS_REFDOMAINS,
    payload: { hosts, raw: data, limit: lim },
    creditsSpent: Math.min(lim, hosts.length || lim),
  });
...
/** Individual backlinks (source page URL + anchor text) pointing at a domain — /backlinks/all. */
export async function fetchBacklinksList(
  cacheSiteUrl,
  domain,
  { allowManual = false, force = false, limit = 25, siteUrlForApi, urlFromFilter = "" } = {}
) {
  const lim = Math.min(100, Math.max(10, Number(limit) || 25));
  const fromFilter = String(urlFromFilter || "").trim();
  // A TLD-filtered list must not read or write the unfiltered snapshot.
  if (!fromFilter) {
    const cached = await getCachedSnapshot(cacheSiteUrl, DATA_TYPES.BACKLINKS_LIST);
    if (!force && cached?.payload && !cached.expired && Array.isArray(cached.payload?.backlinks) && cached.payload.backlinks.length) {
      return { data: cached.payload, fromCache: true, fetchedAt: cached.fetchedAt, creditsSpent: 0 };
    }
  }

  const { target, mode } = resolveBacklinkTarget(siteUrlForApi || cacheSiteUrl, domain);
  const query = { target, mode, limit: String(lim), per_domain: "1", order_by: "domain_inlink_rank", output: "json" };
  if (fromFilter) {
    query.url_from_filter = fromFilter;
    query.url_from_filter_mode = "contains";
  }
  const data = await serankingRequest({
    method: "GET",
    path: "/backlinks/all",
    ...
    siteUrl: cacheSiteUrl,
    allowManual,
    endpointLabel: "backlinks/all",
  });

  if (!fromFilter) {
    await saveSnapshot({ siteUrl: cacheSiteUrl, dataType: DATA_TYPES.BACKLINKS_LIST, payload: data, creditsSpent: lim });
  }
  return { data, fromCache: false, creditsSpent: lim };
}
