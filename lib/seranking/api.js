...
export async function fetchBacklinksList(cacheSiteUrl, domain, { allowManual = false, force = false, limit = 25, siteUrlForApi } = {}) {
  ...
  const data = await serankingRequest({
    method: "GET",
    path: "/backlinks/all",
    query: { target, mode, limit: String(lim), per_domain: "1", order_by: "domain_inlink_rank", output: "json" },
    creditEstimate: lim,
    creditsOnSuccess: lim,
    siteUrl: cacheSiteUrl,
    allowManual,
