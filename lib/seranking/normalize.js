...
export function normalizeKeywordResearchRow(row, source) {
  ...
  normalized.trafficPotential = computeTrafficPotential({
    volume: normalized.volume,
    difficulty: normalized.difficulty,
    existingPosition: normalized.position,
  });
...
export function extractBacklinksSummary(payload) {
  ...
  const row = payload.summary?.[0] || (Array.isArray(payload.summary) ? null : payload);
...
export function normalizeDomainKeywordsList(raw) {
...
export function normalizeAuditReport(raw) {
  ...
  const sections = (report.sections || []).map((sec) => {
    ...
    return {
      uid: sec.uid,
      name: sec.name || sec.title || sec.uid,
      checks,
    };
  });
