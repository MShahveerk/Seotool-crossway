...
import { DEFAULT_SOURCE, DATA_TYPES, TTL_DAYS } from "./config.js";
import {
...
import { buildExplorerCacheMeta } from "./explorerCache.js";
import {
  getLatestAuditJob,
  createAuditJob,
  updateAuditJob,
...
export const SERANKING_SCHEDULES = [
  { id: "seranking", label: "Live SEO snapshots", cron: "45 4 * * *", when: "Daily 04:45", note: "Backlinks, domain, keywords, seeds, audit rotation (~600 cr/day cap)" },
  { id: "authority", label: "Domain authority (Open PageRank)", cron: "30 4 * * *", when: "Daily 04:30" },
  { id: "site-explorer", label: "Common Crawl site explorer", cron: "0 5 * * 1", when: "Mondays 05:00", note: "7-day cache; includes explored domains" },
  { id: "seranking-explorer", label: "Live site explorer", cron: "30 5 * * 1", when: "Mondays 05:30", note: "7-day cache; overview + pages + backlinks" },
  ...
  { id: "client-reports", label: "Monthly reports (weekly send)", cron: "0 7 * * 1", when: "Mondays 07:00" },
...
export async function ensureSerankingSnapshot(
  ...
  const expiresAt = new Date(fetchedAt.getTime() + ttl * 24 * 60 * 60 * 1000);
  return {
    data: result.data,
    fromCache: Boolean(result.fromCache),
    creditsSpent: result.creditsSpent ?? 0,
    fetchedAt: result.fetchedAt || fetchedAt.toISOString(),
...
export async function loadReferringDomains(
  siteUrl,
  { allowManual = true, force = false, limit = 100, target } = {}
) {
  ...
  const { fetchBacklinksRefdomains } = await import("./api.js");
  const result = await fetchBacklinksRefdomains(cacheSite, domain, {
    allowManual,
    force,
    siteUrlForApi,
    limit,
  ...
  const hosts = Array.isArray(result.hosts)
    ? result.hosts
    : Array.isArray(result.data?.hosts)
      ? result.data.hosts
      : [];
  return {
    siteUrl: cacheSite,
...
async function resolveExplorerAudit(cacheSite, domain, { allowManual, force, autostartAudit }) {
  await abandonInFlightAuditJobs(cacheSite, {
    force: Boolean(force),
    reason: force
      ? "Abandoned — user forced a new audit."
      : "Abandoned — audit job timed out (stale).",
  });
...
export async function loadExplorer(
  ...
  const overviewRes = await ensureSerankingSnapshot(
    cacheSite,
  ...
  const pagesRes = await ensureSerankingSnapshot(
    cacheSite,
  ...
  if (!force) {
    ...
    if (cachedBl?.payload && !cachedBl.expired && isBacklinksPayloadUsable(cachedBl.payload)) {
      backlinksRes = {
        data: cachedBl.payload,
        fromCache: true,
        fetchedAt: cachedBl.fetchedAt,
        expiresAt: cachedBl.expiresAt,
        creditsSpent: 0,
      };
    }
  }
  ...
  const cache = buildExplorerCacheMeta([overviewRes, pagesRes, backlinksRes]);
  ...
  return {
    ...
    cacheSchedule: cache.cacheSchedule,
  };
}
