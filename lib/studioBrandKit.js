...
import { chatCompletion, generateImage, defaultImageModelForProvider, imageModelForReferences } from "./blogStudio/providers.js";
...
/**
 * Always appended when a brand logo is attached — forces the image model to place
 * our logo neatly regardless of site visual guidelines / mode.
 */
export const HARDCODED_LOGO_PROMPT_BLOCK = [
  "BRAND LOGO (required — a logo reference image is attached):",
  "- Place the provided brand logo neatly in the preferred corner of the image.",
  "- Logo must be sharp, upright, correctly proportioned, high contrast, and fully readable — never warped, stretched, cropped mid-mark, blurred, or tiny.",
  "- Reproduce the attached logo faithfully; do not invent a different mark or redraw lettering incorrectly.",
  "- Keep clear padding from edges and do not cover the main subject of the photo.",
  "- No extra watermarks, stock logos, or unrelated brand marks.",
].join("\n");
...
export function aiBrandPromptBlock(kitRaw) {
  ...
  return [
    ...
    "Keep the photo subject clean inside the matte.",
...
/** Hardcoded logo placement rules + optional brand notes whenever kit has a logo. */
export function logoPlacementPromptBlock(kitRaw) {
  const kit = normalizeBrandKit(kitRaw);
  if (!kit.enabled || !kit.logoPath) return "";
  const corner = String(kit.logoCorner || "bottom-right").replace(/-/g, " ");
  return [
    HARDCODED_LOGO_PROMPT_BLOCK,
    `Preferred logo corner: ${corner}.`,
...
/** Combined brand prompt for image agents (chrome + always-on logo rules). */
export function brandImagePromptBlock(kitRaw) {
  const kit = normalizeBrandKit(kitRaw);
  if (!kit.enabled) return "";
  return [aiBrandPromptBlock(kit), logoPlacementPromptBlock(kit)].filter(Boolean).join("\n\n");
}
...
function resolveImageGenSettings(siteConfig = {}) {
  ...
  if (!imageModel) imageModel = defaultImageModelForProvider(imageProvider);
  imageModel = imageModelForReferences(imageProvider, imageModel);
