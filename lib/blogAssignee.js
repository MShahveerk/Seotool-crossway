...
export async function findAssigneesForSite(selectedSite) {
  ...
  try {
    ...
    for (const approver of approvers) {
      if (approver.email && !notified.has(approver.email)) {
        await sendBlogApprovalNotification(approver.email, blog, approver, token);
    ...
    for (const admin of superAdmins) {
      if (admin.email && !notified.has(admin.email)) {
        await sendBlogApprovalNotification(admin.email, blog, admin, token);
        notified.add(admin.email);
      }
    }
    if (creator?.email && !notified.has(creator.email)) {
      await sendBlogApprovalNotification(creator.email, blog, creator, token);
    }
  } catch (err) {
