...
export async function buildAiKeywordResearch({
  ...
  const serankingPromise = isSerankingConfigured()
    ? fetchSerankingMetricsMap(allKeywordStrings, geo, siteUrl, { allowManual: true }).catch((error) => {
        console.warn("AI keyword research: SE Ranking enrichment failed:", error.message);
        return { metricsMap: new Map(), creditsSpent: 0, fromCache: false, configured: true };
      })
    : Promise.resolve({ metricsMap: new Map(), creditsSpent: 0, configured: false });
  ...
  const seedVolumeByCountry = siteUrl
    ? await loadVolumeByCountry(siteUrl, trimmedSeed, {
  ...
  const seedEnriched = enrichRow({
    ...
    seranking: buildSerankingMetrics(seedSerankingRow, {
      volumeByCountry: seedVolumeByCountry,
  ...
  const withSeranking = [...keywordRows, seedEnriched].filter((r) => r.seranking?.available).length;
