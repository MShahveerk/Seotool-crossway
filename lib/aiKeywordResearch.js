...
import { fetchSerankingMetricsMap, RESEARCH_CACHE_SITE } from "./seranking/keywordMetrics.js";
const INTENT_COLORS = {
  informational: "sky",
  commercial: "amber",
...
function serankingRowToMetrics(row) {
  if (!row || row.volume == null) return null;
  const cpc =
    row.cpcFormatted ||
    (row.cpc != null && Number.isFinite(Number(row.cpc)) ? `$${Number(row.cpc).toFixed(2)}` : null);
  return {
    avgMonthlySearches: row.volume,
    competition: row.competitionLevel || null,
    competitionIndex: row.difficulty != null ? Number(row.difficulty) : null,
    lowTopOfPageBid: cpc,
    highTopOfPageBid: cpc,
    monthlyTrend: row.monthlyTrend?.length ? row.monthlyTrend : [],
  };
}
...
function mergePlannerMetrics(keyword, metricsMap, aiRow = {}, autocompleteRow = {}, serankingMap = null) {
  const key = normKeyword(keyword);
  const planner = metricsMap.get(key) || null;
  const serankingRaw = serankingMap?.get(key) || null;
  const seranking = serankingRowToMetrics(serankingRaw);
  const hasPlanner = Boolean(planner?.avgMonthlySearches != null || planner?.competitionIndex != null);
  const hasSeranking = Boolean(seranking?.avgMonthlySearches != null);
  ...
  else if (hasSeranking) metricsSource = "se_ranking";
  else if (aiRow.volumeEstimate != null) metricsSource = "ai_estimate";

  const avgMonthlySearches =
    planner?.avgMonthlySearches ?? seranking?.avgMonthlySearches ?? aiRow.volumeEstimate ?? null;
  const competitionIndex =
    planner?.competitionIndex ?? seranking?.competitionIndex ?? aiRow.difficultyEstimate ?? null;
  const monthlyTrend =
    planner?.monthlyTrend?.length ? planner.monthlyTrend : seranking?.monthlyTrend?.length ? seranking.monthlyTrend : aiTrend;
  ...
  const merged = {
    ...
    difficultyEstimate: aiRow.difficultyEstimate ?? seranking?.competitionIndex ?? null,
    volumeEstimate: aiRow.volumeEstimate ?? seranking?.avgMonthlySearches ?? null,
    avgMonthlySearches,
    competition: planner?.competition ?? seranking?.competition ?? null,
    competitionIndex,
    lowTopOfPageBid: planner?.lowTopOfPageBid ?? seranking?.lowTopOfPageBid ?? aiCpc.low,
    highTopOfPageBid: planner?.highTopOfPageBid ?? seranking?.highTopOfPageBid ?? aiCpc.high,
    monthlyTrend,
...
{"seedAnalysis":{"summary":"2 sentences max","intent":"informational|commercial|transactional","parentTopic":"...","volumeEstimate":1200,"difficultyEstimate":45,"cpcLow":1.2,"cpcHigh":3.5,"trendDirection":"rising|stable|declining","monthlyVolumeTrend":[{"month":"JANUARY","year":2025,"searches":1100},{"month":"FEBRUARY","year":2025,"searches":1200}],"contentAngles":["angle1","angle2","angle3"],"serpFeatures":["featured snippet|PAA|video|local pack"]},"clusters":[{"name":"cluster theme","keywords":["kw1","kw2"],"contentType":"guide|comparison|landing|faq","priority":"high|medium|low","recommendation":"one line"}],"keywords":[{"keyword":"exact phrase from list","intent":"informational|commercial|transactional|navigational","type":"question|long_tail|commercial|related","relevanceScore":85,"volumeEstimate":800,"difficultyEstimate":38,"cpcLow":0.5,"cpcHigh":2.1,"contentAngle":"specific page angle","recommendation":"actionable SEO step","funnelStage":"awareness|consideration|decision","priorityTier":"quick_win|growth|strategic","cluster":"cluster name"}],"gaps":[{"keyword":"optional gap","reason":"why"}]}

REQUIRED: seedAnalysis MUST include volumeEstimate, difficultyEstimate (0-100), cpcLow, cpcHigh, and monthlyVolumeTrend (6-12 months) for "${seed}". Each keyword MUST include volumeEstimate and difficultyEstimate. Use realistic market estimates for ${geoLabel} when exact data unavailable. Be specific to the niche. No generic advice. relevanceScore 0-100.`;
}
...
Return ONE JSON: {"seedAnalysis":{"keyword":"${seed}","intent":"informational","summary":"brief","volumeEstimate":1000,"difficultyEstimate":50,"cpcLow":1,"cpcHigh":3,"monthlyVolumeTrend":[{"month":"JANUARY","year":2025,"searches":900}],"contentAngles":["a","b"]},"keywords":[{"keyword":"phrase","intent":"informational","type":"related","relevanceScore":80,"volumeEstimate":500,"difficultyEstimate":40}]}
Include 18-22 keywords with volumeEstimate and difficultyEstimate on each. Seed must have full metrics.`;
}
...
export async function buildAiKeywordResearch({
  ...
  let serankingMap = new Map();
  let serankingCredits = 0;
  let serankingError = null;
  const seedHasPlanner = Boolean(metricsMap.get(normKeyword(trimmedSeed))?.avgMonthlySearches != null);
  const plannerCoverage =
    allKeywordStrings.filter((k) => metricsMap.get(normKeyword(k))?.avgMonthlySearches != null).length /
    Math.max(1, allKeywordStrings.length);

  if (isSerankingConfigured() && (!seedHasPlanner || plannerCoverage < 0.35)) {
    try {
      const sr = await fetchSerankingMetricsMap(allKeywordStrings.slice(0, 50), geo, RESEARCH_CACHE_SITE, {
        allowManual: true,
        seedKeyword: trimmedSeed,
      });
      serankingMap = sr.metricsMap || new Map();
      serankingCredits = sr.creditsSpent || 0;
    } catch (error) {
      serankingError = error.message;
      console.warn("AI keyword research: SE Ranking metrics fallback failed:", error.message);
    }
  }
  ...
  const seedMerged = mergePlannerMetrics(trimmedSeed, metricsMap, seedAi, seedAutocomplete, serankingMap);
  ...
  const keywordRows = mergedRows
    .map((row) => {
      ...
      const merged = mergePlannerMetrics(row.keyword, metricsMap, row, ac, serankingMap);
      return merged;
    })
  ...
  const withSeranking = keywordRows.filter((r) => r.metricsSource === "se_ranking").length;
  ...
  const payload = {
    ...
    seranking: {
      ...
      used: serankingMap.size > 0,
      enrichedCount: withSeranking + (seedEnriched.metricsSource === "se_ranking" ? 1 : 0),
      creditsSpent: serankingCredits,
      error: serankingError,
    },
    metrics: {
      seedSource: seedEnriched.metricsSource,
      withAiEstimates: withAiMetrics,
      withSeranking,
    },
    ai: {
