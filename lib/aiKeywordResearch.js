...
function buildSiteBriefPrompt({ siteHost, geoLabel, rows, summary, strikingDistance }) {
  const snapshot = rows.slice(0, 30).map((r) => ({
    query: r.query,
    position: r.position,
    clicks: r.clicks,
    ...
    tags: r.tags,
    page: r.page ? String(r.page).replace(/^https?:\/\/[^/]+/, "") : null,
  }));

  return `You are a senior SEO strategist analyzing a client's keyword portfolio (like Ahrefs + Google Search Console).

Website: ${siteHost || "unknown"}
Market: ${geoLabel}

Summary stats:
- Total queries tracked: ${summary.total ?? 0}
- With volume data: ${summary.withPlannerData ?? 0}
- Worth fighting for (pos 8-15): ${summary.worthFighting ?? 0}
- Hidden gems: ${summary.hiddenGems ?? 0}
- CTR fixes needed: ${summary.ctrFixes ?? 0}
- Striking distance opportunities: ${(strikingDistance || []).length}
...
{
  "overview": "3-4 sentence executive summary of this site's keyword health and biggest opportunity",
  "topPriorities": [
    { "query": "...", "action": "specific SEO action", "reason": "why now", "impact": "high|medium|low" }
  ],
  "contentClusters": [
    { "theme": "topic cluster name", "keywords": ["kw1", "kw2"], "recommendation": "content strategy for this cluster" }
  ],
  "gaps": [
    { "topic": "missing topic", "reason": "why pursue it" }
  ],
  "quickWins": ["actionable win 1", "actionable win 2", "actionable win 3"]
}

Rules:
- topPriorities: 5-8 items referencing real queries from the data
- contentClusters: 3-5 thematic groups from their existing keywords
- gaps: 3-5 new topics they should explore (not already in top data)
...
export async function buildAiSiteKeywordBrief({
  ...
  const { data: aiPayload, provider: usedProvider, model } = await chatCompletionJson(
    [
      {
        role: "system",
        content: "You are an expert SEO consultant. Respond with strict JSON only. Base advice on the provided data.",
      },
      {
        role: "user",
  ...
  return {
    brief: {
      overview: aiPayload?.overview || "",
      topPriorities: Array.isArray(aiPayload?.topPriorities) ? aiPayload.topPriorities : [],
      contentClusters: Array.isArray(aiPayload?.contentClusters) ? aiPayload.contentClusters : [],
      gaps: Array.isArray(aiPayload?.gaps) ? aiPayload.gaps : [],
      quickWins: Array.isArray(aiPayload?.quickWins) ? aiPayload.quickWins : [],
    },
    ai: {
      provider: usedProvider,
      model,
