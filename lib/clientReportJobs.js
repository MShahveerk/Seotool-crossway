/**
 * Client report delivery — delegates to landscape slide-deck send jobs.
...
export async function runWeeklyClientReports(logger = console) {
  logger.info?.("Client reports: using slide-deck send pipeline…");
  const result = await sendClientReportsNow({ trigger: "cron" });
  logger.info?.(`Client reports: ${(result.results || []).filter((r) => r.ok).length} sent.`);
  return result;
}
