...
import { collectExistingLinkHosts } from "./existingLinkHosts.js";
...
export async function buildAutopilotContext(siteLink, config = {}) {
  ...
  /* No internal-crawl fallback for `pack.audit`. Autopilot's scorecard and briefs
     are shown to people and quoted in reports, so the audit number here has to be
     the same SE Ranking one they'd see on the dashboard — an absent audit is
     better than a second, quietly different score. */
  ...
  try {
    const existing = await collectExistingLinkHosts(site);
    pack.existingLinkHosts = existing.hosts || [];
    pack.existingLinkHostSources = existing.sources || null;
  } catch (err) {
    pack.errors.push(`existing-linkers: ${err.message}`);
  }

  const blockedHostsPreview = (pack.existingLinkHosts || []).slice(0, 80);
  ...
  return {
    ...
    contextText: [
      ...
      blockedHostsPreview.length
        ? [
            "DO NOT PITCH / DO NOT SUGGEST these domains — we already have a backlink from them, or already created a pitch to them:",
            blockedHostsPreview.join(", "),
            `(${pack.existingLinkHosts.length} blocked host(s) total)`,
          ].join("\n")
        : "Existing linker blocklist: none loaded yet (still prefer fresh new domains).",
    ]
      .filter(Boolean)
      .join("\n\n"),
  };
