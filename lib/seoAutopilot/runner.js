  ...
  try {
    ...
    for (let i = 0; i < defs.length; i++) {
      ...
      try {
        ...
        if (
          (stage.agentId === "pitcher" || stage.agentId === "foundation") &&
          Array.isArray(stage.data?.skippedExistingLinkers) === false
        ) {
          /* no-op */
        }
        if (stage.agentId === "pitcher" && Array.isArray(stage.data?.pitches)) {
          for (const p of stage.data.pitches) {
            const h = String(p.targetUrl || "")
              .replace(/^https?:\/\//i, "")
              .split("/")[0]
              .replace(/^www\./, "")
              .toLowerCase();
            if (h && Array.isArray(context.existingLinkHosts)) {
              context.existingLinkHosts.push(h);
            }
          }
        }
        if (stage.agentId === "foundation" && Array.isArray(stage.data?.links)) {
          for (const l of stage.data.links) {
            const h = String(l.url || "")
              .replace(/^https?:\/\//i, "")
              .split("/")[0]
              .replace(/^www\./, "")
              .toLowerCase();
