...
async function runOneAgent(agentDef, config, contextText, priorStages = [], signal = null) {
...
/** In-process abort handles so Cancel kills the active provider HTTP call immediately. */
const autopilotAbortControllers = new Map();

function abortAutopilotExecution(runId) {
  const ctrl = autopilotAbortControllers.get(runId);
...
/** Only write while the run is still live — never resurrect a cancelled run. */
async function patchLiveAutopilotRun(runId, data) {
  return prisma.seoAutopilotRun.updateMany({
    where: {
      id: runId,
...
function cancelRemainingStages(stages, fromIndex = 0) {
  for (let j = fromIndex; j < stages.length; j += 1) {
...
async function finalizeAutopilotCancelled(runId, { stages = null, scorecard = null, totalCost = null } = {}) {
  abortAutopilotExecution(runId);
  const existing = await prisma.seoAutopilotRun.findUnique({ where: { id: runId } });
  if (!existing) return null;
  if (existing.status === "cancelled" && existing.finishedAt && stages == null) {
    return existing;
  }
  const nextStages = Array.isArray(stages)
    ? cancelRemainingStages([...stages], 0)
    : cancelRemainingStages(
        Array.isArray(existing.stagesJson) ? [...existing.stagesJson] : [],
        0
      );
  return prisma.seoAutopilotRun.update({
    ...
    data: {
      ...
      ...(scorecard !== null ? { scorecardJson: scorecard } : {}),
      ...(totalCost != null ? { totalCostUsd: totalCost } : {}),
    },
  });
}

/**
 * Fully cancel a queued/running Autopilot run immediately.
 * Aborts any in-flight provider call and marks status=cancelled (not a soft request).
 */
export async function cancelAutopilotRun(runId) {
  ...
  if (["completed", "failed", "cancelled"].includes(run.status)) {
    ...
    return finalizeAutopilotCancelled(runId);
  }
  return finalizeAutopilotCancelled(runId);
}
...
export async function cancelActiveAutopilotRunsForSite(siteLink) {
  ...
  for (const row of active) {
    cancelled.push(await cancelAutopilotRun(row.id));
  }
...
export async function executeAutopilotRun(runId) {
  ...
  if (run.cancelRequested || run.status === "cancelled") {
    await finalizeAutopilotCancelled(runId);
    return null;
  }

  const claimed = await patchLiveAutopilotRun(runId, {
    status: "running",
    startedAt: new Date(),
    errorMessage: null,
  });
  ...
  autopilotAbortControllers.set(runId, abortController);
  ...
  try {
    ...
    const stageWrite = await patchLiveAutopilotRun(runId, {
      stagesJson: stages,
      totalCostUsd: 0,
    });
    if (!stageWrite.count) {
      await finalizeAutopilotCancelled(runId, { stages, totalCost: 0 });
      return;
    }

    for (let i = 0; i < defs.length; i++) {
      const def = defs[i];
      if (await isAutopilotCancelled(runId) || abortController.signal.aborted) {
        await finalizeAutopilotCancelled(runId, { stages, scorecard, totalCost });
        return;
      }
      ...
      const runningWrite = await patchLiveAutopilotRun(runId, {
        stagesJson: stages,
        totalCostUsd: totalCost,
      });
      if (!runningWrite.count) {
        await finalizeAutopilotCancelled(runId, { stages, scorecard, totalCost });
        return;
      }

      try {
        const stage = await runOneAgent(
          def,
          config,
          context.contextText,
          stages,
        ...
        stages[i] = {
          ...stages[i],
          ...stage,
          status: stage.status || (stage.ok ? "succeeded" : "failed"),
        };
        ...
        if (err?.cancelled || abortController.signal.aborted || (await isAutopilotCancelled(runId))) {
          stages[i] = {
            ...stages[i],
            agentId: def.id,
            title: def.title,
      ...
      const progressWrite = await patchLiveAutopilotRun(runId, {
        stagesJson: stages,
        scorecardJson: scorecard,
        totalCostUsd: totalCost,
      });
      if (!progressWrite.count) {
        await finalizeAutopilotCancelled(runId, { stages, scorecard, totalCost });
        return;
      }
    }
    ...
    const finished = await patchLiveAutopilotRun(runId, {
      status: "completed",
      stagesJson: stages,
      scorecardJson: scorecard,
    ...
    await patchLiveAutopilotRun(runId, {
      status: "failed",
      errorMessage: err.message || "Run failed",
      finishedAt: new Date(),
    });
  } finally {
    autopilotAbortControllers.delete(runId);
  }
}
