...
export async function publishViaWebhook(payload, config) {
  const url = String(config.webhookUrl || "").trim();
  if (!url) {
    const err = new Error("Webhook URL is not configured for this site.");
    err.skippable = true;
    throw err;
  }
  ...
  const headers = {
    ...
    "User-Agent": "Crossway-Blog-Publisher/1.0",
  };
