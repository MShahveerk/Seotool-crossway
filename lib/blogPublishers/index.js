...
export async function publishBlogPost(blog, options = {}) {
  ...
  if (forcePublish) {
    ...
    // Prefer the intended schedule when already due so WP public time matches Crossway.
    const scheduled = blog.scheduledFor ? new Date(blog.scheduledFor) : null;
    const due =
      scheduled && !Number.isNaN(scheduled.getTime()) && scheduled.getTime() <= Date.now() + 90_000;
    payload.date = due ? scheduled.toISOString() : new Date().toISOString();
  }
    ...
    try {
      const result =
        method === "wordpress"
          ...
          : await handler(payload, config, blog);
      // For WordPress live publishes, publisher must verify status=publish via GET.
      if (method === "wordpress" && (options.mode || "publish") === "publish" && result.verified !== true) {
        const msg = `${method}: publish response was not verified as live on WordPress`;
        errors.push(msg);
        await logAttempt(blog.id, method, false, result.externalId, msg);
