...
export async function pullWordpressDraftsForSite(siteLink, opts = {}) {
  ...
  for (const wpPost of wpPosts) {
    ...
    if (existing) {
      ...
      // Blogs imported before notifications existed (or whose emails failed)
      // still need the approval email — send it once.
      if (updatedBlog.status === "pending" && !updatedBlog.approvalNotifiedAt) {
...
export async function runWordpressPullForAllSites(logger = console) {
  ...
  for (const config of configs) {
    try {
      // Cron imports scheduled posts only (future status or drafts with a future date).
      const result = await pullWordpressDraftsForSite(config.siteLink, { force: true, onlyScheduled: true });
