...
export async function pullWordpressDraftsForSite(siteLink, opts = {}) {
  ...
  let { posts: wpPosts, statusCounts, scheduledDraftCount, statusFilterBroken } = await fetchWordpressPosts(config, {
    statuses,
      ...
      wpPosts.length === 0
        ? [
            ...
            statusFilterBroken
              ...
              : "Confirm the post is a standard blog post (not a page) with status draft, future, or pending in wp-admin.",
          ].join(" ")
        : [
