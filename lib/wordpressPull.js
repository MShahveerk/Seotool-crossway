...
export async function pullWordpressDraftsForSite(siteLink, opts = {}) {
  ...
  for (const wpPost of wpPosts) {
    ...
    // Approved blogs: never let a WP pull overwrite Crossway's schedule.
    let scheduledFor;
    if (existing?.status === "approved" && existing.scheduledFor) {
      scheduledFor = existing.scheduledFor;
    } else if (defaultSchedule) {
      scheduledFor = existing?.scheduledFor || defaultSchedule;
    } else {
      scheduledFor = canonical.scheduledFor ?? existing?.scheduledFor ?? null;
    }
