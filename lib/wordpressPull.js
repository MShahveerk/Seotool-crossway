  ...
  for (const wpPost of wpPosts) {
    ...
    if (!featuredImagePath && (canonical.featuredImageUrl || featured.id)) {
      try {
        ...
        featuredImagePath = canonical.featuredImageUrl || null;
      }
    }
    ...
    if (existing) {
      ...
      if (scheduledFor && canonical.externalId && (defaultSchedule || opts.syncScheduleToWordpress)) {
        try {
          ...
          await syncWordpressSchedule(config, canonical.externalId, scheduledFor, { asDraft: false });
        } catch (syncErr) {
          console.warn(
    ...
    const blog = await prisma.blogPost.create({
      data: {
        ...
        excerpt: canonical.excerpt,
        content: canonical.content,
        wpStatus: defaultSchedule ? "future" : canonical.wpStatus,
        featuredImagePath,
    ...
    // Reflect Crossway-assigned schedule back onto WordPress so WP matches the queue.
