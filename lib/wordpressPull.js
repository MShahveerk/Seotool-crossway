    ...
    message:
      wpPosts.length === 0
        ? statuses.includes("draft")
          ? `No WordPress posts found for statuses: ${statuses.join(", ")}. Check that matching posts exist and the WP user can edit them.`
          : `No posts found for statuses: ${statuses.join(", ")} only. Your connection test found drafts in WordPress — include "draft" in pull statuses (e.g. draft, future).`
        : undefined,
  };
}
