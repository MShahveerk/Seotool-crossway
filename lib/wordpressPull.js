...
import { extractFeaturedFromPost, fetchWordpressMediaUrl, fetchWordpressPosts, isWordpressScheduledPost, wpPostToCanonical } from "./wordpressClient.js";
...
export async function pullWordpressDraftsForSite(siteLink, opts = {}) {
  ...
  let { posts: wpPosts, statusCounts, scheduledDraftCount } = await fetchWordpressPosts(config, {
    statuses,
  ...
  return {
    ...
    message:
      wpPosts.length === 0
        ? `WordPress returned 0 posts (${formatStatusCounts(statusCounts, scheduledDraftCount)}). Scheduled items may still appear as drafts in WordPress — pull again after redeploy, or confirm the post uses status future/scheduled in WP admin.`
