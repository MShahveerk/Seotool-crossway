...
async function invokeProvider(providerId, { apiKey, model, messages, temperature, siteUrl, appName }) {
  const maxTokens = Number(process.env.AI_KEYWORD_MAX_TOKENS) || 8192;
  ...
  if (providerId === "openrouter") {
    ...
    const body = { model, temperature, messages, max_tokens: maxTokens };
    if (supportsJsonResponseFormat(model)) {
      body.response_format = { type: "json_object" };
    }
    const res = await axios.post(OPENROUTER_URL, body, { headers, timeout: 180000 });
  ...
  if (providerId === "openai") {
    const res = await axios.post(
      OPENAI_URL,
      { model, temperature, messages, max_tokens: maxTokens, response_format: { type: "json_object" } },
 ...
 */
export async function chatCompletionJson(messages, { temperature = 0.35, provider: preferred } = {}) {
