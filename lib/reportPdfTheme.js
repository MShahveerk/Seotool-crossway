...
export const CONTINUATION_HEADER_H = 34;
export const FOOTER_H = 40;
export const CONTENT_TOP = PAGE_H - HEADER_H - 36;
export const CONTENT_TOP_CONTINUATION = PAGE_H - CONTINUATION_HEADER_H - 28;
...
async function loadBrandLogo(pdf) {
  try {
    const logoPath = path.join(process.cwd(), "public", "brand", "roboseo-lockup.png");
...
function drawLogoMark(page, logoImage, x, y, size = 42) {
  if (!logoImage) return;
  page.drawImage(logoImage, { x, y, width: size, height: size });
}

function drawBrandedHeader(page, fonts, reportTitle, propertyLabel, reportDate, internal, logoImage) {
  const { display, body } = fonts;
  const cardTop = PAGE_H - HEADER_H + 8;
  const cardH = HEADER_H - 14;
  const logoSize = 46;
  const textX = MARGIN + (logoImage ? logoSize + 18 : 16);
  ...
  if (logoImage) {
    drawLogoMark(page, logoImage, MARGIN + 14, PAGE_H - logoSize - 22, logoSize);
  }
...
function drawContinuationHeader(page, fonts, reportTitle, logoImage) {
  const { display, body } = fonts;
  const yLine = PAGE_H - CONTINUATION_HEADER_H + 6;
  const logoSize = 22;

  page.drawLine({
    start: { x: MARGIN, y: yLine + 18 },
    end: { x: PAGE_W - MARGIN, y: yLine + 18 },
    thickness: 0.5,
    color: BRAND.ruleLight,
  });

  if (logoImage) {
    drawLogoMark(page, logoImage, MARGIN, yLine - 2, logoSize);
  }

  page.drawText(safePdfText(reportTitle, 64), {
    x: MARGIN + (logoImage ? logoSize + 10 : 0),
    y: yLine + 2,
    size: 9,
    font: display,
    color: BRAND.muted,
  ...
  const contW = body.widthOfTextAtSize(contLabel, 7);
  page.drawText(contLabel, {
    x: PAGE_W - MARGIN - contW,
    y: yLine + 2,
    size: 7,
    font: body,
    color: BRAND.muted,
...
export async function createBrandedReportContext({
  ...
  const { body: fontBody, display: fontDisplay, bodyBold: fontBodyBold } = await embedNunitoFonts(pdf);
  const fonts = { body: fontBody, display: fontDisplay, bodyBold: fontBodyBold };
  const reportDate = formatReportDate();
  const logoImage = await loadBrandLogo(pdf);
  ...
  const paintTableHeader = (columns, colXs) => {
    page.drawRectangle({
      x: MARGIN,
      y: y - 8,
      width: PAGE_W - MARGIN * 2,
  ...
  const startPage = () => {
    ...
    if (pageNum === 1) {
      drawBrandedHeader(page, fonts, reportTitle, propertyLabel, reportDate, internal, logoImage);
      y = CONTENT_TOP;
    } else {
      drawContinuationHeader(page, fonts, reportTitle, logoImage);
      y = CONTENT_TOP_CONTINUATION;
    }
  ...
  const ensureSpace = (height = LINE + PARA, opts = {}) => {
    if (y - height < CONTENT_BOTTOM) {
      ...
      if (opts.repeatTableHeader && activeTable) {
        paintTableHeader(activeTable.columns, activeTable.colXs);
      }
    }
  };
  ...
  const drawTableHeader = (columns, colXs) => {
    activeTable = { columns, colXs };
    ensureSpace(MIN_TABLE_BLOCK);
    paintTableHeader(columns, colXs);
  };

  const drawTableRow = (cells, colXs, alt = false) => {
    ensureSpace(TABLE_ROW_H + 6, { repeatTableHeader: true });
    if (alt) {
      page.drawRectangle({
        x: MARGIN,
        y: y - 6,
