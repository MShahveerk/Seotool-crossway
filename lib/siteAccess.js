...
function isPlaceholderClientLabel(label) {
  ...
  if (/^your account$/i.test(s)) return true;
  if (/^unnamed page\b/i.test(s)) return true;
  return false;
}

/** Honest label when Graph never returned a page name (stale DB stub). */
export function unnamedMetaPageLabel(facebookPageId) {
  const id = String(facebookPageId || "").trim();
  if (!id) return "Unnamed page";
  return `Unnamed page ${id}`;
}
...
export function pickClientDisplayName({ userName, siteLink, facebookPageId, metaName, preferMetaName = false } = {}) {
  ...
  if (preferMetaName) {
    ...
    return facebookPageId ? unnamedMetaPageLabel(facebookPageId) : "";
  }
  ...
  if (facebookPageId) return unnamedMetaPageLabel(facebookPageId);
  return "Your account";
}
