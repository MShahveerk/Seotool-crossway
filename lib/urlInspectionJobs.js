...
import { listWebsiteUrls, envFlag } from "./seoJobs.js";
...
export async function runUrlInspectionForSite(siteUrl, logger = console, runDateYmd = todayYmd()) {
  ...
  try {
    ...
    return { skipped: false, snapshot: updated, discovered: allUrls.length };
  } catch (err) {
    await prisma.urlInspectionSnapshot.update({
      where: { id: snapshot.id },
      data: {
...
export async function runDailyUrlInspections(logger = console) {
  if (!envFlag("SEO_URL_INSPECT_DAILY")) {
    logger.info?.("SEO_URL_INSPECT_DAILY not enabled — skipping daily URL inspections.");
    return { skipped: true };
  }
