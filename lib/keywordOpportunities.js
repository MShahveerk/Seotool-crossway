 ...
 *
 * Cost: /domain/keywords, /domain/competitors, /domain/overview and
 * /domain/pages are ~100 credits each, so a cold run is roughly 100 x (4 +
 * rivals). Cached per domain, and rivals are shared across analyses in a niche.
 */
...
export async function buildKeywordOpportunities(domainInput, opts = {}) {
  ...
  // The plain picture of the domain, fetched alongside the opportunity work:
  // what it ranks for in total, what that's worth, and which pages carry it.
  // Opportunities are the recommendation; this is the evidence behind it.
  const [own, overview, pages] = await Promise.all([
    keywordsFor(domain),
    fetchDomainOverview(domain, domain, { allowManual: true })
      .then((r) => normalizeDomainOverview(r?.data))
      .catch((err) => {
        logger?.warn?.(`Keyword opportunities: overview for ${domain} failed — ${err.message}`);
        return null;
      }),
    fetchDomainPages(domain, domain, { allowManual: true })
      .then((r) => normalizeDomainPagesList(r?.data))
      .catch((err) => {
        logger?.warn?.(`Keyword opportunities: pages for ${domain} failed — ${err.message}`);
        return [];
      }),
  ]);
  ...
  // Where the domain actually sits across the SERPs it appears in. Computed
  // from the keyword export rather than the overview endpoint so the buckets
  // always agree with the rows in the table below them.
  const ranked = own.filter((r) => r.position != null);
  const inBand = (lo, hi) => ranked.filter((r) => r.position >= lo && r.position <= hi).length;
  const distribution = [
    { band: "1–3", label: "Top 3", count: inBand(1, 3) },
    { band: "4–10", label: "Rest of page 1", count: inBand(4, 10) },
    { band: "11–20", label: "Page 2", count: inBand(11, 20) },
    { band: "21–50", label: "Pages 3–5", count: inBand(21, 50) },
    { band: "51+", label: "Beyond page 5", count: ranked.filter((r) => r.position > 50).length },
  ];
  ...
  const summary = {
    ...
    // Totals across the keywords we can see. `overview` carries the domain-wide
    // figures, which are usually larger — the export is capped.
    sampleTraffic: ranked.reduce((sum, r) => sum + (Number(r.traffic) || 0), 0),
    distribution,
  ...
  return {
    ...
    overview: overview?.hasData ? overview : null,
