...
/** Only real Lighthouse failures — skip informative/manual items PSI lists but does not score. */
function isActionablePsiAudit(audit) {
  if (!audit) return false;
  const mode = audit.scoreDisplayMode;
  if (mode === "notApplicable" || mode === "informative" || mode === "manual") return false;
  if (audit.score != null) return audit.score < 0.9;
  return mode === "error";
}

/** Score aligned with surfaced findings — not the raw Lighthouse SEO category score. */
export function computeSupplementalHealthScore(counts) {
  const critical = counts.critical || 0;
  const warning = counts.warning || 0;
  const notice = counts.notice || 0;
  if (critical === 0 && warning === 0 && notice === 0) return 100;

  const criticalPenalty = Math.min(70, critical * 12);
  const warningPenalty = Math.min(25, warning * 4);
  const noticePenalty = Math.min(10, notice * 1.5);
  return Math.max(0, Math.round(100 - criticalPenalty - warningPenalty - noticePenalty));
}
...
function issuesFromPageSpeed(payload, fetchedAt) {
  ...
  for (const id of ids) {
    ...
    if (!isActionablePsiAudit(audit)) continue;
...
export async function buildSupplementalAudit(siteUrl) {
  ...
  const healthScore = computeSupplementalHealthScore(counts);
  ...
  return {
    ...
    pagespeed: pagespeed
      ? {
          lighthouseSeoScore: pagespeed.seoScore,
