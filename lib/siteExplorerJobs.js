...
export async function enrichWithLiveAuthority(payload, domain) {
  ...
  try {
    ...
    payload.authority = authorityFromOpr(own);
...
/** UR-like per URL = Open PageRank of the URL host (subdomains get their own score). */
export async function enrichPageUrlRatings(items = []) {
  ...
  try {
    ...
    return items.map((row) => {
      ...
      const ur100 = opr ? toScore100(opr.score) : null;
      return { ...row, ur100, urHost: hostDomain };
    });
  } catch {
    return items;
  }
