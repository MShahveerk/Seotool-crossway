/**
 * Site Explorer jobs — weekly Common Crawl CDX fetch + Open PageRank merge, stored per domain.
 */
...
/** Cached CDX + OPR snapshots stay fresh for 7 days; cron reruns every Monday. */
export const SITE_EXPLORER_CACHE_TTL_DAYS = 7;
export const SITE_EXPLORER_CACHE_TTL_MS = SITE_EXPLORER_CACHE_TTL_DAYS * 24 * 60 * 60 * 1000;
export const SITE_EXPLORER_CRON = "0 5 * * 1";

export function isSnapshotStale(finishedAt) {
  if (!finishedAt) return true;
  return Date.now() - new Date(finishedAt).getTime() >= SITE_EXPLORER_CACHE_TTL_MS;
}

export function snapshotCacheExpiresAt(finishedAt) {
  if (!finishedAt) return null;
  return new Date(new Date(finishedAt).getTime() + SITE_EXPLORER_CACHE_TTL_MS);
}
...
const KEEP_SNAPSHOTS_PER_DOMAIN = 12;
...
export function snapshotToApiPayload(snapshot, { view = "overview", page = 1, pageSize = 50 } = {}) {
  ...
  const base = {
    ...
    source: "database",
    stale: isSnapshotStale(snapshot.finishedAt || snapshot.startedAt),
    cacheTtlDays: SITE_EXPLORER_CACHE_TTL_DAYS,
    cacheExpiresAt: snapshotCacheExpiresAt(snapshot.finishedAt || snapshot.startedAt),
  ...
  try {
    ...
    if (includeCrawl) {
      ...
      report = {
        ...
        referring: [],
        notes: [
          "Authority from Open PageRank (live). Indexed pages refresh on the weekly Site Explorer cron to avoid Common Crawl rate limits.",
        ],
        blocked: false,
      };
    }
...
/** Domains to refresh: all known websites + any domain previously cached (incl. explore mode). */
export async function listDomainsForSiteExplorerRefresh() {
  const urls = await listWebsiteUrls();
  const byDomain = new Map();

  for (const url of urls) {
    const normalized = normalizeSiteOrigin(url);
    const domain = toDomain(normalized);
    if (domain) byDomain.set(domain, normalized);
  }

  const cached = await prisma.siteExplorerSnapshot.findMany({
    where: { status: "success" },
    distinct: ["domain"],
    select: { domain: true, siteUrl: true },
  });
  for (const row of cached) {
    if (!row.domain || byDomain.has(row.domain)) continue;
    byDomain.set(row.domain, row.siteUrl || `https://${row.domain}`);
  }

  return [...byDomain.entries()].map(([domain, siteUrl]) => ({ domain, siteUrl }));
}

/** Weekly cron: refresh stale site explorer snapshots (Common Crawl + OPR). */
export async function runSiteExplorerForAllSites(logger = console) {
  const targets = await listDomainsForSiteExplorerRefresh();
  let ok = 0;
  let failed = 0;
  let skipped = 0;

  for (const { domain, siteUrl } of targets) {
    const latest = await prisma.siteExplorerSnapshot.findFirst({
      where: { domain, status: "success" },
      orderBy: { finishedAt: "desc" },
      select: { finishedAt: true },
    });
    if (latest?.finishedAt && !isSnapshotStale(latest.finishedAt)) {
      skipped += 1;
      continue;
    }

    try {
      const snap = await runSiteExplorer(siteUrl, { includeReferring: true, includeCrawl: true });
      if (snap.status === "error") {
        failed += 1;
        logger.error(`Site explorer failed: ${siteUrl} — ${snap.errorMessage}`);
      } else {
        ok += 1;
        logger.info(
          `Site explorer saved: ${siteUrl} (indexed=${snap.indexedUrls}, referring=${snap.referringDomainsCount}, opr=${snap.referringDomainsOpr ?? "n/a"})`
        );
      }
    } catch (err) {
      failed += 1;
      logger.error(`Site explorer crashed: ${siteUrl} — ${err.message}`);
    }
    await new Promise((r) => setTimeout(r, DELAY_BETWEEN_SITES_MS));
  }

  logger.info(
    `Site explorer weekly run done. ok=${ok} failed=${failed} skipped=${skipped} due=${targets.length - skipped}`
  );
