...
/** Fire-and-forget refresh — used when the HTTP request must return before Common Crawl finishes. */
export function kickSiteExplorerRefresh(siteUrl) {
  void runSiteExplorer(siteUrl).catch((err) => {
    console.error(`Site explorer background refresh failed: ${siteUrl}`, err?.message || err);
  });
}
