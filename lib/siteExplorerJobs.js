...
export async function executeSiteExplorerRefresh(siteUrl, snapshotId, { includeReferring = true } = {}) {
  const normalized = normalizeSiteOrigin(siteUrl);
  const domain = toDomain(normalized);
  if (!domain || !normalized) throw new Error("Invalid website URL for site explorer.");

  try {
    const report = await buildSiteExplorerReport(normalized, { includeReferring });
...
export async function runSiteExplorer(siteUrl, { includeReferring = true } = {}) {
  ...
  return executeSiteExplorerRefresh(siteUrl, prep.snapshotId, { includeReferring });
}
...
export async function runSiteExplorerForAllSites(logger = console) {
  ...
  for (const url of urls) {
    try {
      const snap = await runSiteExplorer(url, { includeReferring: true });
