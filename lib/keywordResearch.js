  ...
  if (!ideas.length) {
    try {
      ...
      if (fallbackSeeds.length) {
        const { keywords, meta } = await discoverAutocompleteKeywords(fallbackSeeds, { existingQueries });
...
function deriveSeedsFromSite(siteUrl, gscQueries = []) {
  ...
  try {
    ...
    if (brand && brand.length > 2) return [brand.replace(/-/g, " ")];
  } catch {
    /* ignore */
  }
  return [];
...
export async function buildSuggestKeywordResearch(siteUrl, range = "28d", geoKey = "us", { seed = "", forceRefresh = false } = {}) {
  ...
  if (!seedKeywords.length) {
    const err = new Error("Enter a seed keyword or connect Search Console so we can derive seeds from your queries.");
    err.status = 400;
    throw err;
  }

  const { keywords, meta } = await discoverAutocompleteKeywords(seedKeywords, { existingQueries });
