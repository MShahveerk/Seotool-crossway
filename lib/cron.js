...
export function startCronJobs() {
  ...
  cron.schedule("* * * * *", async () => {
    ...
    try {
      ...
      logger.error(`Post automation cron failed: ${error.message}`);
    }

    try {
      const { runScheduledContentAutoschedule } = await import(
        "./contentAutoschedule/runner.js"
      );
      await runScheduledContentAutoschedule(logger);
    } catch (error) {
      logger.error(`Content autoschedule cron failed: ${error.message}`);
    }
  });

  // 3. Weekly SEO jobs — Mondays 06:00 (server local time)
  // Sitemap: SEO_AUTO_SUBMIT_SITEMAPS. Staff digests: per-user prefs (slide decks).
  ...
  // 3b. Monthly reports (weekly send) — Mondays 07:00 (per-user receive* prefs)
  cron.schedule("0 7 * * 1", async () => {
    ...
    try {
      ...
      logger.error(`Monthly report send failed: ${err.message}`);
    }
  });
  ...
  // 3c3. Email inbound — every 10 minutes (stores one draft per account; subject must include "SMM POST")
  ...
  cron.schedule("0 9 * * *", async () => {
    ...
    try {
      ...
      logger.error(`Email draft promotion failed: ${err.message}`);
    }
  });
