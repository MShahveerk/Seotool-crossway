...
export function startCronJobs() {
  ...
  // 3g. Daily Common Crawl site explorer — 05:00 (stores indexed pages + referring hosts per domain)
  cron.schedule("0 5 * * *", async () => {
    logger.info("Running daily site explorer (Common Crawl) refresh...");
    try {
      const { runSiteExplorerForAllSites } = await import("./siteExplorerJobs.js");
      await runSiteExplorerForAllSites(logger);
    } catch (err) {
      logger.error(`Daily site explorer refresh failed: ${err.message}`);
    }
  });
