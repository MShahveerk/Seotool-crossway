...
export async function fetchOpenPageRank(domains, { includeHistory = false } = {}) {
  ...
  for (const row of data?.results || []) {
    ...
    out.set(domain, {
      ...
      referringDomains:
        row.found && row.referring_domains != null
          ? Number(row.referring_domains)
          : row.referring_domains != null
            ? Number(row.referring_domains)
            : null,
...
export async function getAuthorityScores(domains) {
  ...
  const staleReferring = clean.filter((d) => {
    const row = out.get(d);
    return row && row.referringDomains == null && isAuthorityConfigured();
  });
  const needLive = [...new Set([...missing, ...staleReferring])];

  if (needLive.length && isAuthorityConfigured()) {
    const live = await fetchOpenPageRank(needLive, { includeHistory: missing.some((d) => !out.has(d)) });
