...
export async function buildLinkOpportunitiesPdf(data, { limitPerGroup = 1000, rejectedLimit = 1000 } = {}) {
  ...
  const ctx = await createCarbonReport({
    ...
    meta: `${data.originCountry ? `Links from ${data.originCountry} - ` : ""}${data.location || "No SERP location set"} - ${data.device} - ${nf(data.targets?.length)} ranking sites analysed`,
  ...
  statTiles([
    { label: "You can pitch", value: nf(s.prospects), hint: `${nf(s.unpaid)} confirmed free · ${nf(s.paid)} paid`, accent: true },
