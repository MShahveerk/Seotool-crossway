...
function buildSiteMemory({ siteLink, harvest, config, trends, news }) {
  ...
  return {
    ...
    researcherNotes: String(brief.notes || "").trim().slice(0, 800),
    standingBrief: String(config?.seedPrompt || "").trim().slice(0, 1200),
    brandNotes: String(config?.brandNotes || "").trim().slice(0, 800),
    opportunities,
    seedBag: [...new Set(seedBag)].slice(0, 24),
    inTheAir: (trends || []).slice(0, 8).map((t) => t.query).filter(Boolean),
    headlines: (news || []).slice(0, 4).map((n) => n.title).filter(Boolean),
...
async function collectTrendHints(harvest) {
  if (!harvest) return [];
  try {
    if (!(await isSerpApiReady()) || isSerpApiQuotaExhausted()) return [];
    const pack = await collectTrendCandidates(harvest, {
      market: harvest.market,
      related: false,
    });
    return (pack.candidates || []).slice(0, 12);
  } catch {
    return [];
  }
}
...
async function collectNewsHints(harvest) {
  try {
    ...
    const bits = [harvest?.brief?.category, harvest?.brief?.brandName]
      .map((s) => String(s || "").trim())
      .filter(Boolean);
    if (!bits.length) return [];
    const res = await fetchGoogleSerp(`${bits.join(" ")} news`, { num: 5, skipDuckDuckGo: true });
...
async function loadSiteMemory(siteLink) {
  ...
  const memory = buildSiteMemory({ siteLink, harvest, config, trends, news });
...
function chatSystemPrompt(memory) {
  return `You are ${CHAT_PERSONA}, the in-studio editor sitting with the operator. You already read this website. You are not the Topic Decider pipeline stage and you never call yourself that.

VOICE
- Fun, sharp, slightly teasing — a good editor who did the homework.
- Talk about the business, the domain, the reader, and the move.
- BAN these phrases forever: "Tell me what this article should do", "What tone or angle should the article take", "research shows strong interest in", "Topic Decider", "harvest", "KD", "keyword count", "featured terms", "seedQuery".
- Never wrap a search phrase in quotes. Never paste "near me" queries as if they were the idea.
- Translate demand into an article a human would want to read.

LENGTH
- Greeting or a yes/go: 2–4 sentences. One question max.
- If they asked for topics: a one-line intro, then exactly 3 numbered titles, then "Say the number."
...
- In the air this week: ${(memory.inTheAir || []).join("; ") || "nothing extra"}
- Headlines worth a nod: ${(memory.headlines || []).join("; ") || "none"}

JOB
- NEW chat: greet like you already live here. Prove brand + domain. Offer ONE take. ready:false. Never ask what the article should do.
- "suggest topics" / what should we write: 3 editorial titles. ready:false.
- "yes" / "anything" / "fulfill keywords" / "ok" / "go": pick the strongest title, ready:true, confirm in one breath. Do not ask another scoping question.
...
function instructionForTurn({ greeting, memory, hasDraft, threadStatus, forceReady, classified }) {
  if (greeting) {
    return "Brand-new chat. Greet like you already live on this account. Prove brand and domain. Offer one take. ready:false. Never ask what the article should do.";
  }
...
export async function runDeciderChatTurn({
  ...
  const turnOpts = { memory, greeting, lastUser, forceReady };
  let turn;
  try {
    const result = await chatCompletion({
      ...
      siteConfig: config,
      temperature: greeting ? 0.7 : classified.kind === "topic-ask" ? 0.75 : 0.6,
      maxTokens: 1100,
      jsonMode: true,
    });
