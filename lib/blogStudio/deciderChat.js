...
import { classifyUserTurn, finalizeTurn, isBrokenReply, lastUserText } from "./deciderChatVoice.js";
...
function opportunityLine(topic) {
  ...
  if (!why) return "";
  if (/expanded from seed|did not cluster|featured terms|keyword library/i.test(why)) return "";
  if (
    /favicon|orphan pages?|internal link(?:ing| map)?|pass(?:ing)? authority|crawl(?: errors?)?|seo landers?/i.test(
      why
    )
  ) {
    return "";
...
function instructionForTurn({ memory, hasDraft, threadStatus, forceReady, classified, lastUser }) {
  ...
  if (classified.kind === "topic-ask") {
    return "They asked for topics. Pitch exactly 3 editorial titles in a numbered list. ready:false. Ask them to pick a number. Never use harvest labels like Expanded from seed.";
  }
  if (classified.kind === "steer") {
    return `They just named the idea in their own words: ${JSON.stringify(lastUser)}. Write a clickable title FROM THAT IDEA. Confirm it. ready:false unless they already said to write it. Do not change the subject. Never say Expanded from seed, favicon, orphan pages, or internal links.`;
  }
  if (classified.kind === "go-ahead" || classified.kind === "pick" || forceReady) {
    return `They said go. Lock the title they already chose (see the conversation). ready:true. Do not ask another scoping question. Do not invent harvest labels.`;
  }
  return `Stay in character. Latest operator message: ${JSON.stringify(lastUser)}. Answer that. If they want topics, pitch three. If they named an idea, title it.`;
}
...
function sanitizeMessages(messages) {
  ...
  return messages
    ...
    .filter((m) => m.role === "user" || !isBrokenReply(m.content))
    .slice(-20);
}
...
export async function runDeciderChatTurn({
  ...
  if (greeting) {
    return {
      ...finalizeTurn({}, { memory, greeting: true, entropy: Date.now() }),
  ...
  const turnOpts = { memory, greeting: false, lastUser, forceReady, history };
  let turn;
  try {
