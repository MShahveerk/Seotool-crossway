      ...
      try {
        ...
        stages = await updateStage(runId, stages, stages.length - 1, {
          ...
          note: img.usedReference
            ? "Used Assets reference image + visual guidelines"
            : "Used Assets visual guidelines (no reference file on disk)",
        });
      } catch (imgErr) {
  ...
  try {
    if (source === "excel") {
      ...
      const overrides = {
        ...
        // Per-row direction only — site imagePrompt + reference stay from site config
