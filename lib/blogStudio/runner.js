...
export async function enqueueStudioRun({
  ...
  if (!isRevision) {
    ...
    if (!skipDecider && !(await isSerpApiReady())) {
      const err = new Error(
        "Add a SerpAPI key in Admin → Data sources so the Topic Decider can read Google Trends."
      );
      err.status = 400;
      throw err;
    }
  ...
  assertPrefixKeys(config, { needDecider: !skipDecider });
