...
import { hasOperatorKeywordBank } from "./operatorKeywords.js";
import {
...
export async function enqueueStudioRun({
  ...
  if (!isRevision) {
    ...
    if (!harvest && !hasOperatorKeywordBank(config)) {
      const err = new Error(
...
export async function executeStudioRun(runId, { generateImage = true, fromCheckpoint = false } = {}) {
  ...
  try {
    ...
    const metaStage = Array.isArray(run.stagesJson)
      ? run.stagesJson.find((s) => s?.agent === "_context")
      : null;
    if (metaStage?.topicFull) topic = String(metaStage.topicFull).trim() || topic;
    const revision =
    ...
    if (regenText) {
      if (resumeHeadings) {
        ...
        serpCompete = cp.serpCompete || null;
        harvest = cp.harvestBrief ? { brief: cp.harvestBrief } : await loadLatestResearchForSite(run.siteLink);
        if (serpCompete?.suggestedWordCountRange) {
          config.wordCountRange = serpCompete.suggestedWordCountRange;
        }
        stages = stages.map((s) =>
        ...
        serpCompete = prefix.serpCompete || null;
        harvest = prefix.harvest || null;
      ...
      if (config.humanizerEnabled) {
        ...
        try {
          ...
          stages = await updateStage(runId, stages, stages.length - 1, {
            ...
            note: hum.usedWriterHtml
              ? "Model omitted article HTML. Kept the writer draft and ran the deterministic scrub."
              : undefined,
            preview: previewFromJson({
              title: article.title,
      ...
      article = applyStudioCalendarToDraft(article, {
    ...
    const content = normalizeArticleHtml(String(article.article_html || "")).trim();
    if (!title || !content) {
    ...
    if (regenImage) {
      ...
      featuredImagePath = operatorImagePath;
      backupImagePaths = [];
      stages = await pushStage(runId, stages, {
        agent: "image",
        ...
        note: "You supplied this featured image. The image agent was skipped.",
      });
...
async function pauseForHeadingsApproval({
  ...
  const checkpoint = {
    ...
    bind,
    angle: angle || {},
    serpCompete: serpCompete || null,
    harvestBrief: harvest?.brief || null,
