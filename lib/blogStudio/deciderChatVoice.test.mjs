...
test("greeting is an article pitch, never an audit, and changes with entropy", () => {
  const greet = fallbackGreeting(hala, { entropy: 1, hour: 14 });
  const other = fallbackGreeting(hala, { entropy: 99, hour: 14 });
  assert.match(greet, /Hala Jets/);
  assert.ok(greet.length > 20);
  ...
  const poisoned = finalizeTurn(
    ...
    { memory: hala, greeting: true, entropy: 1, hour: 14 }
  );
