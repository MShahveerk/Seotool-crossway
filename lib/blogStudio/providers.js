...
export async function generateImage({
  ...
  const refs = [];
  // Support referenceImages[] (preferred) or legacy single referenceImage
  ...
  const resolvedQuality = ["low", "medium", "high", "auto"].includes(String(preferredQuality))
    ...
    : "medium";
  // OpenAI edits/generations accept jpg|png|webp (not "jpeg").
  const fmtIn = String(outputFormat || "jpeg").toLowerCase();
  const resolvedFormat = fmtIn === "jpg" || fmtIn === "jpeg" ? "jpg" : fmtIn === "webp" ? "webp" : "png";
  ...
  try {
    ...
    if (refs.length) {
      // fetch + FormData + File — Blob-only parts often drop in Node and the API
      // silently behaves like text-only generation (irrelevant images).
      ...
      console.info(
        `[image] edits with ${refs.length} reference(s), model=${m}, fidelity=${
          modelSupportsInputFidelity(m) ? "high" : "n/a"
      ...
      if (status >= 400) {
        ...
        throw Object.assign(
          new Error(`OpenAI image edits (reference) failed: ${apiMsg}`),
          { response: { status, data } }
        );
      }
    } else {
      console.info(`[image] generations (no reference), model=${m}, promptChars=${cleanPrompt.length}`);
    ...
    return {
      ...
      inputFidelity: refs.length && modelSupportsInputFidelity(m) ? "high" : null,
    };
  } catch (err) {
