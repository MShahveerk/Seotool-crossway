...
async function generateImageViaAnthropic({
  ...
  content.push({
    type: "text",
    text: [
      "Draw a finished marketing illustration as one complete SVG document.",
      "Output only the SVG markup. No markdown fences. No commentary.",
      `Use width="${w}" height="${h}" and viewBox="0 0 ${w} ${h}".`,
      "Filled shapes, gradients, and a clear subject. No <image href>, no scripts, no external URLs.",
      refs.length
