...
export async function generateImage({
  ...
  if (p === "anthropic") {
    const err = new Error(
      "Anthropic Claude cannot generate images (vision only). Switch the Image agent to OpenAI or OpenRouter (Flux, Gemini Image, GPT Image)."
    );
    err.status = 400;
    throw err;
  }
  ...
  if (p !== "openai" && p !== "openrouter") {
    const err = new Error(
      `Unsupported image provider "${p}". Use OpenAI or OpenRouter.`
    );
    err.status = 400;
    throw err;
  }
