...
export async function generateImage({
  ...
  try {
    ...
    if (refs.length) {
      // Use fetch + FormData so multipart file parts are reliable in Node.
      // Append each reference as "image" (OpenAI accepts up to 16; we send ≤4).
      // First image receives the richest input fidelity.
