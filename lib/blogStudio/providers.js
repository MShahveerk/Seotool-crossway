...
export async function generateImage({
  ...
  const hasReference =
    ...
    Buffer.isBuffer(referenceImage.buffer) &&
  ...
  try {
    ...
    if (hasReference) {
      ...
      const fileName = String(referenceImage.fileName || "reference.png");
      const form = new FormData();
      form.append("model", m);
      form.append("prompt", cleanPrompt.slice(0, 32000));
      form.append("size", "1536x1024");
      form.append("quality", "medium");
      // Preserve brand/style cues from the Assets reference image
      if (/gpt-image/i.test(m) && !/mini/i.test(m)) {
        form.append("input_fidelity", "high");
      }
      const bytes = new Uint8Array(referenceImage.buffer);
    ...
    if (res.status >= 400) {
      throw Object.assign(new Error("OpenAI image request failed"), { response: res });
    }

    const extracted = extractImageBuffer(res.data);
    let buffer = extracted.buffer;
    if (!buffer && extracted.url) {
      const img = await axios.get(extracted.url, { responseType: "arraybuffer", timeout: 60000 });
