/**
 * Run Decider → Binder → Checker → SERP battlefield → Headings.
 */
...
import { isSerpApiQuotaExhausted } from "../serpapi.js";
import { hasProviderKey } from "./providers.js";
import { collectOperatorCandidates, hasOperatorKeywordBank } from "./operatorKeywords.js";
import { collectSerpCompete, compactSerpCompete, emptySerpCompete } from "./serpCompete.js";
...
/**
 * @returns {{ topic, a1, lockedHeadings, bind, harvest, serpCompete }}
 */
export async function runDraftPrefix({
  ...
  let seedQuery = String(config.deciderSeedQuery || currentTopic || "").trim();
  let pack = { candidates: [], geo: marketToTrendsGeo(harvest?.market), source: "harvest" };
  let decider = null;

  if (skipDecider && (config.deciderSeedQuery || config.recommendedAngle)) {
    stages = await pushStage(runId, stages, {
      ...
      data: {
        topic: currentTopic,
        seedQuery,
        why: "Agreed in composer chat",
        angle: String(config.recommendedAngle || "").trim() || null,
        source: "chat",
      },
    });
  }
  ...
  if (!skipDecider) {
    const useTrends = (await isSerpApiReady()) && !isSerpApiQuotaExhausted();
  ...
  try {
    ...
    const chatAngle = String(config.recommendedAngle || "").trim();
    if (chatAngle) {
      angle = { ...angle, recommended_angle: chatAngle };
    }
  ...
  try {
    ...
    if (verdict.duplicate) {
      ...
      checkMeta.webSkipped = Boolean(verdict.webSkipped);
    }
  } catch (err) {
    await updateStage(runId, stages, stages.length - 1, {
      status: "failed",
  ...
  stages = await updateStage(runId, stages, stages.length - 1, {
    ...
    data: {
      ...
      webSkipped: Boolean(checkMeta.webSkipped),
    },
  });
  ...
  stages = await pushStage(runId, stages, {
    agent: "serp_compete",
    role: "SERP battlefield",
    status: "running",
    startedAt: new Date().toISOString(),
  });
  const extraQuestions = Array.isArray(harvest?.brief?.buyingQuestions)
    ? harvest.brief.buyingQuestions
    : [];
  let serpCompete;
  try {
    serpCompete = await collectSerpCompete({
      query: bind.primary || currentTopic,
      gl,
      siteLink,
      configuredRange: config.wordCountRange,
      extraQuestions,
    });
  } catch (err) {
    serpCompete = emptySerpCompete(err.message || "search failed", {
      query: bind.primary || currentTopic,
      configuredRange: config.wordCountRange,
    });
  }
  ...
  const competePreview = serpCompete.skipped
    ? serpCompete.reason
      ? `Skipped — ${String(serpCompete.reason).slice(0, 80)}`
      : "Skipped"
    : `${serpCompete.titles?.length || 0} titles · ${serpCompete.peopleAlsoAsk?.length || 0} PAA · ${serpCompete.suggestedWordCountRange || "words"}`;
  stages = await updateStage(runId, stages, stages.length - 1, {
    status: "succeeded",
    finishedAt: new Date().toISOString(),
    preview: competePreview,
    data: compactSerpCompete(serpCompete),
  });
  ...
  try {
    headings = await runHeadingsAgent({
      ...
      harvest,
      angle,
      serpCompete: compactSerpCompete(serpCompete),
    });
  } catch (err) {
    await updateStage(runId, stages, stages.length - 1, {
      status: "failed",
