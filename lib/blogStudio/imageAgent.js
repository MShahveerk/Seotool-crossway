import { generateImage, defaultImageModelForProvider, imageModelForReferences } from "./providers.js";
...
export function buildImagePrompt({
  ...
  const brandBlock = brandImagePromptBlock(kit);
  const revisionNote = String(config.imageRevisionNote || "").trim();
  return [
    subject,
    revisionNote
      ? `REVISION — the previous image was rejected by a reviewer. You MUST address this feedback precisely in the new image:\n${revisionNote}`
      : "",
    referenceStylePromptBlock(referenceCount),
    system || DEFAULT_IMAGE_PROMPT_SYSTEM,
    siteGuidelines ? `Site visual guidelines:\n${siteGuidelines}` : "",
    brandBlock || "No logos, watermarks, or dense text overlays.",
...
async function generateOne({ config, article, topic = "", variantNote = "" }) {
  ...
  const imageProvider = String(config.imageProvider || "openai").toLowerCase();
  let imageModel = String(config.imageModel || "").trim();
  ...
  if (kit.enabled) {
    try {
      ...
      console.warn(`[blogStudio] brand frame failed: ${err.message}`);
    }
  }
