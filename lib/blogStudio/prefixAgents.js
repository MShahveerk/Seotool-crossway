...
export async function runDeciderAgent({ config, harvest, pack }) {
    ...
    user: JSON.stringify(
      {
        ...
        prefer: "library-seed-plus-relevant-world-hook",
        brief: {
          brandName: harvest?.brief?.brandName,
          category: harvest?.brief?.category,
          audience: harvest?.brief?.audience,
        ...
        instruction:
          "Pick candidateId from the library. Optionally set trendHookId from trendHooks to angle the title. Never copy candidate.query.",
      },
      null,
      2
    ),
  ...
  const library = all.filter((c) => c.lane !== "world");
  const pool = library.length ? library : all;
...
function fallbackHeadingsJson({ topic, bind, harvest }) {
  ...
  for (const kw of pool) {
    ...
    if (normalizeTopicString(kw) === normalizeTopicString(topic)) continue;
    const heading = titleCaseHeading(kw);
    if (!heading || used.has(normalizeTopicString(heading))) continue;
    used.add(normalizeTopicString(heading));
    sections.push({
      section_id: `h2_${sections.length + 1}`,
      heading_h2: heading,
      heading_keywords: heads.includes(kw) ? [kw] : [],
      body_keywords: body.includes(kw) ? [kw] : [],
      key_points: [`Cover ${kw} with concrete numbers, ranges, or examples.`],
      subsections: [{ heading_h3: "", writing_instruction: `Use “${kw}” naturally. Stay on ${topic}.` }],
    });
  }
  let i = 0;
  while (sections.length < 4 && i < editorial.length) {
    const heading = editorial[i];
    i += 1;
    ...
    sections.push({
      ...
      body_keywords: [],
      key_points: [`Stay on ${topic}.`],
      subsections: [{ heading_h3: "", writing_instruction: `Explain this in the context of ${topic}.` }],
    });
  }
...
export async function runHeadingsAgent({ config, topic, bind, harvest, angle, feedback = "" }) {
  ...
  const payload = {
    ...
    requirement: "Return 4–7 sections with heading_h2. Empty sections is a failure.",
  };
