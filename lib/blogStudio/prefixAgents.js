...
export function synthesizeAgent1Json({ topic, bind, angle = {}, headings = null, serpCompete = null }) {
  return {
    ...
    outline_direction: headings
      ? asArray(headings.sections)
          ...
          .join(" → ")
      : "",
    word_count_target: serpCompete?.suggestedWordCountRange || "",
...
function mergeFaqFromCompete(json, serpCompete) {
  const faq = [...asArray(json?.faq)];
  const seen = new Set(faq.map((q) => String(q.question || "").toLowerCase()));
  const extras = [
    ...asArray(serpCompete?.peopleAlsoAsk),
    ...asArray(serpCompete?.relatedSearches),
    ...asArray(serpCompete?.gaps),
  ];
...
function fallbackHeadingsJson({ topic, bind, harvest, serpCompete }) {
  ...
  const gapHeads = asArray(serpCompete?.gaps)
    .map((q) => String(q || "").replace(/\?+$/, "").trim())
    .filter((q) => q && q.length < 70);
  const editorial = [
    ...gapHeads.slice(0, 2),
  ...
  const faq = asArray(harvest?.brief?.buyingQuestions)
    ...
    .filter((q) => q.question)
    .slice(0, 5);
  return mergeFaqFromCompete({ h1, sections, faq, repaired: true }, serpCompete);
}

const STALE_HEADINGS_COPY =
  /At least half of H2s must include a heading_keywords term|4–7 H2s are required\./i;
...
export async function runHeadingsAgent({
  config,
  topic,
  bind,
  harvest,
  ...
  const compete = serpCompete && !serpCompete.skipped ? serpCompete : null;
  const payload = {
    topic,
    primary: bind.primary,
    heading_keywords: bind.headingKeywords,
    ...
    word_count_range: compete?.suggestedWordCountRange || config.wordCountRange,
    serp_compete: compete,
    ...(String(feedback || "").trim() ? { reviewer_feedback: String(feedback).trim() } : {}),
    requirement: compete
      ? "Return 5–8 sections with heading_h2. Cover peopleAlsoAsk. Add two H2s from gaps that rivals omit. One comparison/decision section. Never copy a rival title. Empty sections is a failure."
  ...
  if (json.sections.length < 4) {
    json = fallbackHeadingsJson({ topic, bind, harvest, serpCompete: compete });
  } else {
    json = mergeFaqFromCompete(json, compete);
  }
