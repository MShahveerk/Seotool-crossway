...
function deciderSystemPrompt(config) {
  ...
  if (!custom || STALE_DECIDER_COPY.test(custom)) return DEFAULT_DECIDER_PROMPT;
  return custom;
}
    ...
    user: JSON.stringify(
      {
        ...
        instruction: "Pick one candidateId, then write topic as a rewritten blog title — never copy candidate.query.",
      },
      null,
      2
    ),
