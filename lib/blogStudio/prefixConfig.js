...
import { sanitizeOperatorKeywords } from "./operatorKeywords.js";
...
const BOOL_FIELDS = new Set(["headingsApprovalEnabled", "humanizerEnabled", "useOperatorKeywords"]);
const JSON_FIELDS = new Set(["operatorKeywords"]);
...
export async function writePrefixAgents(siteLink, patch = {}) {
  ...
  for (const field of FIELDS) {
    ...
    else if (JSON_FIELDS.has(field)) next[field] = sanitizeOperatorKeywords(patch[field]);
...
export function mergePrefixAgents(row, stored = {}) {
  ...
  for (const field of FIELDS) {
    if (BOOL_FIELDS.has(field)) {
      ...
      out[field] = stored[field] !== undefined ? sanitizeOperatorKeywords(stored[field]) : [];
    } else {
