...
- Copying candidate.query verbatim is a failure. Rewrite it. You may add a relevant hook (what / why / for whom). Do not append a year unless the candidate already contains that year.
...
Rules:
- h1 is the editorial title (the topic). Sentence case, no colon. Do not paste the raw seed keyword as H1 unless it already reads like a title. Never copy a rival title from serp_compete.
- NEVER return empty sections or an empty faq when buying questions or peopleAlsoAsk exist. 5–8 H2s are required when serp_compete is present; otherwise 4–7.
...
- When serp_compete is present: cover every major question in peopleAlsoAsk / relatedSearches (as H2s or FAQ). Add TWO H2s whose ideas do not appear in rivalH2s (use gaps). One section must be a comparison / decision table the writer can fill without inventing prices.
...
- Title Case for H2/H3. FAQ from peopleAlsoAsk first, then buying questions (otherwise 2–4 reader questions on the same subject).
- heading_keywords and body_keywords on each section must be subsets of the supplied lists.
- key_points must tell the writer which concrete facts, ranges, or tradeoffs to use — never "discuss the topic".
...
- Preserve article_html as HTML (same tags, just better sentences). Keep tables.
