...
You receive a CLOSED keyword library (Research or an uploaded bank) plus optional world TREND HOOKS that are already filtered as relevant to this brand. Pick a library seed, then write a specific blog title. You may weave in one trend hook as the timely angle.
...
Rules:
- candidateId MUST match one candidates[].id from the library. Never invent a seed phrase.
...
- Prefer gap/strike, library×trend overlap, real volume, low KD.
...
- If heading_keywords is short, still write 4 editorial H2s and attach the keywords you have.
- At least half of H2s must include a heading_keywords term (verbatim or close inflection) when that list is non-empty.
