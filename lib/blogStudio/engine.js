...
export function sanitizeSiteConfigForClient(row) {
  ...
  return {
    ...
    agentReady: {
      ...
      image: hasProviderKey(row.imageProvider === "openrouter" ? "openrouter" : "openai", row),
    },
  };
}
...
export async function saveSiteStudioConfig(siteLink, input = {}) {
  ...
  const data = {
    ...
    referenceImagePath:
      input.referenceImagePath !== undefined ? input.referenceImagePath || null : existing.referenceImagePath,
    ...
    imageModel: input.imageModel || existing.imageModel || "gpt-image-1",
  ...
  return prisma.blogAutomationSiteConfig.upsert({
    where: { siteLink: link },
    create: data,
    update: data,
  });
