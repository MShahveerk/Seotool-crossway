...
/**
 * Which Excel queue row is due / scheduled for today, plus next-run timing.
 */
export function computeExcelSchedule({ siteConfig, campaign, now = new Date() } = {}) {
  ...
  const intervalMinutes = Math.max(5, Math.round(Number(siteConfig?.autoIntervalMinutes) || 1440));
  ...
  const nextRunAt = lastAutoAt
    ? new Date(lastAutoAt.getTime() + intervalMinutes * 60_000)
    : now;
  ...
  // Row that will run on the next tick (or is already processing)
...
export async function getExcelQueuePayload(siteLink) {
  const { getSiteStudioConfig, sanitizeSiteConfigForClient } = await import("./engine.js");
