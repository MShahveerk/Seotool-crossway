...
- If serp_compete is present, honor suggestedWordCountRange, cover peopleAlsoAsk in faq_section, and put rival gaps into writing_instruction. Intro must answer the query in two sentences.
...
- Instruct the writer to include one HTML comparison table and to use only facts from operator_facts / brand_notes / seed_prompt.`
...
- Allowed tags: h1,h2,h3,p,ul,ol,li,a,table,thead,tbody,tr,th,td,strong
- Include one comparison table (criteria, not invented prices)
...
- Never write a visible backslash-n; use HTML tags for structure
- Natural use of must-follow primary keyword in first 100 words
- First 80 words answer the search. No "in this article we will"
- If serp_compete.writerRules is present, treat every rule as mandatory
- Hit workflow_variables.target_word_count. Thin 1200-word overviews are a failure
- Do not invent facts, prices, certifications, or results. If a number is missing from operator_facts / brand_notes / seed_prompt, tell the reader what to ask the operator instead
