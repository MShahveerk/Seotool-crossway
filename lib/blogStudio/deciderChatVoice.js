...
export function fallbackGreeting(memory, opts = {}) {
  const brand = memory?.brandName || memory?.domain || "this site";
  const pitches = editorialPitches(memory, 1);
  const title = pitches[0]?.title || "";
  const hour = Number.isFinite(Number(opts.hour)) ? Number(opts.hour) : new Date().getHours();
  const entropy = opts.entropy ?? memory?.greetingEntropy ?? 0;
  const lastId = String(opts.lastOpenerId || memory?.lastGreetingId || "");

  const pool = [];
  if (!memory?.hasResearch) {
    pool.push({
      id: "no-research",
      text: `Research for ${brand} is empty. Run it from the dock once, then I'll pitch the article.`,
    });
  } else if (!title) {
    pool.push(
      {
        id: "pov",
        text: `I've got a point of view for ${brand}. Tell me the cut you want, or say suggest topics.`,
      },
      {
        id: "desk",
        text: `I'm on ${brand}. Query, angle, or say suggest topics and I'll pitch three.`,
      }
    );
  } else {
    pool.push(
      {
        id: "id-write",
        text: `I'd write "${title}" first. A buyer piece, not another service page. Want that, or a different cut?`,
      },
      {
        id: "open-with-title",
        text: `${brand}. I'd start with "${title}". Say go, or give me the cut you actually want.`,
      },
      {
        id: "not-service",
        text: `If we publish next for ${brand}, "${title}" is the one I'd fight for. Different angle?`,
      },
      {
        id: "one-line",
        text: `Keep "${title}", or tell me what this piece should win for ${brand}.`,
      }
    );
    if (hour < 12) {
      pool.push({
        id: "morning",
        text: `Caught you early on ${brand}. "${title}" is sitting on the desk. Want that, or something else?`,
      });
    }
    if (hour >= 17) {
      pool.push({
        id: "evening",
        text: `Late pass on ${brand} is fine. "${title}" is the draft I'd queue. Or name a different brief.`,
      });
    }
  }

  const avoided = lastId ? pool.filter((row) => row.id !== lastId) : pool;
  const use = avoided.length ? avoided : pool;
  let h = 2166136261;
  const seed = `${brand}|${title}|${entropy}|${hour}`;
  for (let i = 0; i < seed.length; i += 1) {
    h ^= seed.charCodeAt(i);
    h = Math.imul(h, 16777619);
  }
  const pick = use[(h >>> 0) % use.length];
  return pick.text;
}
 ...
 */
export function finalizeTurn(raw, { memory, greeting = false, lastUser = "", forceReady = false, history = [], entropy, hour } = {}) {
  ...
  if (greeting) {
    return {
      reply: fallbackGreeting(memory, { entropy, hour }),
      ready: false,
      topic: null,
      seedQuery: null,
      angle: null,
