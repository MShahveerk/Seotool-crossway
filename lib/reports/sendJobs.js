...
async function buildAttachmentsForSite(siteKey, {
  ...
  if (wantCombined && (wantWebsite || wantSmm)) {
    try {
      const bytes = await buildSlideDeckPdfBytes("combined", siteKey, {
        reportMonth: month,
        preparedFor,
        includeInternal,
      });
      attachments.push({
        filename: slideDeckFilename("combined", siteKey, month),
  ...
  // If combined failed but separates wanted, fall back
  if (wantCombined && !attachments.some((a) => a.content)) {
    if (wantWebsite && !isMetaPageId(siteKey)) {
      try {
        const bytes = await buildSlideDeckPdfBytes("website", siteKey, {
          reportMonth: month,
          preparedFor,
...
export async function sendClientReportsNow({ siteKey = null, userId = null, trigger = "manual" } = {}) {
  ...
  for (const user of users) {
    ...
    const wantWebsite = isSuper || userWantsClientWebsiteReport(user);
    const wantSmm = isSuper || userWantsClientSmmReport(user);
    const wantCombined = isSuper ? Boolean(user.receiveCombinedReport) : userWantsCombinedReport(user);
