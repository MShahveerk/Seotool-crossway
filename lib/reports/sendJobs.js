...
import { getReportDeckConfig } from "./reportDeckConfig.js";
...
async function buildAttachmentsForPack(pack, {
  ...
  // Resolve studio template across website / www / Meta aliases (same as download).
  const deckConfig = await getReportDeckConfig(pack.websiteUrl || siteKey, {
    equivalents: pack.equivalents || [siteKey, pack.websiteUrl].filter(Boolean),
  });
  ...
  // Each toggle is independent — send every selected deck as its own attachment.
  if (wantCombined) {
    try {
      // Combined pack uses site website data when available, regardless of the
      // separate "website report" toggle.
      const kind = pack.includeWebsite ? "combined" : "smm";
      const bytes = await buildSlideDeckPdfBytes(kind, siteKey, buildOpts);
      attachments.push({
  ...
  if (canWebsite) {
    try {
      const bytes = await buildSlideDeckPdfBytes("website", siteKey, buildOpts);
      attachments.push({
  ...
  if (wantSmm) {
    try {
      const bytes = await buildSlideDeckPdfBytes("smm", siteKey, buildOpts);
      attachments.push({
  ...
  // Combined failed and no other PDFs → last-resort separate builds
  if (wantCombined && !attachments.some((a) => a.content)) {
    if (pack.includeWebsite) {
      try {
        const bytes = await buildSlideDeckPdfBytes("website", siteKey, buildOpts);
        attachments.push({
          filename: slideDeckFilename("website", displayName || siteKey, month),
...
<html><body style="margin:0;padding:32px;background:#faf9f5;font-family:Inter,system-ui,sans-serif;color:#141413;">
  <div style="max-width:560px;margin:0 auto;background:#fff;border:1px solid #e8e6e0;border-radius:12px;padding:28px;">
    <p style="margin:0 0 8px;font-size:12px;letter-spacing:0.08em;text-transform:uppercase;color:#87867f;">RoboSEO.Ai</p>
...
export async function sendClientReportsNow({ siteKey = null, userId = null, trigger = "manual" } = {}) {
  if (!(await isClientReportsEnabled())) {
    return { ok: false, error: "Monthly reports are disabled.", results: [] };
  }
  ...
  for (const user of users) {
    ...
    const wantCombined = userWantsCombinedReport(user);
    const wantWebsite = userWantsClientWebsiteReport(user);
    const wantSmm = userWantsClientSmmReport(user);
...
export async function sendStaffDigestsNow({ userId = null, siteKey = null, trigger = "manual" } = {}) {
  ...
  for (const user of users) {
    ...
    for (const pack of packs) {
      try {
        // Staff digests: one pack PDF (combined when website exists, else SMM).
        const attachments = await buildAttachmentsForPack(pack, {
          wantWebsite: false,
          wantSmm: !pack.includeWebsite,
          wantCombined: true,
...
export async function runWeeklySlideDeckReports(logger = console) {
  ...
  logger.info?.("Running monthly report send (slide decks)...");
  const clients = await sendClientReportsNow({ trigger: "cron" });
  logger.info?.(`Monthly reports: ${clients.results?.length || 0} attempts`);
