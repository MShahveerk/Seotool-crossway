...
export async function getReportDeckConfig(siteKey, { equivalents = [] } = {}) {
  ...
  try {
    for (const key of keys) {
      ...
      try {
        const normalized = normalizeReportDeckConfig(JSON.parse(row.value));
        // Prefer a customized template over an all-on default stub on another alias key.
        if (!summarizeDeckConfig(normalized).isDefault) return normalized;
        if (!fallback) fallback = normalized;
      } catch {
        /* try next key */
      }
    }
    return fallback || defaultReportDeckConfig();
  } catch {
