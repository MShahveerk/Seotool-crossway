...
import { canAccessSection } from "../modulePermissions.js";
...
export const REPORT_SLIDE_CATALOG = [
  ...
  },
  {
    ...
    deck: "website",
    description: "Organic footprint and keyword panels",
  },
  {
    id: "backlinks",
...
export const REPORT_STAT_CATALOG = [
  ...
  { id: "exec.orgTraffic", label: "Org. traffic", slide: "executive", description: "Estimated organic traffic" },
  { id: "exec.keywords", label: "Keywords", slide: "executive", description: "Organic keyword count" },
  ...
  { id: "auth.inlinkRank", label: "Authority rank", slide: "authority", description: "Domain authority rank" },
...
/**
 * Canonical AppSetting key — always https://apex-domain (no www) when we can
 * resolve a hostname, so studio save / export / Send all share one bucket.
 */
export function configStorageKey(siteKey) {
  const raw = String(siteKey || "").trim();
  if (!raw) return null;
  const domain = toDomain(raw);
  if (domain) return `report_deck_config:https://${domain}`;
  const origin = normalizeSiteOrigin(raw);
  if (origin) {
    try {
      const u = new URL(origin);
      const host = u.hostname.replace(/^www\./i, "").toLowerCase();
      return `report_deck_config:${u.protocol}//${host}`;
    } catch {
      /* fall through */
    }
  }
  return `report_deck_config:${raw}`;
}

/** All historical / alias keys we might have written before apex-canonicalization. */
export function candidateConfigStorageKeys(siteKey, equivalents = []) {
  const inputs = [siteKey, ...(equivalents || [])].map((k) => String(k || "").trim()).filter(Boolean);
  const keys = new Set();
  for (const input of inputs) {
    const primary = configStorageKey(input);
    if (primary) keys.add(primary);
    const origin = normalizeSiteOrigin(input);
    if (origin) keys.add(`report_deck_config:${origin}`);
    const domain = toDomain(input);
    if (domain) {
      for (const proto of ["https", "http"]) {
        keys.add(`report_deck_config:${proto}://${domain}`);
        keys.add(`report_deck_config:${proto}://www.${domain}`);
      }
    }
    if (input && !primary?.endsWith(input)) keys.add(`report_deck_config:${input}`);
  }
  return [...keys];
}
...
/**
 * The canonical key is authoritative. Reading used to prefer whichever alias
 * row happened to be customized, which meant a template saved as "everything
 * on" was silently replaced on reload by a stale pre-canonicalization row —
 * the save looked successful and then reverted. Alias keys are now only a
 * fallback for sites that have never been saved through the studio.
 */
export async function getReportDeckConfig(siteKey, { equivalents = [] } = {}) {
  const keys = candidateConfigStorageKeys(siteKey, equivalents);
  if (!keys.length) return defaultReportDeckConfig();
  const canonical = configStorageKey(siteKey);
  const ordered = canonical ? [canonical, ...keys.filter((key) => key !== canonical)] : keys;
  try {
...
/**
 * Persist under the apex-domain key. Also mirrors onto websiteUrl when the
 * sidebar selection is a Meta page / alias so Send all packs resolve it.
 */
export async function setReportDeckConfig(siteKey, config, { websiteUrl = null, equivalents = [] } = {}) {
  const writeKeys = new Set();
  const primary = configStorageKey(websiteUrl || siteKey);
  if (primary) writeKeys.add(primary);
  const fromSite = configStorageKey(siteKey);
  if (fromSite) writeKeys.add(fromSite);
  for (const eq of equivalents || []) {
    const k = configStorageKey(eq);
    if (k) writeKeys.add(k);
  }
  if (!writeKeys.size) {
  ...
  // Bring any legacy alias rows (www / http / raw-input spellings) in step so
  // they can't hand a stale template back to a reader that resolves them first.
  // updateMany touches only rows that already exist, so this creates no spam.
  const aliasKeys = candidateConfigStorageKeys(websiteUrl || siteKey, [
    siteKey,
    ...(equivalents || []),
  ]).filter((key) => !writeKeys.has(key));
  if (aliasKeys.length) {
    await prisma.appSetting.updateMany({ where: { key: { in: aliasKeys } }, data: { value } });
  }
  return normalized;
}

export function canAccessReportsStudio(user) {
  return canAccessSection(user, "reports-studio");
}
