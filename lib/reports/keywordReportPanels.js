/**
 * Bucket SE Ranking domain keywords for report slides.
 */
...
function toRow(k) {
  const p = posOf(k);
  return {
    keyword: String(k.keyword || k.name || "—").slice(0, 28),
...
export function drawKeywordInsightPanels(deck, page, keywords, {
  ...
  const { ranked, crucial, traffic } = bucketKeywordsForReport(keywords, { limit });
  const gap = 12;
  const panelW = (PAGE_W - MARGIN * 2 - gap * 2) / 3;
  const panels = [
    {
      title: "Ranked",
  ...
  const headerH = 28;
  const rowH = 18;
  const usableH = Math.max(80, yTop - yBottom - headerH);
  const maxRows = Math.max(3, Math.min(limit, Math.floor(usableH / rowH)));
  ...
  panels.forEach((panel, i) => {
    ...
    y -= 12;
    page.drawText(safePdfText(panel.caption, 36), {
      x,
      y,
      size: 7,
    ...
    const rows = panel.rows.slice(0, maxRows).map(toRow);
    if (!rows.length) {
    ...
    const cols = [
      { key: "keyword", label: "Keyword", w: 0.48 },
      { key: "pos", label: "Pos", w: 0.14 },
      { key: "vol", label: "Vol", w: 0.18 },
      { key: "traf", label: "Traf", w: 0.2 },
    ];
    let cx = x;
    cols.forEach((c) => {
    ...
    rows.forEach((row, ri) => {
      ...
      let cxi = x;
      cols.forEach((c) => {
        page.drawText(safePdfText(row[c.key] ?? "", 22), {
          x: cxi + 4,
          y: y - 12,
          size: 7,
          font: deck.fonts.regular,
