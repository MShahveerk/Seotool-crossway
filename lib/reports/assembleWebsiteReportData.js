/**
 * Gather GSC + organic SEO + audit + authority + PageSpeed for website decks.
...
export async function assembleWebsiteReportData(
  ...
  /* SE Ranking only. A client report must never carry a health score whose
     provenance we can't state, so there's deliberately no fallback here. */
  result.audit = byLabel["audit-seranking"]?.data || null;
  ...
  result.authority = {
    ...
    preferredLabel: inlink != null ? "Authority rank" : "Authority (OPR)",
