...
export const COLORS = {
  ...
  slate: rgb(0.043, 0.043, 0.039), // #0b0b0a — primary ink
  slateSoft: rgb(0.08, 0.08, 0.07), // near-black body text
  muted: rgb(0.12, 0.12, 0.11), // #1f1f1c — captions (high visibility)
  cloud: rgb(0.14, 0.14, 0.13), // #242421 — small labels / eyebrows
  mist: rgb(0.86, 0.86, 0.84), // light text on dark cover slides only
  stone: rgb(0.72, 0.7, 0.67),
  border: rgb(0.72, 0.7, 0.67),
  accent: rgb(0, 0.639, 1), // #00a3ff
  accentDeep: rgb(0, 0.467, 0.8), // #0077cc
...
/**
 * Word-wrap for PDFs. Prefers breaking on spaces; only splits a token if it
 * alone is wider than maxWidth (e.g. long URLs) — never truncates mid-word.
 */
...
async function loadBrandLogos(pdf) {
  ...
  const logo = await embedPngCandidate(pdf, [
    path.join(root, "brand", "roboseo-mark.png"),
    path.join(root, "brand", "roboseo-lockup.png"),
...
export async function createSlideDeck({
  ...
  function drawLineChart(page, series, {
    ...
    const points = (series || [])
      .map((p) => {
        let val = Number(p[valueKey] ?? p.clicks ?? p.value ?? 0);
        // Scale CTR decimals to percentages for cleaner labels and trends
        if (valueKey === "ctr" && Math.abs(val) <= 1) {
          val = val * 100;
        }
        return {
          value: val,
    ...
    const padL = 50; // Increased padding to prevent wide tick labels from overlapping border
    const padR = 8;
    const padT = 16; // Added top padding for titles & summaries
    const padB = 22;
    const plotX = x + padL;
    const plotY = y + padB;
    ...
    const min = reverseY ? Math.min(...points.map((p) => p.value), 1) : 0;
    ...
    for (let i = 0; i <= ticks; i += 1) {
      const t = i / ticks;
      const val = reverseY ? max - span * t : min + span * t;
      ...
      // Format label values neatly
      let label = "";
      if (valueKey === "ctr") {
        label = `${val.toFixed(1)}%`;
      } else if (valueKey === "position") {
        label = `${val.toFixed(1)}`;
      } else {
        if (val >= 1000000) {
      ...
      page.drawText(label, {
        x: plotX - 8 - fonts.regular.widthOfTextAtSize(label, 7), // 8px margin from the Y axis line
        y: ty - 2,
        size: 7,
        font: fonts.regular,
        color: COLORS.muted,
    ...
    // Summaries within chart boxes for comprehensiveness
    const sum = points.reduce((acc, p) => acc + p.value, 0);
    const avg = sum / points.length;
    const peak = max;
    
    let summaryText = "";
    if (valueKey === "clicks" || valueKey === "impressions") {
      const fmtAvg = avg >= 1000 ? `${(avg / 1000).toFixed(1)}K` : Math.round(avg);
      const fmtPeak = peak >= 1000 ? `${(peak / 1000).toFixed(1)}K` : Math.round(peak);
      summaryText = `Avg: ${fmtAvg}/d  •  Peak: ${fmtPeak}`;
    } else if (valueKey === "ctr") {
      summaryText = `Avg: ${avg.toFixed(2)}%  •  Peak: ${peak.toFixed(2)}%`;
    } else if (valueKey === "position") {
      const best = Math.min(...points.map((p) => p.value));
      summaryText = `Avg: ${avg.toFixed(1)}  •  Best: ${best.toFixed(1)}`;
    }

    if (summaryText) {
      page.drawText(summaryText, {
        x: x + width - 12 - fonts.regular.widthOfTextAtSize(summaryText, 6.5),
        y: y + height - 12,
        size: 6.5,
        font: fonts.regular,
        color: COLORS.muted,
    ...
    for (let i = 1; i < points.length; i += 1) {
      ...
      const val1 = points[i - 1].value;
      const val2 = points[i].value;
      const y1 = reverseY ? plotY + ((max - val1) / span) * plotH : plotY + ((val1 - min) / span) * plotH;
      const y2 = reverseY ? plotY + ((max - val2) / span) * plotH : plotY + ((val2 - min) / span) * plotH;
      page.drawLine({
        start: { x: x1, y: y1 },
        end: { x: x2, y: y2 },
        thickness: 2,
  ...
  /** Horizontal bar chart for ranked items. Labels wrap on word boundaries. */
  function drawBarList(page, rows, { x, yTop, width, maxRows = 6 } = {}) {
    ...
    let y = yTop;
    const labelW = Math.max(40, width - 52);
    slice.forEach((row) => {
      const val = Number(row.value) || 0;
      const valText = safePdfText(row.display ?? String(val));
      const labelLines = wrapText(safePdfText(row.label || "", 220), fonts.regular, 8, labelW).slice(0, 3);
      ...
      y -= Math.max(12, labelLines.length * 10 + 2);
