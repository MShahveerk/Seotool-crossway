  ...
  // Site audit
  {
    ...
    if (score != null || errors || warnings) {
      deck.drawKpiCards(page, [
        ...
        { label: "ERRORS", value: nfExact(errors) },
        { label: "WARNINGS", value: nfExact(warnings) },
        { label: "NOTICES", value: nfExact(notices) },
      ]);
      ...
      if (Array.isArray(issues) && issues.length) {
        deck.drawTable(
          ...
          issues.slice(0, 8).map((iss) => ({
            ...
            count: nfExact(iss.count ?? iss.pages),
          })),
