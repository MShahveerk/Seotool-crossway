...
async function appendWebsiteSlides(deck, data, { includeInternal = false, reportMonth = null } = {}) {
  const gsc = data.gsc;
  ...
  // ── Blogs created & published this month (cards with image + description) ──
  {
    ...
    if (!blogs.length) {
      try {
        const pack = await loadPublishedBlogsForReport(
          data.siteUrl || data.siteKey,
          reportMonth,
          { limit: 9 }
        );
        blogs = pack.blogs || [];
...
async function drawContentCards(deck, page, items) {
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    const excerptLines = deck.wrapText(
      safePdfText(item.excerpt || "No description available.", 160),
      deck.fonts.regular,
      9,
      cardW - 24
    );
...
async function appendSmmSlides(deck, smm, { siteKey = "", reportMonth = null } = {}) {
  const page = deck.addSlide();
...
export async function buildSlideDeckPdfBytes(
  ...
  if (kind === "smm" || kind === "combined") {
    ...
    const smmSummary = await appendSmmSlides(deck, smm, { siteKey, reportMonth: month });
