...
async function appendWebsiteSlides(deck, data, { includeInternal = false } = {}) {
  ...
  // ── 1. Executive dashboard — search command center (distinct from auth page) ──
  {
    ...
    const qBars = (gsc?.topQueries || []).slice(0, 7).map((q) => ({
      label: String(q.query || q.keys?.[0] || "—").slice(0, 28),
  ...
  // ── 2. Authority & Performance ──
  {
    ...
    const authRows = [
      ...
      ["Dofollow", nf(bl?.dofollowBacklinks)],
      ["Nofollow", nf(bl?.nofollowBacklinks)],
      ["Org. traffic", nf(ov?.traffic)],
      ["Org. keywords", nf(ov?.keywords)],
      ["Traffic value", ov?.price != null ? `$${nf(ov.price)}` : "—"],
      ["Top 10 keywords", nf(ov?.top10)],
    ];
    ...
    cats.forEach((c, i) => {
      ...
      page.drawText(value, {
        x: midX + midW - 14 - deck.fonts.bold.widthOfTextAtSize(value, 20),
        y: rowTop - 4,
        size: 20,
        font: deck.fonts.bold,
        color: scoreTone(c[1]),
    ...
    if (lab.length) {
      ...
      lab.forEach((m, i) => {
        page.drawText(safePdfText(`${m.title}: ${m.displayValue || "—"}`, 42), {
          x: midX + 14,
          y: 60 - i * 13,
          size: 8,
          font: deck.fonts.regular,
  ...
  // ── Published blogs (cards with image + link) ──
  {
    ...
    if (blogs.length) {
      for (let i = 0; i < blogs.length; i += 3) {
        ...
        deck.drawSlideTitle(
          page,
          "Content published",
          `${data.blogsMonthLabel || period} · ${blogs.length} post${blogs.length === 1 ? "" : "s"}`
        );
        await drawBlogCards(deck, page, chunk);
      }
    }
  }
...
async function drawBlogCards(deck, page, blogs) {
  const gap = 14;
  const cols = Math.min(3, blogs.length);
  ...
  for (let i = 0; i < blogs.length; i += 1) {
    ...
    if (blog.imageBytes && blog.imageKind) {
      try {
        const embedded =
          blog.imageKind === "png"
            ? await deck.pdf.embedPng(blog.imageBytes)
            : await deck.pdf.embedJpg(blog.imageBytes);
        const maxW = cardW - 2;
        const maxH = imgH;
    ...
    let ty = y - imgH - 22;
    const titleLines = deck.wrapText(safePdfText(blog.title, 80), deck.fonts.bold, 11, cardW - 24);
    ...
    ty -= 4;
    const excerptLines = deck.wrapText(
      safePdfText(blog.excerpt || "No description available.", 160),
      deck.fonts.regular,
      9,
      cardW - 24
    );
    ...
    if (blog.publishedAt) {
      const when = new Date(blog.publishedAt).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
      });
      page.drawText(safePdfText(when), {
        x: x + 12,
        y: y - cardH + 36,
        size: 8,
        font: deck.fonts.semibold,
    ...
    const link = safePdfText(blog.url || "", 70);
    if (link) {
      page.drawText("READ", {
        x: x + 12,
        y: y - cardH + 20,
