...
import { canAccessSection, MODULES, resolveModulePermissions } from "./modulePermissions";
...
export async function resolveWebsiteAccess(req) {
  ...
  const seoPerms = resolveModulePermissions(session.user)[MODULES.SEO] || [];
  if (seoPerms.length === 0 && session.user.role !== ROLES.SUPER_ADMIN) {
    const err = new Error("Forbidden: SEO tools access not granted.");
    err.status = 403;
    throw err;
  }
